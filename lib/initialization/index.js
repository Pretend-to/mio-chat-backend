...
export async function performFullInitialization() {
  ...
  // 3. 先跑代码自带的结构迁移（幂等 + 事务 + 行数校验）：需要重建表的变更在这里完成，
  //    随后 schema 检查看到的就是空 diff，不会触发任何自动重建。
  const { runSchemaMigrations } = await import('../migration/schemaRunner.js')
  await runSchemaMigrations()
  // 4. 检查 Schema 同步（如果变更则会在此处 exit 并重启）
  await checkAndSyncSchema()
  // 5. 然后执行常规修复（表缺失等）
  await checkAndFixPrisma()
  // 6. 确保 TokenCipher 加解密密钥就绪
