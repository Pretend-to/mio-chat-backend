...
export async function performFullInitialization() {
  ...
  // 2. 检查 Schema 同步（如果变更则会在此处 exit 并重启）
  await checkAndSyncSchema()
  // 3. 然后执行常规修复（表缺失等）
  await checkAndFixPrisma()
  // 4. 在任何 Channel/HTTP 服务启动前，一次性迁移存量 JSON 数据。
  //    迁移失败会阻止本次启动，旧文件和版本快照均保留。
  const prisma = await getLocalPrismaClient()
  try {
    const { runLegacyMigrationBootstrap } = await import('../chat/persistence/LegacyMigrationBootstrap.js')
    await runLegacyMigrationBootstrap({ prisma, rootDir: process.cwd(), logger })
  } catch (error) {
    logger.error('❌ Channel 存储初始化失败，服务保持停止。旧 JSON 与迁移快照未删除。')
    logger.error(`👉 修复原因后重启；错误代码: ${error.code || 'CHANNEL_STORAGE_INIT_FAILED'}；${error.message}`)
    throw error
  } finally {
    await prisma.$disconnect()
  }
