...
export default class Prodia {
    ...
    async getText2Img(config,engine = "sd") {
        const { data } = engine === "sd"? await this.sdk.generate(config) : await this.sdk.sdxlGenerate(config);
        const result = await this.getJob(data.job);
        return result.imageUrl;
    }
    async getImg2Img(config,engine = "sd") {
        const { data } = engine === "sd"? await this.sdk.transform(config) : await this.sdk.sdxlTransform(config);
        const result = await this.getJob(data.job);
        return result.imageUrl;
    }
