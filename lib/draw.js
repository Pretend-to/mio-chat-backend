...
import { getImgUrl,getBigImgUrl } from "./prodia.js";
...
export default class Drawer {
    ...
    async draw(engine, prompt, config) {
        ...
        case "prodia.biggerimg":
            try {
                const data = {
                    resize: Number(prompt),
                    imageData: config.base64 ? config.base64 : null,
                }

                const imgUrl = await getBigImgUrl(data);
        ...
        case "prodia.img2img":
            try {
                ...
                const imgUrl = await getImgUrl(data);
