...
const baseNega =
  'EasyNegative, lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, lowquality, normal quality, jpeg artifacts, signature, watermark, username, blurry'
...
export default class Drawer {
  ...
  async draw(engine, prompt, config) {
    ...
    logger.info(
      '[' + id + '] 开始使用' + engine + '-' + config.model + '进行绘画...',
    )
    ...
    switch (engine) {
      case 'prodia.v1':
        try {
          const data = {
            ...
            sampler: config.sampler_index
              ? config.sampler_index
              : 'DPM++ 2M Karras',
          }
      ...
      case 'pixart.a':
        try {
            ...
            pixart.a(
              {
                ...
                data: {
                  ...
                  dpm_guidance_scale: config.dpm_guidance_scale
                    ? config.dpm_guidance_scale
                    : 4.5,
                  dpm_inference_steps: config.dpm_inference_steps
                    ? config.dpm_inference_steps
                    : 14,
                  sa_guidance_scale: config.sa_guidance_scale
                    ? config.sa_guidance_scale
                    : 3,
                  sa_inference_steps: config.sa_inference_steps
                    ? config.sa_inference_steps
                    : 25,
                },
              },
              (err, data) => {
          ...
          // logger.debug(data); // 可以在这里处理返回的数据
