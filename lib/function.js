...
class MioFunction {
  ...
  async run(e) {
    const access = evaluateToolAccess(this, e, e?.isMetaCall ? 'meta_call' : 'execute')
    if (!access.allowed) {
  ...
  getDescription(_context = null) {
    return this.description
  }

  getParameters(_type = null, _context = null) {
  ...
  toSchema(type, context = null) {
    return this.json(type, context)
  }

  toCatalogEntry(context = null) {
    if (!this.#canProject(context, 'catalog')) return null
    return {
      description: this.getDescription(context),
      name: this.name,
      parameters: this.getParameters(null, context),
    }
  }

  toMetaSummary(context = null) {
    if (!this.#canProject(context, 'meta')) return null
    return {
      description: this.getDescription(context),
      group: this.parentPlugin?.name || null,
      name: this.name.split('_mid_')[0],
    }
  }

  toMetaSchema(context = null) {
    const summary = this.toMetaSummary(context)
  ...
  #canProject(context, operation) {
    return evaluateToolAccess(this, context, operation).allowed
  }
  ...
  async saveTextFile(baseUrl, data, name) {
    try {
      ...
      return formatAssetUrl(baseUrl, result.url)
    } catch (error) {
