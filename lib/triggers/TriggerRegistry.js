...
export class TriggerRegistry {
  constructor({ dataDir = DEFAULT_DATA_DIR, mode = null, prisma = null } = {}) {
    mode ||= dataDir !== DEFAULT_DATA_DIR ? 'legacy' : 'database'
  ...
  async create(opts = {}) {
    ...
    const trigger = {
      ...
      deliveryBindingId: opts.deliveryBindingId || null,
      deliveryMode: normalizeDeliveryMode(
        opts.deliveryMode ||
          (opts.deliveryBindingId
            ? DELIVERY_MODES.CHANNEL
            : DELIVERY_MODES.DEFAULT),
      ),
      cooldownSec:
  ...
  async remove(id, { agentId } = {}) {
    ...
    const triggers = await this._loadTriggers()
    const idx = triggers.findIndex(
      (t) => t.id === id && !t.deletedAt && (!agentId || t.agentId === agentId),
    )
  ...
  async getExecution(id) {
    ...
    if (this._usesDatabase()) {
      const prisma = await this._database()
      const row = await prisma.triggerExecution.findUnique({
        where: { id: String(id) },
      })
      return row ? this._executionFromDatabase(row) : null
    }
    const executions = await this._loadExecutions()
    return executions.find((execution) => execution.id === String(id)) || null
  }
  ...
  async updateExecution(id, patch = {}) {
    ...
    const data = {
      ...
      ...(Object.hasOwn(patch, 'durationMs')
        ? {
            durationMs: Number.isFinite(patch.durationMs)
              ? Math.round(patch.durationMs)
              : 0,
          }
        : {}),
    ...
    if (this._usesDatabase()) {
      const prisma = await this._database()
      try {
        const row = await prisma.triggerExecution.update({
          data,
          where: { id: String(id) },
        })
        return this._executionFromDatabase(row)
    ...
    const index = executions.findIndex(
      (execution) => execution.id === String(id),
    )
    ...
    executions[index] = {
      ...
      ...patch,
      ...(Object.hasOwn(patch, 'firedAt')
        ? { firedAt: patch.firedAt ? new Date(patch.firedAt).getTime() : null }
        : {}),
    }
    await this._saveExecutions()
