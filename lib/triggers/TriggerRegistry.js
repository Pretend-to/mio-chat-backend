...
export class TriggerRegistry {
  constructor({
    ...
    mode ||= process.env.MIO_CHANNEL_PERSISTENCE_MODE || (dataDir !== DEFAULT_DATA_DIR ? 'legacy' : 'database')
  ...
  async _resolveDatabaseReferences(prisma, { agentId, deliveryBindingId, sourceChannelId, sessionId }) {
    if (!agentId || !sessionId) throw new Error('Trigger requires agentId and sessionId')
    const [agent, session, binding, sourceChannel] = await Promise.all([
      prisma.agent.findUnique({ where: { id: agentId } }),
      prisma.session.findUnique({ where: { id: sessionId } }),
      deliveryBindingId ? prisma.agentChannelBinding.findUnique({ where: { id: deliveryBindingId } }) : null,
      sourceChannelId ? prisma.channel.findUnique({ where: { id: sourceChannelId } }) : null,
    ])
    if (!agent) throw new Error(`agent ${agentId} not found`)
    if (!session || session.agentId !== agentId) throw new Error(`session ${sessionId} not found for agent ${agentId}`)
    if (deliveryBindingId && (!binding || binding.agentId !== agentId)) throw new Error(`delivery binding ${deliveryBindingId} not found for agent ${agentId}`)
    if (sourceChannelId && !sourceChannel) throw new Error(`source channel ${sourceChannelId} not found`)
  }
  ...
  async _saveTriggers() {
    ...
    await fs.promises.writeFile(
      tmp,
      JSON.stringify(this._triggersCache ?? [], null, 2),
      UTF8,
    )
  ...
  async _saveExecutions() {
    ...
    await fs.promises.writeFile(
      tmp,
      JSON.stringify(this._executionsCache ?? [], null, 2),
      UTF8,
    )
  ...
  async get(id, { agentId, includeDeleted = false } = {}) {
    if (this._usesDatabase()) {
      ...
      if (agentId) where.agentId = agentId
      return this._fromDatabase(
        await prisma.trigger.findFirst({ where }),
      )
    }
    const triggers = await this._loadTriggers()
    return (
  ...
  async list(filter = {}) {
    ...
    return triggers.filter((t) => {
      ...
      if (filter.enabled !== undefined && t.enabled !== filter.enabled)
        return false
      return true
    })
  }
  ...
  async update(id, patch = {}, scope = {}) {
    if (scope.agentId && patch.agentId && patch.agentId !== scope.agentId) {
      return null
    }
    if (this._usesDatabase()) return await this._updateDatabase(id, patch, scope)
    const triggers = await this._loadTriggers()
    const idx = triggers.findIndex(
      (t) =>
        t.id === id &&
        !t.deletedAt &&
        (!scope.agentId || t.agentId === scope.agentId),
    )
  ...
  async remove(id, { agentId } = {}) {
      ...
      if (current.scriptPath) {
        try {
          ...
          if (error.code !== 'ENOENT')
            console.warn(
              `[TriggerRegistry] 删除脚本文件失败: ${current.scriptPath}`,
              error,
            )
        }
      }
    ...
    if (deleted.scriptPath) {
      try {
        ...
        if (e.code !== 'ENOENT')
          console.warn(
            `[TriggerRegistry] 删除脚本文件失败: ${deleted.scriptPath}`,
            e,
          )
      }
    }
  ...
  async listExecutions(triggerId = null, { limit = 50 } = {}) {
    ...
    const executions = await this._loadExecutions()
    let list = triggerId
      ? executions.filter((e) => e.triggerId === triggerId)
      : executions
