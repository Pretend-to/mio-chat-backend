...
export class WakeInjector {
  constructor({ registry, dispatcher = null, onWakeStatus = null } = {}) {
    ...
    this._workItemStatusByExecutionId = new Map()
    this._startedExecutionIds = new Set()
  ...
  async _handleWorkItemStatus({ status, workItem, previousStatus } = {}) {
    ...
    while (this._workItemStatusByExecutionId.size > 1024) {
      const oldest = this._workItemStatusByExecutionId.keys().next().value
      this._workItemStatusByExecutionId.delete(oldest)
    }
    ...
    if (startedNow) {
      ...
      while (this._startedExecutionIds.size > 1024) {
        const oldest = this._startedExecutionIds.values().next().value
        this._startedExecutionIds.delete(oldest)
      }
    }
    const statusMap = {
      accepted: 'queued',
