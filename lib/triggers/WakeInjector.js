...
export class WakeInjector {
  ...
  async processWake(trigger, payload = {}, opts = {}) {
    ...
    if (trigger.lastFiredAt && now - trigger.lastFiredAt < cooldownMs) {
      ...
      return { injected: false, reason: skipReason }
    }
    ...
    if (this.registry) {
      ...
      const recentLogs = await this.registry.listExecutions(trigger.id, { limit: 100 })
      const todayWakeCount = recentLogs.filter((l) => l.wake && l.firedAt >= todayStart).length
    ...
    if (this.channelRuntime) {
      // 优先根据 agentId 查找运行中的渠道
      const running = Array.from(this.channelRuntime.running.values())
      const entry = running.find((e) => e.chn?.memory?.agentId === trigger.agentId || e.chn?.id === trigger.agentId)
      targetChannel = entry?.chn
    }

    if (!targetChannel) {
      // 若 runtime 中未找到，尝试直接查找任意 active channel
      const activeEntries = this.channelRuntime ? Array.from(this.channelRuntime.running.values()) : []
      if (activeEntries.length > 0) {
        targetChannel = activeEntries[0].chn
      }
    }
    ...
    if (targetChannel && typeof targetChannel.appendUserMessage === 'function') {
      try {
        ...
        await targetChannel.appendUserMessage(sid, wakeMessage, { source: 'trigger', triggerId: trigger.id })
        injected = true
      } catch (e) {
        injectError = `注入 Channel 会话失败: ${e.message}`
      }
    } else {
      injectError = `未找到运行中的 Channel 或渠道未实现 appendUserMessage`
    }
    ...
    if (this.registry) {
      ...
      if (injected) {
        if (trigger.mode === 'once') {
          ...
          await this.registry.remove(trigger.id)
        } else {
          // persistent 模式：更新计数与最后唤醒时间
          await this.registry.update(trigger.id, {
