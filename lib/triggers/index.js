...
export class TriggerService {
  constructor(opts = {}) {
    ...
    this.injector.onWakeStatus = (event) => this._handleWakeStatus(event)
    this._processes = new Map()
    this._runtimeStates = new Map()
    this._restartAttempts = new Map()
    this._restartTimers = new Map()
    this._wakeStatusCache = new Map()
    this._schedulerStarted = false
    this._stopping = false
  }

  setChannelRuntime(_channelRuntime) {
    // Kept for the ChannelRuntime lifecycle adapter. Trigger wakes now route
    // through ChatEventDispatcher and do not depend on a live channel instance.
  }
  ...
  async startScheduler() {
    ...
    this.injector.startListening?.()
    await this.injector.reconcileOpenWorkItems?.()
  ...
  stopScheduler() {
    ...
    for (const state of this._processes.values()) {
      ...
      state.handle?.stop('SIGKILL')
    }
    this.runner.stopAll?.({ force: true })
  }
  ...
  async _reconcile() {
    ...
    for (const result of results) {
      if (result.status === 'rejected') {
        console.error(
          '[TriggerService] failed to start sentinel:',
          result.reason,
        )
      }
    }
      ...
      state.handle = this.runner.startScript(trigger, {
        ...
        onExit: (result) => {
          return this._handleProcessExit(trigger, state, result).catch(
            (error) =>
              this._recoverAfterExitHandlingFailure(trigger, state, error),
          )
        },
        onWake: (payload, meta) => {
  ...
  async stopTriggersByAgent(agentId) {
    const triggers = await this.registry.list({ agentId })
    for (const trigger of triggers)
      await this.stopTrigger(trigger.id, { agentId })
  }
  ...
  _runtimeSnapshot(state) {
    return {
      ...
      pid: state.pid,
      pendingExecutionId: state.pendingExecutionId,
  ...
  async _handleScriptWake(trigger, state, payload, meta = {}) {
    ...
    if (wakeResult?.accepted && wakeResult.executionId) {
      state.pendingExecutionId = wakeResult.executionId
      state.wakeStarted = false
      state.restartRequested = false
      state.status = 'wake_queued'
      this._rememberRuntimeState(state)
      const earlyStatus = this._wakeStatusCache.get(wakeResult.executionId)
      if (earlyStatus) {
        this._wakeStatusCache.delete(wakeResult.executionId)
        await this._handleWakeStatus(earlyStatus)
      } else if (wakeResult.started) {
        await this._handleWakeStatus({
          executionId: wakeResult.executionId,
          started: true,
  ...
  async _handleWakeStatus(event = {}) {
    const { executionId, status, started } = event
    if (!executionId) return
    const state = [...this._processes.values()].find(
      (candidate) =>
        candidate.pendingExecutionId === executionId ||
        candidate.lastExecutionId === executionId,
    )
    if (!state) {
      this._wakeStatusCache.delete(executionId)
      this._wakeStatusCache.set(executionId, event)
      while (this._wakeStatusCache.size > 1024) {
        const oldest = this._wakeStatusCache.keys().next().value
        this._wakeStatusCache.delete(oldest)
      }
      return
    }
    const trigger = await this.registry.get(state.id, {
    ...
    if (started) {
      ...
      state.wakeStarted = true
      state.status = 'woken'
      this._rememberRuntimeState(state)
      if (state.lastExitAt) {
        await this._resumeAfterWake(state, trigger)
      }
      return
    }
    ...
    if (status === 'failed') {
      state.pendingExecutionId = null
      state.wakeStarted = false
      state.lastError = 'Queued wake failed before it started'
      state.status = 'wake_skipped'
      this._rememberRuntimeState(state)
      if (state.lastExitAt) {
        await this._resumeAfterWake(state, trigger)
      }
    ...
    if (status === 'needs_attention') {
      ...
      state.wakeStarted = false
      state.stopRequested = true
      state.lastError = 'Queued wake needs attention'
      state.status = 'needs_attention'
      this._rememberRuntimeState(state)
      if (state.lastExitAt) {
        if (state.stableTimer) clearTimeout(state.stableTimer)
        this._processes.delete(state.id)
      }
    }
  }

  async _resumeAfterWake(state, trigger) {
    if (state.stableTimer) clearTimeout(state.stableTimer)
    if (
      state.stopRequested ||
      this._stopping ||
      !trigger ||
      !trigger.enabled ||
      (state.wakeStarted && trigger.mode === 'once')
    ) {
      state.status = state.wakeStarted ? 'stopped' : 'disabled'
      this._rememberRuntimeState(state)
      this._processes.delete(state.id)
      return
    }
  ...
  async _handleProcessExit(trigger, state, result) {
    ...
    if (current.mode === 'once' && state.wakeStarted) {
      if (state.stableTimer) clearTimeout(state.stableTimer)
      this._processes.delete(trigger.id)
      return
    }
