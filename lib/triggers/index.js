...
export class TriggerService {
  ...
  setChannelRuntime(channelRuntime) {
    ...
    if (this.injector.sessionTurnService) this.injector.sessionTurnService.channelRuntime = channelRuntime
  }
  ...
  async stopTrigger(id, { agentId } = {}) {
    ...
    if (state) {
      ...
      if (state.handle?.done) {
        await Promise.race([state.handle.done, new Promise((r) => setTimeout(r, 3000))])
      }
  ...
  async stopTriggersByAgent(agentId) {
    ...
    for (const trigger of triggers) await this.stopTrigger(trigger.id, { agentId })
  }
