...
constructor() {
  ...
  async loadLLMAdapters() {
    ...
    // 启动时审计存量模型与注册表的匹配情况
    try {
      const { modelRegistryService } = await import('./chat/llm/adapters/registry.js')
  ...
  async broadcastPluginsUpdate() {
    try {
      ...
      logger.info(`[插件系统] 已向 ${clients.length} 个活跃端推送插件更新通知并触发系统钩子`)
    } catch (error) {
  ...
  _setupPluginWatchers() {    // --- 内置 Hooks 热重载监听 ---
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      hooksWatcher.on('all', async (event, filePath) => {
        logger.info(`[热重载] 检测到内置钩子变化 (${event}): ${path.basename(filePath)}`)
