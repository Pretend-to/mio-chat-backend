...
export default class Middleware {
    ...
    sendServer(e) {
        try {
            ...
            case "message":{
                logger.debug("收到私聊消息");
                const warpedMessage = this.onebot.messageWarrper(data.data, sender,data.message_id);
    ...
    // 向客户端发送消息
    sendClient(proctol, data) {
        switch (proctol) {
        case "onebot":
            this.wsServer.sendOnebot(data);
            break;
        case "openai":
            this.wsServer.sendOpenai(data);
            break;
        default:
            logger.warn("未知的协议类型");
            break;
