...
export default class {
  ...
  async loadLLMAdapters() {
    ...
    if (availableInstanceList.length === 0) {
      logger.info(
        '当前没有配置任何 LLM 适配器实例，LLM 模块已初始化，可通过管理界面动态添加',
      )
    } else {
        ...
        availableInstanceList.map((instance) => {
          ...
          return new Promise((resolve) => {
            this.llm.initServer(
              ...
              (error) => {
                if (error) {
                  resolve({ displayName, error, success: false })
                } else {
                  logger.debug(`[${displayName}] 加载成功`)
                  instance.setAvailable()
                  resolve({ displayName, success: true })
                }
              },
            )
          })
    ...
    try {
      ...
      modelRegistryService.auditConfiguredModels(this.llm)
    } catch (auditError) {
      logger.warn('[ModelRegistry] 启动模型匹配审计失败:', auditError.message)
    }
  ...
  async reloadLLMAdapters() {
        ...
        this.llm.initServer(
          ...
          (error) => {
            if (error) {
              ...
              resolve({ displayName, error: error.message, success: false })
            } else {
    ...
    try {
      ...
      logger.warn('[ModelRegistry] 热重载模型匹配审计失败:', auditError.message)
    }

    const { default: sessions } =
      await import('./server/socket.io/services/sessions.js')
    ...
    for (const client of clients) {
      ...
      const filteredProviders = enrichedProviders.filter((provider) => {
        ...
        const instanceModels = filteredModels[instanceId]
        return instanceModels && instanceModels.length > 0
      })
    ...
    logger.info(
      `[热重载] 已向 ${clients.length} 个客户端推送模型更新（根据权限筛选）`,
    )
    logger.info(
      `LLM适配器热重载完成: ${loadResults.filter((r) => r.success).length}/${loadResults.length} 成功`,
    )

    return { loadResults, models, providers: enrichedProviders }
  }
  ...
  async broadcastPluginsUpdate() {
    try {
      // 触发内部系统钩子
      await hookManager.execute(HOOK_POINTS.PLUGINS_UPDATED, {
      ...
      logger.info(
        `[插件系统] 已向 ${clients.length} 个活跃端推送插件更新通知并触发系统钩子`,
      )
    } catch (error) {
      logger.error('[插件系统] 广播插件更新失败:', error)
    }
  }
  ...
  initSocketServer() {
    ...
    this.socketServer.setClientMessageHandler('llm', (e) => {
      if (this.llm) {
        ...
        // 发送错误消息给客户端
        e.error(
          new Error('未配置任何 LLM 适配器，请在管理界面中添加适配器配置'),
        )
      }
    })
  }
  ...
  /**
   * 智能工具查找：支持完整名称匹配和去掉 Hash 后缀的模糊匹配
   */
  findTool(fullName) {
    // 1. 尝试精确匹配 (带 Hash)
    for (const plugin of this.plugins) {
      if (plugin.tools.has(fullName)) {
        return { exact: true, tool: plugin.tools.get(fullName) }
      }
    }

    // 2. 尝试去掉 _mid_ 后的模糊匹配
    const baseName = fullName.split('_mid_')[0]
    const suggestions = []

    for (const plugin of this.plugins) {
      for (const [name, tool] of plugin.tools) {
        if (name.split('_mid_')[0] === baseName) {
          suggestions.push({ name, tool })
        }
      }
    }
    ...
    if (suggestions.length > 0) {
      return {
        ...
        tool: suggestions[0].tool, // 默认返回第一个匹配项
      }
    }
  ...
  async loadPlugin(plugin) {
    ...
    // ── 生命周期：工具加载完成后 ──
    await hookManager.execute(HOOK_POINTS.PLUGIN_TOOLS_LOADED, {
      plugin,
      toolsCount: plugin.tools?.size || 0,
    })
  ...
  _setupPluginWatchers() {
    // --- 内置 Hooks 热重载监听 ---
    const hooksDir = path.join(process.cwd(), 'lib/hooks/builtins')
    if (fs.existsSync(hooksDir)) {
      ...
      hooksWatcher.on('all', async (event, filePath) => {
        logger.info(
          `[热重载] 检测到内置钩子变化 (${event}): ${path.basename(filePath)}`,
        )
        const { loadBuiltinHooks } = await import('./hooks/index.js')
        await loadBuiltinHooks()
      })

      this._pluginWatchers.push(hooksWatcher)
    }
    ...
    const debouncedReload = this._debounceByPath((folderPath) => {
      this._reloadPluginByPath(folderPath)
    }, 600)

    const debouncedLoad = this._debounceByPath((folderPath) => {
    ...
    for (const basePath of bases) {
      ...
      watcher
        ...
        .on('add', (filePath) => {
          // Copying a whole plugin directory can emit addDir before index.js
          // exists. Handle the later index.js add so the new plugin is loaded.
          ...
          logger.info(`[PluginWatcher] 检测到新插件入口: ${rel}/index.js`)
          debouncedLoad(folderPath)
        })
        .on('change', (filePath) => {
          if (!filePath.endsWith(`${path.sep}index.js`)) {
            return
        ...
        .on('error', (err) => logger.error('[PluginWatcher] 监听错误:', err))
  ...
  async _reloadPluginByPath(folderPath) {
    ...
    try {
      ...
      const pluginUrl = `${pathToFileURL(indexJs).toString()}?t=${Date.now()}`
      const pluginModule = await import(pluginUrl)
      ...
      logger.error(`[PluginWatcher] 插件重载失败: ${pluginName}:`, error)
    }
  }
  ...
  async _loadPluginByPath(folderPath) {
    ...
    if (this.plugins.some((p) => p.pluginPath === folderPath)) {
      logger.debug(`[PluginWatcher] 插件已存在，跳过加载: ${pluginName}`)
      return
    }
    ...
    try {
      ...
      logger.error(`[PluginWatcher] 新插件加载失败: ${pluginName}:`, error)
    }
  }
  ...
  async _unloadPluginByPath(folderPath) {
    ...
    try {
      ...
      // ── 级联清理已被彻底删除的插件在数据库中的残留配置 ──
      try {
        const PluginConfigService = (
          await import('./database/services/PluginConfigService.js')
        ).default
        await PluginConfigService.delete(pluginName)
        logger.info(
          `[PluginWatcher] 插件物理目录已删除，已级联清理数据库配置: ${pluginName}`,
        )
      } catch (dbErr) {
        logger.debug(
          `[PluginWatcher] 清理插件数据库配置记录跳过: ${pluginName}`,
          dbErr.message,
        )
      }
      ...
      logger.error(`[PluginWatcher] 插件卸载失败: ${pluginName}:`, error)
    }
  }
  ...
  getAllLocalPresets() {
    return this.plugins.reduce(
      (acc, plugin) => [...acc, ...(plugin.localPresets || [])],
      [],
    )
  }
}
