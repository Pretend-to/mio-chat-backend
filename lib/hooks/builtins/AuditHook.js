...
export default class AuditHook extends BaseHook {
  ...
  async [HOOK_POINTS.TOOL_BEFORE_EXECUTE](ctx) {
    ...
    logger.debug(`[审计] 用户 ${ctx.user?.name || '未知'} 准备执行工具: ${ctx.tool.name}`)
    return true
  }
  ...
  async [HOOK_POINTS.TOOL_AFTER_EXECUTE](ctx) {
    ...
    logger.info(`[审计] 工具执行完成: ${ctx.tool.name} | 耗时: ${duration}ms | 用户: ${ctx.user?.name || 'Guest'}`)
    return true
  }
  ...
  async [HOOK_POINTS.LLM_AFTER_CHAT](ctx) {
    ...
    let logMsg = `[审计] 对话完成 | 模型: ${model} | 用户: ${user?.name || 'Guest'} | Tokens: P=${prompt}, C=${candidates}`
