...
export default class AutoTitleHook extends BaseHook {
  ...
  async [HOOK_POINTS.LLM_BEFORE_CHAT](ctx) {
    const { event, instanceId, llmService, providerName } = ctx
    if (!event || !llmService) {
      return true
    }
    ...
    if (
      ...
      this._triggerSummarize(event, llmService, [...messages], {
        instanceId,
        model: event.settings?.base?.model || null,
        provider: providerName || event.settings?.provider || null,
      }).catch(
        (error) => {
  ...
  async _triggerSummarize(event, llmService, messages, modelTarget) {
    logger.info(
      `[AutoTitleHook] 开始为会话 ${event.contactorId} 调用 LLM 生成标题...`,
    )
    const newTitle = await llmService.generateChatTitle(messages, modelTarget)
    if (!newTitle) {
