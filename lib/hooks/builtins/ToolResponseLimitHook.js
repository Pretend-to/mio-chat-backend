...
export default class ToolResponseLimitHook extends BaseHook {
  constructor(options = {}) {
    ...
    // 默认 100k 字符。落盘内容不进上下文，阈值只决定"多大算大"，
    // 100k ≈ 3.3 万 tokens，对 200k 上下文安全，且覆盖更多合法输出
  ...
  async [HOOK_POINTS.TOOL_AFTER_EXECUTE](ctx) {
    ...
    try {
      ...
      logger.error(`[ToolResponseLimit] 写入 /tmp 失败: ${error.message}，降级为截断`)
    }
  ...
  _cleanupOldFiles() {
    try {
      ...
      logger.debug(`[ToolResponseLimit] 清理旧落盘文件失败: ${error.message}`)
    }
  }
}
