...
export default class HookManager {
  ...
  async execute(hookPoint, ctx) {
    ...
    for (const hook of chain) {
      ...
      try {
        let result;
        const traceRequestId = ctx?.event?.requestId;
        const traceHook = traceRequestId && (
          hookPoint === HOOK_POINTS.LLM_BEFORE_CHAT ||
          hookPoint === HOOK_POINTS.LLM_BEFORE_RECURSION
        );
        const hookStart = traceHook ? performance.now() : 0;
        if (traceHook) {
          logger.info(`[LLMTrace] requestId=${traceRequestId} stage=hook.start point=${hookPoint} hook=${hook.name}`);
        }
        ...
        if (traceHook) {
          logger.info(`[LLMTrace] requestId=${traceRequestId} stage=hook.done point=${hookPoint} hook=${hook.name} durationMs=${(performance.now() - hookStart).toFixed(1)}`);
        }
        ...
        if (ctx?.event?.requestId && (
          ...
          logger.warn(`[LLMTrace] requestId=${ctx.event.requestId} stage=hook.failed point=${hookPoint} hook=${hook.name}`);
        }
    ...
    for (const point in this.hooks) {
      ...
      for (const hook of targets) {
        try {
          ...
          logger.error(`[HookManager] Error destroying hook "${hook.name}":`, error);
        }
      }
    }
  }
