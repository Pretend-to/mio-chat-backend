...
class Config {
  constructor() {
    ...
    this.llm_presets_dir = path.resolve(__dirname, '../config/presets/')
    this.onebot = {}
    this.presets = {}
  }
  ...
  loadLLMPresets() {
    ...
    const files = fs.readdirSync(this.llm_presets_dir)
    const presets = []
    const recommended = []
    const hiddenPresets = []

    files.forEach((file) => {
      const filePath = path.resolve(this.llm_presets_dir, file)
      try {
  ...
  /**
   * 获取所有的LLM预设配置。
   * 该方法会返回包含通用预设、推荐预设和隐藏预设的对象。
   * @returns {Object} 包含所有预设配置的对象，结构为 { common: [], recommended: [], hidden: [] }
   */
  getAllPresets() {
    // 返回所有预设配置
    return this.presets
  ...
  getDefaultModel() {
    const { openaiAvalible, geminiAvalible } = this.getLLMEnabled()
    if (geminiAvalible) {
      return this.gemini.default_model 
    } else if (openaiAvalible) {
      return this.openai.default_model 
    }
  }
