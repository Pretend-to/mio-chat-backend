...
class Config {
  ...
  _initializePaths() {
    ...
    this.vertex_auth_path = path.resolve(__dirname, '../config/config/vertex.json')
  }
  ...
  _loadAdditionalConfigs() {
    ...
    if (this.vertex.enable) {
      try {
        const vertexAuthConfig = this.loadConfig('vertex')
        this.vertex.authConfig = vertexAuthConfig
        this.vertex.geminiConfig = this.gemini
      } catch (error) {
        logger.warn(`vertex 密钥加载失败，请参考 https://github.com/MartialBE/one-hub/wiki/VertexAI 下载密钥文件后置于 ${this.vertex_auth_path}`)
      }
    }
    // Load Vertex auth configuration
  ...
  getLLMEnabled() {
    ...
    const getAdapters = () => {
      const allAdapters = ['openai', 'gemini', "vertex"]
      ...
      for (const adapter of allAdapters) {
        const elm = {
          ...
          setAvaliable: () => {
            this.avaliableList.push(adapter)
          },

          isEnabled: () => this[adapter].enable,
        }

        elm.isEnabled() && list.push(elm) 
      }
      return list
    }
  ...
  getDefaultModel() {
    ...
    if (this.vertex.enable) {
      defaultModel.vertex = this.vertex.default_model 
    }
