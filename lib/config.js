...
class Config {
   ...
   * @private
   * @deprecated Most file paths are no longer needed since migration to database
   */
  _initializePaths() {
    // 保留用于向后兼容，但大部分路径已不再使用
    // 所有配置现在都存储在 SQLite 数据库中
  }
  ...
  _loadAdditionalConfigs() {
    ...
    this.loadPluginConfigFromDatabase().catch(error => {
    ...
    this.loadOwnersFromDatabase().catch(error => {
    ...
    // Load LLM presets (async)
    this.loadLLMPresets().catch(error => {
   ...
   * @deprecated This method is deprecated, use database instead
   */
  loadPluginConfig(_filePath) {
    logger.warn('loadPluginConfig is deprecated, use database instead')
    return null
  }
  ...
  async loadLLMPresets() {
    ...
    try {
      ...
      // 如果数据库加载失败，回退到文件系统加载
      this._loadPresetsFromFileSystem()
    }
  }

  /**
   * Fallback method to load presets from file system
   * @private
   * @deprecated Presets are now loaded from database only
   */
  _loadPresetsFromFileSystem() {
    logger.warn('预设文件系统加载已废弃，所有预设现在从数据库加载')
