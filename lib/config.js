import { getAdapterAliasesMap, getAdapterTypesSync, getAvailableAdapterTypes } from './chat/llm/adapters/registry.js'
...
class Config {
  ...
  async loadAndMergeConfig() {
    logger.debug('配置系统初始化成功! (使用数据库存储)')
    ...
    try {
      ...
      for (const atype of availableAdapterTypes) {
        if (! Object.hasOwn(this.llm_adapters, atype)) {
          this.llm_adapters[atype] = []
        }
      }
    } catch (error) {
      logger.warn('在补齐 llm_adapters 字段时发生错误:', error.message)
    }
    ...
    logger.debug('系统配置与 LLM 预设、插件配置加载成功')
  }
   ...
   */
  _setupDebugMode() {
    global.debug = Boolean(this.debug)
    if (logger && typeof logger.setLevel === 'function') {
      logger.setLevel(global.debug ? 'DEBUG' : (process.env.LOG_LEVEL || 'INFO'))
    }
  }
  ...
  async loadPluginConfigFromDatabase() {
    try {
      ...
      // OneBot 配置现在从 SystemSettingsService 加载，这里只处理其他插件配置
      // 设置默认的 OneBot 配置结构（实际值会在 _loadSystemConfigFromDatabase 中加载）
      if (!this.onebot) {
        this.onebot = {
          admin_qq: '',
          bot_qq: '',
      ...
      logger.debug('插件配置加载完成（OneBot 配置从系统设置加载）')
    } catch (error) {
      logger.error('从数据库加载插件配置失败:', error)
      throw error
    }
  ...
  async _checkAndMigrateOneBotConfig() {
    try {
      ...
      // 执行无感迁移
      logger.info('🔄 检测到 OneBot 配置需要迁移，正在自动迁移...')
      
      const {configData} = oldConfig
      await SystemSettingsService.set('onebot', configData, 'onebot', 'OneBot 协议配置')
      
      logger.info('✅ OneBot 配置自动迁移完成')
      
      // 注意：不删除旧配置，保持安全
      
    } catch (error) {
      logger.warn('OneBot 配置自动迁移失败，将使用默认配置:', error.message)
      // 不抛出错误，让应用继续启动
    }
  }

  /**
   * 运行通用的适配器迁移逻辑 (基于 aliases)
   * @private
   */
  async _runAdapterMigrations() {
    try {
      if (!this.llm_adapters) {return}

      const aliasMap = await getAdapterAliasesMap()
      let hasChanges = false

      for (const [alias, mainType] of Object.entries(aliasMap)) {
        if (this.llm_adapters[alias] && Array.isArray(this.llm_adapters[alias])) {
          logger.info(`🔄 检测到旧适配器类型 "${alias}"，正在自动迁移到 "${mainType}"...`)
          
          const oldInstances = this.llm_adapters[alias]
          if (!this.llm_adapters[mainType]) {
            this.llm_adapters[mainType] = []
          }

          // 合并实例
          this.llm_adapters[mainType] = [...this.llm_adapters[mainType], ...oldInstances]
          
          // 删除旧键
          delete this.llm_adapters[alias]
          hasChanges = true
          
          logger.info(`✅ "${alias}" -> "${mainType}" 自动迁移完成`)
        }
      }
      ...
      if (hasChanges) {
        // 持久化到数据库
        const { default: SystemSettingsService } = await import('./database/services/SystemSettingsService.js')
        await SystemSettingsService.set('llm_adapters', this.llm_adapters, 'llm_adapters', 'LLM 适配器配置 (自动迁移)')
      }
    } catch (error) {
      logger.warn('适配器自动迁移过程中发生错误:', error.message)
    }
  }
  ...
  async _loadSystemConfigFromDatabase() {
    try {
      ...
      if (process.env.PORT) {
        ...
        logger.info(`使用环境变量 PORT=${process.env.PORT} 覆盖数据库端口配置`)
      }
      
      if (process.env.HOST) {
        this.server.host = process.env.HOST
        logger.info(`使用环境变量 HOST=${process.env.HOST} 覆盖默认主机配置`)
      }
      ...
      if (systemLlmChannel) {
        this.system.system_llm_channel = systemLlmChannel.value
      }
      if (systemLlmModel) {
        this.system.system_llm_model = systemLlmModel.value
      }
      if (systemLlmTitlePrompt) {
        this.system.system_llm_title_prompt = systemLlmTitlePrompt.value
      }
      ...
      if (llmAdapters && llmAdapters.value) {
        ...
        // 执行通用的无感迁移
        await this._runAdapterMigrations()
        logger.debug('从数据库加载 LLM 适配器配置成功')
      }
      
            // 加载 Webhook 配置
      const webhookConfig = await SystemSettingsService.get('webhook')
      if (webhookConfig && webhookConfig.value) {
        this.webhook = webhookConfig.value
        logger.debug('从数据库加载 Webhook 配置成功')
      } else {
        this.webhook = {
        ...
        logger.debug('使用默认 Webhook 配置')
      }
      
            // 加载 Webhook 配置
      if (onebotConfig && onebotConfig.value) {
        Object.assign(this.onebot, onebotConfig.value)
        logger.debug('从数据库加载 OneBot 配置成功')
      } else {
        logger.debug('使用默认 OneBot 配置')
      }
      
      logger.debug('从数据库加载系统配置成功')
    } catch (error) {
  ...
  async loadOwnersFromDatabase() {
    try {
      ...
      if (ownersSetting && ownersSetting.value) {
        ...
        logger.debug(`从数据库加载了 ${this.owners.length} 个模型所有者配置`)
      } else {
        this.owners = []
  ...
  async loadLLMPresets() {
    ...
    try {
      ...
      logger.debug(`从数据库加载了 ${totalCount} 个 LLM 预设`)
      
      this.presets = presetCategories
    } catch (error) {
      logger.error('从数据库加载预设失败:', error)
      // 如果数据库加载失败，使用空预设
      this.presets = { common: [], hidden: [], recommended: [] }
    }
  }
  ...
  async getDefaultModel() {
    ...
    for (const instance of instances) {
      const { displayName, config: instanceConfig } = instance
      
      if (instanceConfig && instanceConfig.default_model) {
        defaultModel[displayName] = instanceConfig.default_model
      }
    }
  ...
  async reload() {
    ...
    // 重新加载和合并配置（这会自动处理 Vertex 等配置，loadAndMergeConfig 已在内部调用了 _loadAdditionalConfigs）
    await this.loadAndMergeConfig()
    
    logger.debug('配置文件重新加载完成')
  }

  /**
   * Clear list of available provider instances.
   */
  clearAvailableInstances() {
    this.availableInstances = []
  }
  ...
  async reloadPresets() {
    try {
      logger.debug('重新加载所有预设...')
      await this.loadLLMPresets()
      logger.debug('预设重新加载完成')
