...
class Config {
  ...
  async _runAdapterMigrations() {
    try {
      ...
      // 确保每个适配器实例都具备全局唯一的稳定 ID (adp_xxx)
      const usedIds = new Set()
      for (const instances of Object.values(this.llm_adapters)) {
        if (Array.isArray(instances)) {
          for (const inst of instances) {
            if (!inst.id || usedIds.has(inst.id)) {
              inst.id = `adp_${crypto.randomUUID()}`
              hasChanges = true
            }
            usedIds.add(inst.id)
          }
      ...
      if (hasChanges) {
        // 持久化到数据库
        const { default: SystemSettingsService } = await import('./database/services/SystemSettingsService.js')
        await SystemSettingsService.set('llm_adapters', this.llm_adapters, 'llm_adapters', 'LLM 适配器配置 (自动迁移与补齐 ID)')
      }
    } catch (error) {
  ...
  async getLLMEnabled() {
    ...
    for (const adapterType of allAdapterTypes) {
      ...
      for (const instanceConfig of instancesList) {
        ...
        // 如果没有指定 name，自动生成
        const displayName =
          instanceConfig.name || `${adapterType}-${instanceConfig.id.slice(-6)}`
        const instanceId = instanceConfig.id
