...
export default class Plugin {
  constructor(info, settings = {}) {
    ...
    this.reloadCallback = null // 初始化 reloadCallback 为 null
    ...
    try {
      ...
      for (const toolFile of toolFiles) {
        ...
        try {
          ...
          logger.error(`[工具加载失败] ${toolPath}:`, error)
        }
      }
    } catch (error) {
  ...
  _setupWatchers() { // 私有方法，监听插件变化
    ...
    const debouncedLoadTools = this.debounce(this.loadTools.bind(this), 500) // Debounce the loadTools function
    ...
    this.toolWatcher
      .on('add', (filePath) => {
        ...
        debouncedLoadTools() // Use the debounced function
      })
      .on('change', (filePath) => {
