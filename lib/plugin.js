...
export default class Plugin extends EventEmitter {
  constructor(info, settings = {}) {
    ...
    // If importMetaUrl is provided, auto-derive the plugin path
    // This eliminates the need to implement getFilePath() in subclasses
    if (info?.importMetaUrl) {
      this._autoPluginPath = path.dirname(fileURLToPath(info.importMetaUrl))
    } else {
      // Legacy: require subclass to implement getFilePath()
  ...
  async initialize() {
    ...
    await this.loadTools({ silent: true }) // 首次加载工具
    await this.loadHooks() // [新增] 自动加载插件专属 hooks
    this._setupWatchers() // 启动工具文件监听
    logger.debug(`[${this.name}] 初始化完成，共加载了 ${this.tools.size} 个工具`)
  }
   ...
   */
  async seedPresets() {
    if (!fs.existsSync(this.presetsPath)) return { count: 0 }
    try {
      ...
      if (presets.length > 0) {
        const { default: PresetService } = await import(
          './database/services/PresetService.js'
        )
        // 假设 PresetService 有 upsertMany 或 createMany 方法
        const result = await PresetService.createMany(presets)
        logger.info(`[${this.name}] 已同步 ${presets.length} 个插件预设`)
        return result
      }
    } catch (error) {
      logger.error(`[${this.name}] 同步预设失败:`, error)
    }
    return { count: 0 }
  }
  ...
  async loadTools(options = { silent: false }) {
    ...
    if (!options.silent) {
      logger.debug(`[${this.name}] 工具加载完成，发射 reloaded 事件`)
      this.emit('reloaded')
    } else {
      logger.debug(`[${this.name}] 工具加载完成 (静默模式)`)
    }
  }
  ...
  async loadConfig() {
    ...
    try {
      ...
      if (dbConfig && dbConfig.configData) {
        ...
        logger.debug(`[${this.name}] 创建默认配置到数据库`)
      }
    } catch (error) {
  ...
  getTools() {
    const map = new Map()
    // Convert Iterator to Array immediately to prevent exhaustion
