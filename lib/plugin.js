...
export default class Plugin extends EventEmitter {
  constructor(info, settings = {}) {
    ...
    this.skillsPath = settings.skillsPath || path.join(this.pluginPath, SKILLS_DIR)
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (typeof HookClass === 'function') {
          const hookInstance = new HookClass({ namespace: this.name, plugin: this })
  ...
  async loadLocalPresets() {
    ...
    try {
      ...
      this.localPresets = files.map((f) => {
        try {
          const content = fs.readFileSync(path.join(this.presetsPath, f), 'utf8')
          return {
            ...JSON.parse(content),
            provider: this.name,
            source: 'plugin',
      ...
      if (this.localPresets.length > 0) {
        ...
        logger.debug(`[${this.name}] 已加载并同步 ${this.localPresets.length} 个静态预设`)
      }
    } catch (error) {
