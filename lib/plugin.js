    ...
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        try {
          ...
          logger.error(
            `[${this.name}] [配置文件解析失败] ${this.configPath}. 使用默认配置.`,
            parseError,
          )
        ...
        logger.info(
          `[${this.name}] [配置文件未找到] ${this.configPath}. 创建默认配置文件.`,
        )
        // configData already holds the default config here
        try {
          ...
          logger.error(
            `[${this.name}] [无法写入默认配置文件] ${this.configPath}:`,
            writeError,
          )
      ...
      logger.error(
        `[${this.name}] [配置文件加载/检查错误] ${this.configPath}. 使用默认配置.`,
        error,
      )
      // configData already holds the default config here
    }
    ...
    if (!this.configWatcher) {
      ...
      this.configWatcher
        .on('add', (filePath) => {
          ...
          logger.info(
            `[${this.name}] [配置文件已创建/添加] ${path.basename(filePath)}. Reloading config.`,
          )
          this.debouncedLoadConfig() // Reload the newly created config
        })
        .on('change', (filePath) => {
          logger.info(
            `[${this.name}] [配置文件已更改] ${path.basename(filePath)}. Reloading config.`,
          )
        ...
        .on('unlink', (filePath) => {
          ...
          logger.warn(
            `[${this.name}] [配置文件已删除] ${path.basename(filePath)}. Reloading default config.`,
          )
        ...
        .on('error', (error) =>
          logger.error(
            `[${this.name}] [配置文件监听错误] ${this.configPath}:`,
            error,
          ),
        )
