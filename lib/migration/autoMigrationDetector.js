...
class AutoMigrationDetector {
  ...
  async needsMigration() {
    try {
      ...
      logger.debug(`配置文件检测: ${hasOldConfig ? '发现' : '未发现'}老版本配置`)
      logger.debug(`数据库状态: ${isDatabaseEmpty ? '空' : '已有数据'}`)
  ...
  async performAutoMigration() {
    try {
      ...
      return { stats, success: true, validation }
    } catch (error) {
  ...
  async checkAndMigrate() {
    try {
      ...
      return { noMigrationNeeded: true, success: true }
