...
class TaskScheduler {
  ...
  async _executeTask(task) {
    const { id, preset, triggerPrompt } = task
    try {
...
STRICT RULE: Do NOT ask for permission or wait for user input. Your goal is to use your available tools to complete the requested task fully and independently in this single session. Once finished, output your final result or report immediately.`
      ...
      if (!channelEntry) {
        logger.warn(
          `[TaskScheduler] 任务 "${id}" 未找到正在运行的匹配渠道实例 (agentId=${preset})，跳过执行`,
        )
        await TaskService.updateLastRun(id)
        return
      }

      // 纯粹的 Channel 路径：继承宿主渠道的全部配置、模型与上下文
      const agentId =
        channelEntry.channel?.agentId ||
        channelEntry.chn?.memory?.agentId ||
        preset
      logger.info(
        `[TaskScheduler] 任务 "${id}" 绑定至 Channel (agentId=${agentId})，继承当前会话 Provider/Model (${channelEntry.chn?.provider || '默认'}/${channelEntry.chn?.model || '默认'})`,
      )
      ...
      // 经过 Channel 单飞锁与排队队列触发任务执行，完全沿用当前渠道的 Provider 与 Model
      await chn.appendUserMessage(sid, `[定时任务触发]\n${finalPrompt}`, {
        channelId: agentId,
  ...
  _findChannelByPreset(preset) {
    if (!this.channelRuntime) return null
    if (preset) {
      for (const [, entry] of this.channelRuntime.running) {
        const agentId =
          entry.channel?.agentId ||
          entry.chn?.memory?.agentId ||
          entry.channel?.id
        if (agentId === preset) return entry
      }
    }
    // 兜底：若只有一个运行中的渠道，直接复用该渠道
    if (this.channelRuntime.running?.size === 1) {
      return this.channelRuntime.running.values().next().value
    }
    return null
  }
