...
class TaskScheduler {
  ...
  async initialize(llmService, channelRuntime = null) {
    ...
    // SessionTurnService consumes the backend bridge contract (`process`).
    // The bootstrap input is the raw LLMChatService (`handleMessage`), so the
    // composition root performs the adaptation once and injects the bridge.
    this.sessionTurnService = new SessionTurnService({
      channelRuntime,
      llm: createBackendLlm({ llmService: llmService }),
    })
...
STRICT RULE: Do NOT ask for permission or wait for user input. Your goal is to use your available tools to complete the requested task fully and independently in this single session. Once finished, output your final result or report immediately.`
      ...
      execution = await TaskExecutionService.create({
        agentId, deliveryBindingId, inputMessages: [{ role: 'user', content: finalPrompt }],
        round, sessionId, taskId: id, triggerPrompt,
      })
      if (!this.sessionTurnService) {
        const { SessionTurnService } = await import('./chat/sessions/SessionTurnService.js')
        this.sessionTurnService = new SessionTurnService({ channelRuntime: this.channelRuntime })
      }
      const result = await this.sessionTurnService.runTurn({
        agentId,
        deliveryBindingId,
        isTask: true,
        isWeb: false,
      ...
      await TaskExecutionService.complete(execution.id, {
        ...
        finalAssistantMsg: result.reply,
      })
      ...
      // 异步发送 Web Push 离线通知
      try {
        const { default: pushService } = await import('./push/PushService.js')
        await pushService.sendNotification({
          body: `任务已执行完成，点击查看执行轨迹与回复`,
          contactorId: agentId,
          title: `定时任务「${task.name || id}」`,
        })
      } catch {}
    } catch (error) {
      if (execution) await TaskExecutionService.fail(execution.id, { errorMessage: error.message }).catch(() => {})
  ...
  async stopTasksByAgent(agentId) {
    ...
    await this.sessionTurnService?.disposeAgent(agentId)
  }
