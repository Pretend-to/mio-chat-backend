...
const DEFAULT_MODE = 'database'

export class LegacyMigrationBootstrapError extends Error {
  constructor(code, message) {
    super(message)
    this.code = code
    this.name = 'LegacyMigrationBootstrapError'
  }
}
...
async function ensureEncryptionKey({ allowGenerate = true, keyPath }) {
  ...
  if (!selected && !allowGenerate) {
    throw new LegacyMigrationBootstrapError(
      'CHANNEL_STORAGE_KEY_MISSING',
      `Channel storage migration is already completed, but no instance key exists at ${keyPath} and MIOCHAT_ENC_KEY is unset. Restore the original key before restarting; generating a new key would make stored Channel credentials unreadable.`,
    )
  }
...
async function inspectPersistenceData(prisma) {
  const models = [
    ['agents', prisma.agent],
    ['agentMeta', prisma.agentMeta],
    ['channels', prisma.channel],
    ['sessions', prisma.session],
    ['messages', prisma.message],
    ['archives', prisma.sessionArchive],
    ['crystals', prisma.crystal],
    ['pendingMemories', prisma.pendingMemory],
    ['globalMemories', prisma.globalMemory],
    ['triggers', prisma.trigger],
    ['triggerExecutions', prisma.triggerExecution],
  ]
  const counts = Object.fromEntries(await Promise.all(models.map(async ([name, model]) => [
    name,
    await model.count(),
  ])))
  return {
...
async function writeCompletedState(prisma, state) {
...
export async function runLegacyMigrationBootstrap({
  ...
  const persistenceData = await inspectPersistenceData(prisma)
  if (inventory.files.length === 0 && inventory.agentDirs.length === 0) {
    await ensureEncryptionKey({
      allowGenerate: persistenceData.total === 0,
      keyPath: path.join(rootDir, 'prisma/data/channel-storage.key'),
    })
    const mode = explicitMode || DEFAULT_MODE
    if (!explicitMode) process.env.MIO_CHANNEL_PERSISTENCE_MODE = mode
    const completedState = {
      completedAt: new Date().toISOString(),
      manifestHash: inventory.manifestHash,
      mode: DEFAULT_MODE,
      source: persistenceData.total > 0 ? 'existing-database' : 'fresh-instance',
      version: CHANNEL_STORAGE_MIGRATION_VERSION,
    }
    await writeCompletedState(prisma, completedState)
    return { inventory, migrated: false, mode, reason: 'no-legacy-data', state: completedState }
  }
  ...
  if (persistenceData.total > 0) {
    const occupied = Object.entries(persistenceData.counts)
      .filter(([, count]) => count > 0)
      .map(([name, count]) => `${name}=${count}`)
      .join(', ')
    throw new LegacyMigrationBootstrapError(
      'CHANNEL_STORAGE_DATABASE_ALREADY_POPULATED',
      `Automatic legacy import refused because ${CHANNEL_STORAGE_MIGRATION_SETTING} is missing while persistence tables already contain data (${occupied}). This is a transitional or previously imported instance. Verify the database against the legacy JSON and write the completion marker deliberately; automatic re-import could collide with or overwrite database-native records.`,
    )
  }
