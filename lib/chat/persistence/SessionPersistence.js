...
export class SessionPersistence {
  ...
  async getSessionParentId(id) {
    if (typeof this.primary.getSessionParentId === 'function') {
      return await this.primary.getSessionParentId(id)
    }
    return (await this.primary.getSession(id))?.parentSessionId || null
  }
  ...
  async appendToChat(id, message) {
    ...
    return await this.#invoke('appendToChat', [id, canonical])
  }
  async appendUserMessage(id, message) {
    ...
    if (this.mode === 'database') return await this.database.appendUserMessage(id, canonical)
    if (this.mode === 'database-shadow') {
      const result = await this.database.appendUserMessage(id, canonical)
      try {
        await this.legacy.appendToChat(id, canonical)
      } catch (error) {
    ...
    const result = await this.legacy.appendToChat(id, canonical)
    if (this.mode === 'shadow') {
      try {
        await this.database.appendUserMessage(id, canonical)
      } catch (error) {
  ...
  async finalizeAssistantMessage(messageId, message, status = 'final') {
    ...
    if (writeLegacy) {
      ...
      try {
        await this.legacy.appendToChat(draft.sessionId, {
          ...message,
          id: messageId,
          role: 'assistant',
        })
