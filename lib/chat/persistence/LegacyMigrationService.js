  ...
  async verify({ encryptionKey = this.encryptionKey, inventory = null } = {}) {
    ...
    if (triggersFile) {
      for (const source of validateArrayFile(triggersFile)) {
        ...
        if (!row || row.legacyJson !== json(omit(source, ['webhookSecret']))) {
          problems.push(`${triggersFile.relativePath}: Trigger ${source.id} mismatch`)
        } else if (source.webhookSecret && row.webhookSecretHash !== crypto.createHash('sha256').update(String(source.webhookSecret)).digest('hex')) {
          problems.push(`${triggersFile.relativePath}: Trigger ${source.id} webhook secret mismatch`)
        }
      }
    }
  ...
  async #migrateTriggers(tx, inventory) {
    ...
    for (const source of triggers) {
      ...
      const data = {
        ...
        legacyJson: json(omit(source, ['webhookSecret'])),
