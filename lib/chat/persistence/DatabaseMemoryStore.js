...
function rowToMessage(row) {
  const legacy = parseJson(row.legacyJson)
  const hasLegacy = legacy && typeof legacy === 'object'
  const message = hasLegacy
    ? legacy
    : { content: parseJson(row.content, null), role: row.role }
  if (!hasLegacy) {
    if (row.fromUserId != null) message.from_user_id = row.fromUserId
    if (row.text != null) message.text = row.text
    if (row.businessTime != null) message.time = row.businessTime.getTime()
  }
  if (row.status !== 'final') {
...
/** Database-backed compatibility implementation of channels/memory/MemoryStore. */
    ...
    await this.prisma.$transaction(async tx => {
      ...
      await tx.message.update({
        data: {
          ...
          content: json(content),
          legacyJson: json({ ...message, role: 'assistant' }),
