...
export class SessionWorkCoordinator {
  ...
  async acquireLease({
    ...
    const existing = await prisma.sessionWorkLease.findUnique({
    ...
    if (!existing) {
      try {
        ...
        // Two writers can race to create a brand-new Session lease. Only that
        // race needs the unique-conflict fallback; existing leases are reused.
        if (!isUniqueConflict(error)) throw error
      }
    }
