...
export class SessionTurnService {
  ...
  async runTurn(options = {}) {
    const { sessionLeaseWaitMs = 0, ...turn } = options
    const { agentId, sessionId, sessionLease = null } = turn
    if (sessionLease || !this.sessionWorkCoordinator) {
      return await this._runTurnUnlocked(turn)
    }
    return await this.sessionWorkCoordinator.withSessionLease(
      { agentId, sessionId, waitMs: sessionLeaseWaitMs },
      async ({ sessionLease: acquiredLease }) =>
        await this._runTurnUnlocked({ ...turn, sessionLease: acquiredLease }),
    )
  }
