...
function resolveLlmExecutor(llm) {
  if (!llm) return createBackendLlm()
  if (typeof llm.process === 'function') return llm
  if (typeof llm.handleMessage === 'function') {
    return createBackendLlm({ llmService: llm })
  }
  throw new TypeError(
    '[SessionTurnService] llm must expose process() or handleMessage()',
  )
}
...
export class SessionTurnService {
  constructor({ prisma = null, llm = null, channelRuntime = null, persistenceFactory = createSessionPersistence } = {}) {
    this.prisma = prisma
    // SessionTurnService executes through BaseChannel, which requires the
    // backend bridge contract (`process`). App bootstrap historically passed
    // the raw LLMChatService (`handleMessage`) here, so normalize both forms
    // at the boundary instead of failing later inside a scheduled turn.
    this.llm = resolveLlmExecutor(llm)
