...
import { ChatEventFactory } from '../llm/events/ChatEventFactory.js'
...
export class VisionService {
  ...
  async saveConfig(newConfig = {}) {
    try {
      ...
      throw new Error(`保存识图配置失败: ${err.message}`, { cause: err })
    }
  }
  ...
  async analyze(options = {}) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const e = ChatEventFactory.createInternal({
        model: targetModel,
        messages: [
          {
            role: 'user',
  ...
  async _prepareImageData(image) {
    ...
    if (image.startsWith('http://') || image.startsWith('https://')) {
      ...
      if (image.startsWith('file://')) {
        try {
          absolutePath = new URL(image).pathname
        } catch {
          absolutePath = image.replace(/^file:\/\//, '')
        }
      } else if (image.startsWith('/f/up/')) {
        const parts = image.split('/')
        const type = parts[3]
        const name = parts[4]
        absolutePath = path.join(process.cwd(), 'output', 'uploaded', type, name)
      } else if (image.startsWith('/f/gen/')) {
        const parts = image.split('/')
        const name = parts[4]
        absolutePath = path.join(process.cwd(), 'output', 'generated', 'file', name)
      } else {
        absolutePath = path.isAbsolute(image) ? image : path.join(process.cwd(), image)
      }
