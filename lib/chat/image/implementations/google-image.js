...
export default class GoogleImageAdapter extends BaseImageAdapter {
  ...
  getImageInputLimit() {
    ...
    if (model.startsWith('gemini-2.5-flash-image')) return 3
    if (model.startsWith('gemini-3-pro-image')) return 14
    return 10
  }
  ...
  async _getAuthHeaders() {
    if (this.platform === 'vertex' && !this.expressMode) {
      try {
        ...
        throw new Error(`[GoogleImageAdapter] Vertex AI ADC 认证失败: ${error.message}`, { cause: error })
      }
    }
    return {}
  }
  ...
  async generate(options = {}) {
    const { prompt, size = '1024x1024', n = 1, aspectRatio = '1:1' } = options
    const images = this._getImageSources(options)
    ...
    if (platform === 'vertex') {
      return this._generateVertex({ prompt, images, model, apiKey, n, targetAspectRatio })
    } else {
      return this._generateAIStudio({ prompt, images, model, apiKey, n, targetAspectRatio })
    }
  }

  async _generateAIStudio({ prompt, images, model, apiKey, n, targetAspectRatio }) {
    if (!apiKey) {
    ...
    const imgInfos = await Promise.all(images.map(image => this._resolveImageBase64(image)))
  ...
  async _generateVertex({ prompt, images, model, apiKey, n, targetAspectRatio }) {
