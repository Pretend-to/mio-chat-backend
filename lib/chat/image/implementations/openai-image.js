...
export default class OpenAIImageAdapter extends BaseImageAdapter {
  ...
  async generate(options = {}) {
    ...
    const supportsEdit = imgInfo && (model.startsWith('gpt-image') || model.includes('dall-e-2'))

    // gpt-image 编辑端点的合法尺寸与 DALL-E 生成尺寸不同，需翻译
    ...
    if (supportsEdit) {
      ...
      const referenceFile = await toFile(
        Buffer.from(imgInfo.base64, 'base64'),
        'reference.' + (imgInfo.mimeType === 'image/jpeg' ? 'jpg' : 'png'),
        { type: imgInfo.mimeType || 'image/png' }
      )
      const editParams = {
        model,
        image: referenceFile,
      ...
      if (imgInfo) {
        logger.warn(`[OpenAIImageAdapter] 模型 ${model} 不支持参考图编辑，参考图将被忽略（降级为文生图）`)
      }
      try {
