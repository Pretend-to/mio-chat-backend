...
export default class OpenAIImageAdapter extends BaseImageAdapter {
  ...
  supportsImageInput() {
    ...
    return model.startsWith('gpt-image') || model.includes('dall-e-2')
  }

  getImageInputLimit() {
    const model = this.config.model || 'gpt-image-2'
    return model.includes('dall-e-2') ? 1 : (this.supportsImageInput() ? 16 : 0)
  }
  ...
  async generate(options = {}) {
    ...
    const toGptImageSize = (s) => {
      if (s === '1536x1024' || s === '1024x1536' || s === '1024x1024') return s
      if (s === '1792x1024') return '1536x1024'
      if (s === '1024x1792') return '1024x1536'
      return model.startsWith('gpt-image-2') ? s : 'auto'
    }

    if (model.startsWith('gpt-image')) targetSize = toGptImageSize(targetSize)
    ...
    if (model.includes('dall-e')) {
      requestParams.response_format = this.config.responseFormat || 'b64_json'
    }
    ...
    if (model.includes('dall-e-3')) {
      ...
      requestParams.quality = ['low', 'medium', 'high', 'auto'].includes(quality) ? quality : 'auto'
    }

        // —— img2img：参考图存在且模型支持编辑时，走官方 images.edit 通道 ——
    // 依据 OpenAI 官方文档：gpt-image 系列与 dall-e-2 支持 images/edits；dall-e-3 无编辑端点
    const supportsEdit = imgInfo && this.supportsImageInput()

    logger.info(`[OpenAIImageAdapter] 请求生图: model="${model}", size="${targetSize}", prompt="${prompt?.slice(0, 80)}...", baseURL="${baseURL}"${imgInfo ? ', 包含参考图' : ''}`)

    let response
    if (supportsEdit) {
      // 官方 SDK v6：toFile 将 Buffer 转为可上传文件对象（images/edits 为 multipart 上传）
      const extensionForMime = mime => ({
        'image/jpeg': 'jpg',
        'image/png': 'png',
        'image/webp': 'webp'
      })[mime] || 'png'
      const referenceFiles = await Promise.all(imgInfos.map((info, index) => toFile(
        Buffer.from(info.base64, 'base64'),
        `reference-${index + 1}.${extensionForMime(info.mimeType)}`,
      ...
      const editParams = {
        ...
        prompt,
        n: requestParams.n,
        size: model.startsWith('gpt-image') ? toGptImageSize(targetSize) : '1024x1024'
      }
      if (this.config.inputFidelity && model.startsWith('gpt-image') && !model.includes('mini')) {
        editParams.input_fidelity = this.config.inputFidelity
      }
      logger.info(`[OpenAIImageAdapter] 走 images.edit 图生图通道 (input_fidelity=${editParams.input_fidelity || 'default'})`)
      ...
      try {
        ...
        if (requestParams.response_format && err.message && (err.message.includes('response_format') || err.message.includes('b64_json'))) {
