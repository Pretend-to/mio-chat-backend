...
export class ImageService {
  ...
  async generate(options = {}, adapterIdentifier = null) {
    ...
    const inputImages = adapter._getImageSources(options)
    if (inputImages.length > 0 && !adapter.supportsImageInput(options)) {
      const model = adapter.config?.model ? `（模型 ${adapter.config.model}）` : ''
      throw new Error(`[ImageService] 生图通道「${adapterDisplayName}」${model}不支持参考图输入，请更换支持 img2img 的模型或通道。`)
    }

    const imageInputLimit = adapter.getImageInputLimit(options)
    if (inputImages.length > imageInputLimit) {
      throw new Error(`[ImageService] 生图通道「${adapterDisplayName}」最多支持 ${imageInputLimit} 张参考图，当前收到 ${inputImages.length} 张。`)
    }

    const normalizedOptions = inputImages.length > 0
      ? { ...options, image: inputImages[0], images: inputImages }
      : options

    const startTime = Date.now()
    const results = await adapter.generate(normalizedOptions)
