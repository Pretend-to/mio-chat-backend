...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  async _prepareChatBody(body) {
    ...
    const { temperature, thinking_effort } = chatParams

    const avaliableThinkingModels = [
      'gemini-2.5-flash',
    ]
    ...
    const ThinkingConfig = {
      ...
      thinkingBudget: budgetTable.get(thinking_effort),
    }

    const processedChatParams = {
      temperature,
    }
    if (avaliableThinkingModels.some((model) => base.model.includes(model))) {
