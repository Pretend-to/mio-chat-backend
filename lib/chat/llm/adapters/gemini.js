...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  static getAdapterMetadata() {
    return {
      ...
      extraSettingsSchema: {
        gemini: {
          imageGeneration: { type: 'boolean', default: false, label: '图像生成 (DALL-E style)' },
          internalTools: {
            type: 'group',
            label: '内置工具',
            fields: {
              google_search: { type: 'boolean', default: false, label: 'Google Search' },
              code_execution: { type: 'boolean', default: false, label: '代码执行 (Code Execution)' },
              url_context: { type: 'boolean', default: false, label: '网页解析 (URL Context)' },
            }
          },
          safetySettings: {
            type: 'group',
