...
export default class OpenAIBot extends BaseLLMAdapter {
  ...
  get gemini() {
    ...
    // 日志用的哪个
    logger.info(
      `使用Gemini API：${base_url} , key：${selectedKey.substring(0, 6)}...}`
    )
  ...
  async _executeChatRequest(body, e) {
    ...
    for await (const chunk of stream) {
      ...
      if (delta.parts[0].text) {
        e.update({
          type: 'content',
          content: delta.parts[0].text,
        })
        cachedMessage.content += delta.content
      } else if (delta.parts[0].functionCall) {
        // 暂时不考虑多函数同时调用
        callMessage = delta.parts[0]
  ...
  async models() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const data = await response.json()
        // 过滤不可用模型
        const avaliableModelNames = ['gemini-2.0', 'gemini-1.5']
      ...
      console.error('Error during models:', error)
