...
export default class OpenAIBot extends BaseLLMAdapter {
  ...
  async _getModels() {
    try {
      const models = await this.gemini.models()
        ...
        for (const elem of content.parts) {
          if ('text' in elem) {
            ...
            const toolCallData = {
              ...
              action: 'started',
              id: toolCallElem.id,
              params: toolCallElem.functionCall.args || '',
              result: '',
            }
            e.update({
              type: 'toolCall',
  ...
  async _handleToolCalls(toolCalls, e) {
    ...
    for (const toolCall of toolCalls) {
      ...
      tasks.push(middleware.llm.runTool(toolCallData, e.user))
    }

    const toolResults = await Promise.allSettled(tasks)

    for (const toolResult of toolResults) {
      const res = toolResult.value
      const { call, result } = res
...
class Gemini {
  ...
  *_parseMutiJson(buffer) {
    ...
    // 检查 cache 能否解析为完整的 JSON
    if (this.cache.trim() !== '') {
      try {
        const jsonData = JSON.parse(this.cache)
        yield jsonData
        this.cache = '' // 解析成功，清空 cache
      } catch (error) {
        // 解析失败，继续等待更多数据
      }
    }
