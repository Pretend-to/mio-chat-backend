...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  async _prepareChatBody(body) {
    ...
    const hasInternalTools = Object.values(internalTools).some(
      (value) => value
    )
    if (hasInternalTools) {
      if (!parsedTools) {
    ...
    const isReasoningAvaliable = avaliableReasoningModels.some((model) => base.model.includes(model.name))

    if(isReasoningAvaliable) {
      const rangeType = avaliableReasoningModels.find((model) => base.model.includes(model.name)).rangeType
...
export class Gemini {
  ...
  async *chat({
    ...
    const generationConfig = { temperature, response_modalities, thinkingConfig }
  ...
  *_parseMutiJson(buffer) {
    ...
    for (const line of lines) {
      ...
      // 尝试解析缓存中的内容
      try {
        const cacheData = this.cache.startsWith('data: ') ? JSON.parse(this.cache.substring(6).trim()) : JSON.parse(this.cache)
        yield cacheData
        this.cache = '' // 清空缓存
      } catch (e) {
