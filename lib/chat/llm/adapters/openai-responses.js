...
export default class OpenAIResponsesBot extends BaseLLMAdapter {
        ...
        openai: {
          ...
          settings: {
            reasoning_effort: {
              ...
              default: 'medium',
              description: '推理模型的思考强度 (仅针对 o1/gpt-5 等推理模型)',
              label: '推理强度',
            },
          },
        },
  ...
  async _prepareChatBody(body) {
    ...
    preparedBody.tool_choice = (mode === 'ANY' && parsedTools.length > 0) ? 'required' : undefined
    ...
    for (const message of messages) {
      ...
      if (role === 'assistant' && tool_calls && Array.isArray(tool_calls)) {
        for (const call of tool_calls) {
          processed.push({
            type: 'function_call',
            id: call._itemId || call.id, // 内部使用的 fc_... ID
            call_id: call.call_id || call.id, // 标准的 call_... ID
      ...
      if (enableStream) {
        ...
        for await (const chunk of stream) {
          ...
          // 3. 处理工具调用开始 (output_item.added)
            ...
            const existingCall = {
              id: item.call_id || item.id, // 标准 tool_call_id
              call_id: item.call_id,
              _itemId: item.id, // 用于匹配增量事件
              type: 'function',
          ...
          // 4. 处理工具调用参数增量 (response.function_call_arguments.delta)
            ...
            const existingCall = callMessage.tool_calls.find(c => c._itemId === chunk.item_id)
            if (existingCall) {
