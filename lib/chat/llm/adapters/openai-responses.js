...
export default class OpenAIResponsesBot extends BaseLLMAdapter {
  ...
  static getAdapterMetadata() {
    return {
      ...
      description:
        'OpenAI 统一 Responses API (v1/responses)，支持高级代理功能与长文本状态保持',
      supportedFeatures: [
        'chat',
        'streaming',
        'function_calling',
  ...
  async _prepareChatBody(body) {
    ...
    // 映射推理强度 (优先使用适配器专属设置)
    const effortValue =
      openaiSettings.reasoning_effort || chatParams.reasoning_effort
    if (effortValue !== undefined && effortValue !== -1) {
      preparedBody.reasoning = {
        effort:
          typeof effortValue === 'string'
            ? effortValue
            : reasoningEffortMap[effortValue] || 'none',
      }
    }
    ...
    try {
      if (enableStream) {
        ...
        for await (const chunk of stream) {
          ...
          // logger.info(
          //   `[${this.provider.toUpperCase()}-Responses] Chunk: ${JSON.stringify(chunk)}`,
          // )
