...
export default class BaseLLMAdapter {
  ...
  logUsage(providerName, usage, timeMetrics) {
    ...
    hookManager.execute(HOOK_POINTS.LLM_AFTER_CHAT, {
      providerName,
      usage,
      model: timeMetrics?.model || this.config?.model,
      user: timeMetrics?.e?.user,
      timeMetrics,
      adapter: this
  ...
  _normalizeToolCallIds(messages) {
    ...
    const copiedMessages = JSON.parse(JSON.stringify(messages))
    ...
    for (const message of copiedMessages) {
      if (message.role === 'assistant' && Array.isArray(message.tool_calls)) {
        for (const toolCall of message.tool_calls) {
          ...
          if (idStr.length > 64) {
            // 使用确定性的 MD5 哈希缩短，确保每次相同历史请求前缀缓存（Prompt Context Cache）能稳定命中
        ...
        if (toolCallIdMap.has(idStr)) {
          message.tool_call_id = toolCallIdMap.get(idStr)
        } else if (idStr.length > 64) {
          const md5 = crypto.createHash('md5').update(idStr).digest('hex')
          const shortId = 'mc_' + md5
          toolCallIdMap.set(idStr, shortId)
          message.tool_call_id = shortId
        }
      }
    }
    ...
    for (const call of toolCalls) {
      ...
      const runTask = async () => {
        try {
          ...
          let innerRender = toolCallData.innerRender || []
          let outerRender = toolCallData.outerRender || []
          let extraRender = null

          if (result && typeof result === 'object') {
            if (result.extraRender) {
              extraRender = result.extraRender
              if (Array.isArray(extraRender)) {
                extraRender.forEach(item => {
                  const processedItem = { placement: 'inner', ...item }
                  if (processedItem.placement === 'outer') {
                    outerRender.push(processedItem)
                  } else {
                    innerRender.push(processedItem)
                  }
                })
              }
            }
          ...
          toolCallData.innerRender = innerRender
          toolCallData.outerRender = outerRender
          toolCallData.extraRender = [...innerRender, ...outerRender]
          e.update({
            type: 'toolCall',
            content: toolCallData,
          })
