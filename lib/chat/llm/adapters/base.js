...
export default class BaseLLMAdapter {
  ...
  async handleChatRequest(e, firstCall = true) {
    try {
      ...
      logger.info(`[LLMTrace] requestId=${e.requestId} stage=body.prepare.start round=${e._recursionRound}`)
      const processedBody = await this._prepareChatBody(e.body, e)
      logger.info(
        `[LLMTrace] requestId=${e.requestId} stage=body.prepare.done ` +
          `round=${e._recursionRound} durationMs=${(performance.now() - prepareStartedAt).toFixed(1)}`,
      )
