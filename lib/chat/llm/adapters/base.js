...
function getAuditContext(e) {
  ...
  const explicitOrigin = e?.user?.origin
  const sourceType =
    explicitOrigin === 'web' || explicitOrigin === 'channel'
      ? explicitOrigin
      : requestId.startsWith('system_') ||
          requestId.startsWith('task-') ||
          requestId.startsWith('vision-')
        ? 'system'
        : 'web'
  ...
  return {
    ...
    channelId: e?.body?.channelId || e?.metaData?.channelId || null,
    contactorId: e?.body?.contactorId || null,
    sessionId: e?.sessionId || e?.body?.sessionId || null,
    sourceType,
  }
}
