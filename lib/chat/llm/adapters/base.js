...
export default class BaseLLMAdapter {
  ...
  async testConnection({ model } = {}) {
    ...
    const event = ChatEventFactory.createForInternal({
      chatParams: { max_tokens: 1, reasoning_effort: 0 },
  ...
  _upsertNativeTool(preparedBody, descriptor, body) {
    ...
    const providerKey = descriptor.provider || this.provider
    const providerSettings =
      extraSettings?.[providerKey] || extraSettings || {}
    const toolKey = descriptor.key || descriptor.type
    const toolConfig =
      providerSettings[toolKey] !== undefined
        ? providerSettings[toolKey]
        : extraSettings[toolKey]
