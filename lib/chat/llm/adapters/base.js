...
export default class BaseLLMAdapter {
  ...
  _shouldFilterVision(modelName, visionOverride = undefined) {
    // 0) 联系人/请求显式覆写最高优先（true: 强行放行; false: 强行过滤）
  ...
  logUsage(providerName, usage, timeMetrics) {
    ...
    if (event) {
      ...
      event._cumulativeUsage.completion_tokens += (normalizedUsage.completion_tokens || 0)
      event._cumulativeUsage.reasoning_tokens += (normalizedUsage.reasoning_tokens || 0)
      event._cumulativeUsage.total_tokens += (normalizedUsage.total_tokens || 0)
    }

    // 推送用量信息给客户端（纯前端存储与展示）
    if (event && typeof event.update === 'function' && usage) {
      const overallDurationMs = Date.now() - (event._totalStartTime || event.requestStartTime || finalTimeMetrics?.startTime || Date.now())
      const lastRoundDurationMs = finalTimeMetrics.elapsedTime || (finalTimeMetrics.startTime ? (Date.now() - finalTimeMetrics.startTime) : (Date.now() - (finalTimeMetrics.e.requestStartTime || Date.now())))
      const ttftMs = (finalTimeMetrics?.firstTokenTime && finalTimeMetrics?.startTime)
        ? Math.max(0, finalTimeMetrics.firstTokenTime - finalTimeMetrics.startTime)
        : null

      const cumCompletion = event._cumulativeUsage?.completion_tokens || normalizedUsage.completion_tokens
      const cumReasoning = event._cumulativeUsage?.reasoning_tokens || normalizedUsage.reasoning_tokens
  ...
  async _handleToolCalls(toolCalls, e, _stepId) {
    ...
    for (let ci = 0; ci < toolCalls.length; ci++) {
      ...
      if (!tr) {
        const abortedMsg = e.aborted ? 'User aborted.' : '[System Error] 工具执行失败或未返回结果'
        tr = {
          content: abortedMsg,
          name: toolName,
          tool_call_id: toolCallId,
