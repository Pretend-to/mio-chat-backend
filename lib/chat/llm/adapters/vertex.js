...
export default class VertexAdapter extends GeminiAdapter {
  ...
  async _getModels() {
    ...
    // 在尝试获取模型前，先验证登录状态(获取令牌)
    const { error: authError } = await this.core.getAuthentication()
    if (authError) {
      throw authError
    }
...
class Vertex extends Gemini {
  ...
  async models() {
    ...
    if (this.geminiAvaliable) {
      ...
      // 经典平替方案：如果系统没配置 Gemini，我们直接拿着 OAuth 令牌去请求 Gemini 端点！
      const fallbackUrl = 'https://generativelanguage.googleapis.com/v1beta/models'
      try {
        ...
        if (response.ok) {
          ...
          if (res.models) {
            ...
            return res.models
              ...
              .filter((model) => avaliableModelNames.some((name) => model.id.includes(name)))
              .filter((model) => !disabledModelNames.some((name) => model.id.includes(name)))
          }
        } else {
          console.warn('Vertex Bearer 跨端拉取模型列表失败，状态码:', response.status)
        }
      } catch (err) {
        console.warn('Vertex Bearer 跨端拉取模型列表网络错误:', err.message)
      }
      
      // 如果跨端也失败了，返回空列表，让外层自动触发 manualModels 和 defaultModels 兜底
      return []
    }
  }
