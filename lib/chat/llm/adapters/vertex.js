    ...
    async _getModels() {
      try {
        ...
        if (models.length === 0) {
          ...
          const claudeModels = this.defaultModels.filter((model) => {
...
export class Vertex extends Gemini{
    ...
    getEndPoint(region, projectId) {
      ...
      const base = "https://{region}-aiplatform.googleapis.com/v1/projects/{projectId}/locations/{region}"
      const url = base
    ...
    async models() {
        if (!this.geminiAvaliable) {
          ...
          return []
           ...
           return geminiModels.map(model => ({ id: model.id.replace('models/', '') }));
        }
    }
    ...
    async *chat({
        ...
        let method 
        if (stream) {
          method = provider === 'google' ? ':streamGenerateContent' : ':streamRawPredict' 
        } else {
        ...
        if (provider === 'google') {
          ...
          const { system, claudeMessages } = await this._preProcessMessage(messages, provider)
        }
    }
