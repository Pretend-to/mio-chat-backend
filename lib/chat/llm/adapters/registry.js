...
export function getAdapterTypesSync() {
  const adapterFiles = readdirSync(implementationsDir).filter(
...
export async function getAvailableAdapterTypes() {
  ...
  for (const file of adapterFiles) {
    try {
      const adapterModule = await import(`./implementations/${file}`)
      const AdapterClass = adapterModule.default

      if (
        AdapterClass &&
        typeof AdapterClass.getAdapterMetadata === 'function'
      ) {
...
export const getModelMetadata = (model) => modelRegistryService.getModelMetadata(model)
export const getWatermark = (model) => modelRegistryService.getWatermark(model)
export const supportsVision = (model) => modelRegistryService.supportsVision(model)
