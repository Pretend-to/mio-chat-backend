...
export default class DeepSeekAdapter extends OpenAIBot {
  ...
  static getAdapterMetadata() {
    return {
      ...
      description: 'DeepSeek 特殊适配器，基于 OpenAI 实现并额外缓存 reasoning_content 以便工具调用后一并返回。',
      authSchema: {
        base_url: {
          description: 'DeepSeek API 的基础 URL，例如 https://api.deepseek.com',
          default: 'https://api.deepseek.com/v1',
          required: true,
          type: 'string',
        },
        api_key: {
          description: '用于调用 DeepSeek 的 API Key，支持以逗号分隔的多个 key',
          default: '',
          required: true,
          type: 'string',
        },
  ...
  /**
   * 处理工具调用
   * DeepSeek: 继承父类即可，不需要特殊处理
   * @param {object} toolCalls
   * @param {object} e
   */
  async _handleToolCalls(toolCalls, e) {
    // DeepSeek API 标准，直接使用父类的实现
    return this._handleToolCalls(toolCalls, e)
  }
