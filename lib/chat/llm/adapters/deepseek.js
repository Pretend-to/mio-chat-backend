...
export default class DeepSeekAdapter extends OpenAIBot {
  ...
  static getAdapterMetadata() {
    return {
      type: 'deepseek',
      name: 'DeepSeek',
      description: 'DeepSeek 系列模型适配器，支持推理链和工具调用功能',
  ...
  async _executeChatRequest(body, e) {
    ...
    if (enableStream) {
      ...
      if (callMessage.tool_calls && callMessage.tool_calls.length > 0) {
        // DeepSeek: 如果有普通内容，将其 push 到消息列表（防止 toolCall 模式下丢失普通文本输出）
        ...
        // 推理内容由于需要和 tool_calls 关联，暂时存入 body 供 _handleToolCalls 使用
