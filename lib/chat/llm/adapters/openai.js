...
export default class OpenAIBot {
  ...
  constructor(openaiConfig) {
    ...
    this.openai = new OpenAI({ baseURL: base_url, apiKey: api_key })
    this.models = [] // 可用模型列表
  ...
  async _getModels() {
    try {
      ...
      const ownerList = this.config.owners // 假设 owners 是一个配置数组
...
// 创建 OpenAIBot 实例
// const { openai_base_url, openai_api_key } = config.openai
// const openai = new OpenAIBot(openai_base_url, openai_api_key)
