...
export default class OpenAIBot extends BaseLLMAdapter {
      ...
      if (models.includes(model)) {
        if (provider === 'openai') {
          ...
          if (chatParams.reasoning_effort !== undefined) {
            chatParams.reasoning_effort = chatParams.reasoning_effort >= 2 ? 'high' : 'low'
          }
        }
      } else {
  ...
  async _executeChatRequest(body, e) {
    ...
    if (enableStream) {
      ...
      const stream = await this.openai.chat.completions.create({ ...body, signal: controller.signal })
      ...
      const runTask = async () => {
        try {
          ...
          if (err.message === 'USER_ABORT') {
            logger.info(`Tool execution ${toolCall.name} aborted for request ${e.requestId}`)
            return
          }
          throw err
        }
