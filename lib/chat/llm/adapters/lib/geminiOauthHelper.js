...
export function encryptState(verifier, createdAt = Date.now()) {
  try {
    ...
    const payload = JSON.stringify({ v: verifier, t: createdAt })
    ...
    log.error('[Gemini OAuth] encryptState failed:', err)
    return null
  }
}
...
export function decryptState(stateStr) {
  try {
    ...
    log.debug(`[Gemini OAuth] decryptState failed (might be legacy state): ${err.message}`)
  }
  return null
}
