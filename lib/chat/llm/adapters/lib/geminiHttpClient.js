...
export class Gemini {
  ...
  async *chat(params, e = null) {
    ...
    log.debug?.(`[geminiHttpClient] 请求体: ${JSON.stringify(body)}`)
    try {
  ...
  async _preProcessMessage(messages) {
    ...
    for (const msg of messages) {
      if (Array.isArray(msg.tool_calls)) {
        for (const tc of msg.tool_calls) {
          ...
          if (callId && name) {
            toolCallNameMap.set(callId, name)
            if (callId.startsWith('gsig_')) {
              try {
                const parts = Buffer.from(callId.substring(5), 'hex').toString().split('|')
                if (parts[1]) toolCallNameMap.set(parts[1], name)
              } catch {}
            }
          }
        }
    ...
    for (const [index, message] of messages.entries()) {
      if (message.role === 'system' || message.role === 'developer') {
        ...
        const contentElement = { parts: [], role }

        // 如果模型消息包含 reasoning_content，作为 thought part 优先注入
        if (
          role === 'model' &&
          ...
          for (const element of message.content) {
            if (element.type === 'text') {
              ...
              if (typeof url === 'string' && url.startsWith('http')) {
                ...
                contentElement.parts.push({ inline_data: { data, mime_type } })
              }
            }
          }
        }
        ...
        if (message.tool_calls) {
          for (const toolCall of message.tool_calls) {
            ...
            const callElm = { functionCall: { args, name } }

            let _parsedThoughtSignature = toolCall.thoughtSignature || null
            ...
            if (toolCallId && typeof toolCallId === 'string') {
              if (toolCallId.startsWith('gsig_')) {
                ...
                _parsedThoughtSignature = _parsedThoughtSignature || parts[0] || null
                ...
                try {
                  ...
                  if (decoded === toolCallId) {
                    _parsedThoughtSignature = _parsedThoughtSignature || toolCallId
                  }
                } catch {
                  // Ignore
                }
            ...
            const supportsToolCallId = this.provider !== 'Gemini' || (this.base_url && this.base_url.includes('mock'))

            if (_parsedThoughtSignature) {
              callElm.thoughtSignature = _parsedThoughtSignature
            }
            if (parsedFunctionCallId && supportsToolCallId) {
        ...
        if (
          ...
          (messages[index + 1].role === 'tool' || messages[index + 1].role === 'function')
        ) {
          continue
        } else {
          const contentElement = { parts: [], role: 'user' }
          for (const toolCallResult of toolCallResultsCache) {
            const toolCallId = toolCallResult.tool_call_id
            let funcName = toolCallResult.name || toolCallNameMap.get(toolCallId) || ''
            if (!funcName && toolCallId && toolCallId.startsWith('gsig_')) {
              try {
                const parts = Buffer.from(toolCallId.substring(5), 'hex').toString().split('|')
                if (parts[1]) funcName = toolCallNameMap.get(parts[1]) || ''
              } catch {}
            }

            let responseData = toolCallResult.content
            let responseObj
            if (typeof responseData === 'string') {
              try {
                const parsed = JSON.parse(responseData)
                if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {
                  responseObj = { ...parsed, name: funcName }
                } else {
                  responseObj = { content: responseData, name: funcName }
                }
              } catch {
                responseObj = { content: responseData, name: funcName }
              }
            } else if (typeof responseData === 'object' && responseData !== null && !Array.isArray(responseData)) {
              responseObj = { ...responseData, name: funcName }
            } else {
              responseObj = { content: responseData, name: funcName }
            }
            ...
            let _parsedThoughtSignature = toolCallResult.thoughtSignature || null
            let parsedFunctionCallId = null
