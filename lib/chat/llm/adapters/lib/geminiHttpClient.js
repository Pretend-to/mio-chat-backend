      ...
      if (message.role === 'system' || message.role === 'developer') {
        ...
        if (message.tool_calls) {
          for (const toolCall of message.tool_calls) {
            ...
            if (thoughtSignature && typeof thoughtSignature === 'string' && thoughtSignature.length > 50) {
              try {
                const decoded = Buffer.from(thoughtSignature, 'base64').toString('base64')
                if (decoded === thoughtSignature) {
                  callElm.thoughtSignature = thoughtSignature
                }
              } catch {
                // 不是合法 base64，跳过
              }
        ...
        if (index + 1 < messages.length && messages[index + 1].role === 'tool') {
          ...
          for (const toolCallResult of toolCallResultsCache) {
            ...
            if (toolCallId && typeof toolCallId === 'string' && toolCallId.length > 50) {
              try {
                const decoded = Buffer.from(toolCallId, 'base64').toString('base64')
                if (decoded === toolCallId) {
                  responsePart.functionResponse.id = toolCallId
                }
              } catch {
                // 不是合法 base64，跳过
              }
