...
/** 允许出现在模型列表中的名称关键词 */
const AVAILABLE_MODEL_NAMES = ['gemini', 'gemma']
/** 应从模型列表中排除的名称关键词 */
const DISABLED_MODEL_NAMES = ['computer', 'embedding', 'audio']

/**
 * 过滤 Gemini 模型列表：保留包含可用关键词的、排除包含禁用关键词的
 * @param {Array<{id: string}>} models - 原始模型列表
 * @returns {Array<{id: string}>} 过滤后的模型列表
 */
export function filterGeminiModels(models) {
  return models
    .filter((model) =>
      AVAILABLE_MODEL_NAMES.some((name) => model.id.includes(name)),
    )
    .filter(
      (model) =>
        !DISABLED_MODEL_NAMES.some((name) => model.id.includes(name)),
    )
}
...
export class Gemini {
  ...
  *_parseMultiJson(buffer, cacheHolder) {
  ...
  async models() {
    ...
    const mapped = models.map((model) => ({ id: model.name.replace(/^models\//, '') }))
        ...
        if (Array.isArray(message.content)) {
          ...
          for (const toolCall of message.tool_calls) {
            ...
            if (typeof args === 'string') {
              try { args = JSON.parse(args) } catch { /* ok */ }
            }
            const callElm = { functionCall: { name, args } }
            // 通过 round-trip base64 编解码检测 thought signature 是否为合法 base64
            // 合法 base64 的 thought signature 表明该 id 由 Gemini 原生工具调用产生
            if (thoughtSignature && typeof thoughtSignature === 'string') {
              try {
                const decoded = Buffer.from(thoughtSignature, 'base64').toString('base64')
