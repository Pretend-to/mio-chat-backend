...
export class GeminiOauth extends Gemini {
  constructor(config) {
    ...
    this.userAgent = 'antigravity/1.23.2 windows/amd64'
  ...
  async _ensureInitialized() {
    ...
    if (isNewUrlOrCode) {
      ...
      // 【诊断】打印 _ensureInitialized 中 sessionStore 的状态
      const allEntries = [...sessionStore.entries()]
      log.info(`[DIAG] _ensureInitialized 开始查找 state=${state}，sessionStore 有 ${allEntries.length} 个条目`)
      log.info(`[DIAG] sessionStore keys=[${allEntries.map(([s]) => s.substring(0, 10)).join(', ')}]`)
      if (allEntries.find(([s]) => s === state)) {
        log.info(`[DIAG] state ${state.substring(0, 10)}... 在 sessionStore 中找到！`)
      } else {
        log.warn(`[DIAG] state ${state.substring(0, 10)}... 不在 sessionStore 中！已列出所有 ${allEntries.length} 个 state 的前10字符`)
      }
      ...
      if (matchedState) {
        verifier = decryptState(matchedState)
        if (verifier) {
          log.info(`[Gemini OAuth] 通过 state 解密成功匹配 verifier: ${matchedState.substring(0, 10)}...`)
        }
      }

      if (!verifier && matchedState) {
        const session = sessionStore.get(matchedState)
        if (session) {
          verifier = session.verifier
          sessionStore.delete(matchedState)
          log.info(`[Gemini OAuth] 通过 state 成功匹配 verifier: ${matchedState.substring(0, 10)}...`)
        }
      }

      if (!verifier) {
        // 尝试从 sessionStore 中寻找最新创建的未消费会话
        const allSessions = [...sessionStore.entries()]
          .map(([s, data]) => ({ state: s, ...data }))
          .filter(s => s.verifier && s.createdAt)
          .toSorted((a, b) => b.createdAt - a.createdAt)

        if (allSessions.length > 0) {
          const newestSession = allSessions[0]
          verifier = newestSession.verifier
          matchedState = newestSession.state
          sessionStore.delete(matchedState)
          log.info(`[Gemini OAuth] 未匹配到指定 state，已退避匹配最新 session 会话 (state: ${matchedState.substring(0, 10)}...)`)
        }
      }

      if (!verifier) {
        log.warn(`[Gemini OAuth] 未在 sessionStore 中找到任何有效 verifier，将使用默认 verifier`)
  ...
  async _initProjectAndPrivacy() {
    ...
    try {
      ...
      log.error(`[Gemini OAuth] 隐私设置失败: ${error.message}`)
    }
  ...
  async fetchAvailableModels(token, projectId) {
    ...
    for (const baseUrl of baseUrls) {
      try {
        ...
        log.warn(`[Gemini OAuth] fetchAvailableModels 端点 ${baseUrl} 返回 HTTP ${res.status}`)
      } catch (error) {
        log.warn(`[Gemini OAuth] fetchAvailableModels 端点 ${baseUrl} 异常: ${error.message}`)
      }
    }
    return {}
  }
  ...
  async models() {
    ...
    if (this.api_key && this.project_id) {
      try {
        ...
        log.warn(`[Gemini OAuth] 获取远程模型失败: ${error.message}`)
      }
    }
  ...
  async *chat(params, e) {
    ...
    if (global.debug || process.env.NODE_ENV === 'development') {
      log.info(`[Gemini OAuth] >>>>> 发起 LLM 请求 - 模型: ${modelName}, 动作: ${action}, 是否流式: ${Boolean(params.stream)}`)
    ...
    const executeRequest = async (isTokenRetry = false) => {
      ...
      for (const currentBaseUrl of baseUrls) {
        ...
        try {
          ...
          if (error === lastError && finalResponse && ![429, 500, 502, 503, 504].includes(finalResponse.status)) {
            throw error
          }
          lastError = error
          log.warn(`[Gemini OAuth] 请求端点 ${url} 异常: ${error.message}`)
        }
      }
