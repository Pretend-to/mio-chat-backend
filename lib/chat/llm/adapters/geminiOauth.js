...
class GeminiOauth extends Gemini {
  ...
  async refreshAccessToken() {
    ...
    logger.info(`[Gemini OAuth] >>>>> 正在使用 Refresh Token 刷新 Access Token...`)
    const response = await fetch('https://oauth2.googleapis.com/token', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
    ...
    logger.error(`[Gemini OAuth] 刷新 Token 失败`, data)
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        for (const currentBaseUrl of baseUrls) {
          ...
          try {
            ...
            logger.info(`[Gemini OAuth] 收到 HTTP 响应 - 状态码: ${fetchResponse.status} ${fetchResponse.statusText}`)
            ...
            if (fetchResponse.status === 403 && wrappedPayload.project) {
              ...
              logger.info(`[Gemini OAuth] 移除 project 后收到 HTTP 响应 - 状态码: ${retryResponse.status}`)
              finalResponse =
            ...
            if (finalResponse.status === 429) {
              logger.warn(`[Gemini OAuth] 遇到 429 频控限制，重试中...`)
              lastError = new Error(await finalResponse.text())
              continue
            }

            if (finalResponse.ok) {
              logger.info(`[Gemini OAuth] 请求成功！开始处理响应...`)
            ...
            logger.error(`[Gemini OAuth] 请求失败 (HTTP ${finalResponse.status}): ${errorBody}`)
            ...
            logger.error(`[Gemini OAuth] 请求发生异常`, err)
            lastError = err
          }
        }

        const waitTime = Math.pow(2, attempt) * 1000
        logger.warn(`[Gemini OAuth] 尝试重试中 (第 ${attempt + 1} 次重试)，等待 ${waitTime}ms...`)
