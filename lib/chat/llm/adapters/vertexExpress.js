...
export default class VertexExpressAdapter extends GeminiAdapter {
  ...
  static getAdapterMetadata() {
    return {
      type: 'vertexExpress',
      name: 'Vertex Express',
      description: 'Vertex AI Express Mode 适配器，仅需 API Key 即可调用 Vertex 模型',
  ...
  async _getModels() {
    ...
    try {
      ...
      logger.warn('无法从 API 获取模型列表，将使用手动配置的模型:', error.message)
    }
...
class VertexExpress extends Gemini {
  ...
  async *chat({
    ...
    const { systemInstruction, contents } = await this._preProcessMessage(messages)
    const body = {
  ...
  async models() {
    ...
    try {
      ...
      // 跨端失败也没关系，闭着眼睛直接降级走人工列表，不再打印报错恶心人
