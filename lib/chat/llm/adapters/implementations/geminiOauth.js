...
export default class GeminiOauthAdapter extends GeminiAdapter {
  static getAdapterMetadata() {
    ...
    const existingSessions = Array.from(sessionStore.entries())
      .map(([s, data]) => ({ state: s, ...data }))
      .filter(s => s.verifier && s.createdAt && (now - s.createdAt < reuseWindow))
    ...
    if (existingSessions.length > 0) {
      ...
      log.info(`[Gemini OAuth] 复用 10 分钟内的有效 OAuth 会话 state=${state.substring(0, 10)}...`)
    } else {
      verifier = generateCodeVerifier()
      state = encryptState(verifier, now) || generateState()
      challenge = generateCodeChallenge(verifier)
      sessionStore.set(state, { verifier, createdAt: now })
      log.info(`[Gemini OAuth] 生成新 OAuth 会话 state=${state.substring(0, 10)}...`)
    }

    // 【诊断】打印保存的 state 和当前 sessionStore 内容
    const allSessionEntries = Array.from(sessionStore.entries())
    log.info(`[DIAG] sessionStore 现有 ${allSessionEntries.length} 个条目: states=[${allSessionEntries.map(([s]) => s.substring(0, 10)).join(', ')}]`)
