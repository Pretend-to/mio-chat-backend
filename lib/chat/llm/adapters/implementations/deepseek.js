...
export default class DeepSeekAdapter extends OpenAIBot {
  ...
  // `context` 必须透传给父类：工具 schema 投影依赖它做权限判定，
  // 丢掉会让所有 requires.admin / requires.agentContext 的工具被静默剔除。
  async _prepareChatBody(body, context = null) {
    ...
    const preparedBody = await super._prepareChatBody(body, context)
