...
import { imgUrlToBase64, resolveImageAsBase64 } from '../../../../../utils/imgTools.js'
...
export default class OpenAIResponsesBot extends BaseLLMAdapter {
  ...
  async _prepareChatBody(body, context = null) {
    ...
    // 2. 处理消息中的图片等
    const processedInput = await this._processMessages(filteredMessages, model, visionOverride)
    ...
    if (mode !== 'NONE' && tools?.length > 0) {
      parsedTools = this._getFormattedTools(tools, toolCallSettings.passthrough, context || body)
    }
