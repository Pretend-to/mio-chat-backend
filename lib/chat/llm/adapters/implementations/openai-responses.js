...
export default class OpenAIResponsesBot extends BaseLLMAdapter {
  ...
  constructor(openaiConfig) {
    ...
    // reasoning 回传模式：null=按 base_url 自动判断；'echo' | 'strict'（见 _shouldEchoReasoning）
    this._reasoningEchoMode = null
  }
  ...
  /**
   * 各实现方对 input 中 reasoning 项的要求相反：
   * - OpenAI 官方：content 必须为空数组、且不接受本地生成的 id（会 404 Item not found）；
   * - DeepSeek / 火山方舟等：thinking 模式下要求把 reasoning_text 原样回传（否则 400）。
   * 默认按 base_url 判断，运行中命中特定报错时自动纠偏（_reasoningEchoMode）。
   */
  _shouldEchoReasoning() {
    if (this._reasoningEchoMode) {
      return this._reasoningEchoMode === 'echo'
    }
    return !/openai/i.test(String(this?.config?.base_url || ''))
  }

  /**
   * 命中“reasoning 回传格式不合适”的报错时切换模式，并基于原始事件体重建请求体供重试。
   * @returns {Promise<object|null>} 重建后的 preparedBody；无需/无法重试时为 null
   */
  async _maybeRetryBodyAfterReasoningError(error, e) {
    const msg = String(error?.message || '')
    let wantEcho = null
    if (/reasoning_text/i.test(msg) && /passed back/i.test(msg)) {
      wantEcho = true
    } else if (
      /input\[\d+\]\.content/i.test(msg) &&
      /(array too long|maximum length 0)/i.test(msg)
    ) {
      wantEcho = false
    } else if (/Item with id ['`]?rs_[^'`]*['`]? not found/i.test(msg)) {
      wantEcho = false
    }
    if (wantEcho === null || wantEcho === this._shouldEchoReasoning()) {
      return null
    }
    this._reasoningEchoMode = wantEcho ? 'echo' : 'strict'
    const rawBody = e?.body || null
    if (!rawBody) {return null}
    return await this._prepareChatBody(rawBody, e)
  }
  ...
  async _processMessages(messages, modelName, visionOverride = undefined) {
    ...
    for (const message of messages) {
      ...
      if (role === 'assistant' && tool_calls && Array.isArray(tool_calls)) {
        ...
        // （DeepSeek V4 / 火山方舟等要求回传 reasoning_text；OpenAI 官方则禁止 content/id，见 _shouldEchoReasoning）
        if (reasoningText && this._shouldEchoReasoning()) {
          processed.push({
            content: [
              {
                text: reasoningText,
      ...
      if (Array.isArray(mainContent)) {
        ...
        for (const part of mainContent) {
          if (part.type === 'image_url' || part.type === 'input_image') {
            ...
            // 无法解析的图片（blob:/失效地址）直接跳过，避免整轮请求被上游 400 打挂
  ...
  async _executeChatRequest(body, e) {
    ...
    try {
      if (enableStream) {
        ...
        // 使用新的 responses.create（reasoning 回传格式不符时自动纠偏并重试一次）
        let stream
        try {
          stream = await this.openai.responses.create(body, {
            signal: controller.signal,
          })
        } catch (error) {
          const retriedBody = await this._maybeRetryBodyAfterReasoningError(
            error,
            e,
          )
          if (!retriedBody) {throw error}
          logger.warn(
            `[${this.provider.toUpperCase()}-Responses] reasoning 回传模式切换为 ${this._reasoningEchoMode}，重试请求`,
          )
          stream = await this.openai.responses.create(retriedBody, {
            signal: controller.signal,
          })
        }
        ...
        let response
        try {
          response = await this.openai.responses.create(body, {
            signal: controller.signal,
          })
        } catch (error) {
          const retriedBody = await this._maybeRetryBodyAfterReasoningError(
          ...
          response = await this.openai.responses.create(retriedBody, {
            signal: controller.signal,
          })
        }
