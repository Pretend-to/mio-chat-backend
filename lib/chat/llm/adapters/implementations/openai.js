...
export default class OpenAIBot extends BaseLLMAdapter {
    ...
    for (const [provider, models] of Object.entries(avaliableReasoningModels)) {
      if (models.includes(model)) {
        if (provider === 'openai') {
          ...
          if (chatParams.reasoning_effort !== undefined) {
            chatParams.reasoning_effort =
              chatParams.reasoning_effort >= 2 ? 'high' : 'low'
          }
        }
      } else {
        // 非推理模型移除该参数，避免 API 报错
  ...
  async _executeChatRequest(body, e) {
    ...
    if (enableStream) {
      ...
      for await (const chunk of stream) {
        console.log('Received chunk:', JSON.stringify(chunk, null, 2)) // 调试日志，查看每个chunk的结构和内容
        if (e.aborted) break
  ...
  async _handleToolCalls(toolCalls, e, stepId) {
    ...
    for (const call of toolCalls) {
      ...
      const runTask = async () => {
        try {
          ...
          // Handle special result format that includes post-tool messages (e.g., for vision injection)
    ...
    // 核心修复：所有 tool 结果注入完成后，合并并统一注入后置消息（如图片），避免角色交替错误
