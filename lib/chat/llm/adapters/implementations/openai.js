...
export default class OpenAIBot extends BaseLLMAdapter {
  ...
  /**
   * 构建基础请求体（规范化参数、处理多模态与消息、投射 context 权限工具）
   */
   ...
   * 应用思考/推理强度策略
   */
  _applyReasoningEffort(preparedBody, rawEffort) {
    const reasoningStrategy =
      this.reasoningStrategy || this.profile?.reasoningStrategy
    const defaultEffort =
      this.defaultReasoningEffort !== undefined
        ? this.defaultReasoningEffort
        : this.profile?.defaultReasoningEffort
    const effort =
      rawEffort !== undefined && rawEffort !== null
        ? rawEffort
        : defaultEffort

    if (reasoningStrategy === 'extra_body') {
      const effortTable =
        this.reasoningEffortTable || this.profile?.reasoningEffortTable || null
      this._applyExtraThinkingBody(preparedBody, effort, effortTable)
      return
    }

    const openaiReasoningEffortTable =
      this.reasoningEffortTable ||
      this.profile?.reasoningEffortTable || {
        '-1': undefined,
        0: undefined,
        1: undefined,
        2: 'low',
    ...
    if (effort !== undefined && effort !== null) {
      mappedReasoningEffort =
        openaiReasoningEffortTable[String(effort)] ??
  ...
  /**
   * 模板方法：准备聊天请求体（Final，子类/厂商严格禁止重写以保证 context 透传）
   * 步骤：
   * 1. _buildChatRequestBody 构建基础体并严格透传 context 投影工具
   * 2. _applyReasoningEffort 格式化推理强度/思考模式
   * 3. _upsertNativeTool 装配声明的原生内置工具
   * 4. postProcessChatBody 执行保护扩展钩子
   * 5. cleanUndefined 清理并返回
   */
  async _prepareChatBody(body, context = null) {
    ...
    this._applyReasoningEffort(preparedBody, rawEffort)

    const nativeTools = this.nativeTools || this.profile?.nativeTools || []
  ...
  async _executeChatRequest(body, e) {
    ...
    if (enableStream) {
      ...
      logger.info(`[LLMTrace] requestId=${e.requestId} stage=upstream.start provider=${this.provider} model=${body.model} round=${e._recursionRound}`)
      const stream = await this.openai.chat.completions.create(body, {
        signal: controller.signal,
      })
      logger.info(`[LLMTrace] requestId=${e.requestId} stage=upstream.headers durationMs=${(performance.now() - upstreamStartedAt).toFixed(1)}`)
      ...
      logger.info(`[LLMTrace] requestId=${e.requestId} stage=upstream.response durationMs=${(performance.now() - upstreamStartedAt).toFixed(1)}`)
