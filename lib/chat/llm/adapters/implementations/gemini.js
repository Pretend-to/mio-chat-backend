    ...
    for (const toolCall of toolCalls) {
      ...
      const runTask = async () => {
        try {
          ...
          executedTools.push({
            name: call.name,
            arguments: typeof call.params === 'string' ? call.params : JSON.stringify(call.params),
...
export class Gemini {
  ...
  async *chat(params, e = null) {
    const stepId = e?._currentStepId || Math.random().toString(36).substring(2, 9)
    const timeMetrics = {
      startTime: Date.now(),
      firstTokenTime: null,
      model: params.model,
    ...
    try {
      ...
      if (stream) {
        ...
        if (usage) {
          logger.mark(`[Gemini Usage Metadata / 用量数据]:\n${JSON.stringify(usage, null, 2)}`)
        }
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('data: ')) {
        ...
        try {
          ...
          if (usage) {
            logger.mark(`[Gemini Usage Metadata / 用量数据] (Stream):\n${JSON.stringify(usage, null, 2)}`)
          }
          if (usage && usage.promptTokenCount !== undefined) {
            BaseLLMAdapter.prototype.logUsage(this.provider || 'Gemini', usage, timeMetrics)
          }
      ...
      try {
        ...
        if (usage) {
          logger.mark(`[Gemini Usage Metadata / 用量数据] (Stream Cache):\n${JSON.stringify(usage, null, 2)}`)
        }
        if (usage && usage.promptTokenCount !== undefined) {
          BaseLLMAdapter.prototype.logUsage(this.provider || 'Gemini', usage, timeMetrics)
        }
