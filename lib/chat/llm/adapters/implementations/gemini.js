...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  async _prepareChatBody(body) {
    ...
    // Gemini 家族适配器的 extraSettings key 名（gemini/geminiOauth/agentPlatform）
    const GEMINI_FAMILY_KEYS = ['gemini', 'geminiOauth', 'agentPlatform']
    const gemini = GEMINI_FAMILY_KEYS.reduce((acc, key) => acc || extraSettings[key], null) || extraSettings || {}
    const internalTools = gemini.internalTools || extraSettings.internalTools || {}
    const { imageGeneration, safetySettings = {} } = gemini
