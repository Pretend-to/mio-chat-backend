...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  async _executeChatRequest(body, e) {
    ...
    const providerName = this.provider === 'gemini' ? 'Gemini' : this.provider
    ...
    if (callMessage.length > 0) {
      ...
      return { toolCalls: callMessage, stepId }
    }
  ...
  async _prepareChatBody(body) {
    ...
    const gemini = extraSettings.gemini || extraSettings.geminiOauth || extraSettings.agentPlatform || {}
    ...
    const hasBuiltIn = parsedTools.some(t => t.googleSearch || t.googleSearchRetrieval || t.codeExecution || t.urlContext)
    const hasCustom = parsedTools.some(t => t.functionDeclarations)
    ...
    // 动态判断思考配置类型：
    // 1. 黑名单拦截机制：拦截非 Gemini 模型和 2.0/1.5/1.0 等老旧模型
    const modelLower = base.model.toLowerCase()
    const isNonGemini = !modelLower.includes('gemini')
    const isOldGemini = modelLower.includes('2.0') || modelLower.includes('1.5') || modelLower.includes('1.0')

    const isReasoningAvaliable = !isNonGemini && !isOldGemini && !isImageModel

    // 2. 动态路由配置档：
    // - 2.5-pro 模型使用 pro 档 (数字 budget)
    // - 2.5-flash 模型使用 flash 档 (数字 budget)
    // - 其他通过校验的新思考模型 (如 3.x 及未来版本) 默认使用 level 档 (LOW/HIGH 状态)
    let rangeType = null
    if (isReasoningAvaliable) {
      if (modelLower.includes('2.5-pro')) {
        rangeType = 'pro'
      } else if (modelLower.includes('2.5-flash') || modelLower.includes('2.5-flash-light')) {
        rangeType = 'flash'
      } else {
        rangeType = 'level'
      }
  ...
  async _handleToolCalls(toolCalls, e, stepId) {
    ...
    const content = e.body.extraCachedContent || null
    if (e.body.extraCachedContent) {
      delete e.body.extraCachedContent
    }

    Gemini.pushToolCallMessages(e.body.messages, toolCalls, content)
    ...
    for (const toolCall of toolCalls) {
      ...
      const runTask = async () => {
        try {
          ...
          executedTools.push({
            name: call.name,
            arguments: typeof call.parameters === 'string' ? call.parameters : JSON.stringify(call.parameters),
...
export class Gemini {
  ...
  *_parseMutiJson(buffer) {
  ...
  // 把 Gemini message push 到 openai messages，支持传入 content 合并
  static pushToolCallMessages(messages, res, content = null) {
    const tool_calls = res.map((res) => {
      return {
        ...
        function: {
          ...
          arguments: typeof res.functionCall.args === 'string'
            ? res.functionCall.args
            : (res.functionCall.args ? JSON.stringify(res.functionCall.args) : ''),
        },
      }
    })
    messages.push({
...
export function logGeminiUsage(usage, timeMetrics) {
  // Deprecated, handled inside adapter _executeChatRequest
}
