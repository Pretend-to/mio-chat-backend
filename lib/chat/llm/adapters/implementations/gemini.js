...
export default class GeminiAdapter extends BaseLLMAdapter {
  ...
  async _executeChatRequest(body, e) {
    ...
    for await (const chunk of stream) {
      ...
      // 仅在收到首个有效输出块时记录 TTFT，避免把空 Candidate/元数据包当首字。
      const hasVisibleOutput = chunk.candidates?.[0]?.content?.parts?.some(
        (part) =>
          part.text ||
          part.functionCall ||
          part.inlineData ||
      ...
      if (hasVisibleOutput && !timeMetrics.firstTokenTime) {
        ...
        for (let partIdx = 0; partIdx < content.parts.length; partIdx++) {
          ...
          if (text) {
            ...
            if (!thought) {
              ...
              cachedMessage.reasoning_content += text
            }
          }

          const actualThoughtSignature =
            thoughtSignature || thought_signature || null
          ...
          if (functionCall) {
            ...
            if (!isDuplicate) {
              const fcId = functionCall.id || null
              const sig = actualThoughtSignature || null
              let callId
              if (sig || fcId) {
                callId = `gsig_${Buffer.from(`${sig || ''}|${fcId || ''}`).toString('hex')}`
              } else {
                callId = `local_${this._getRandomCallId()}`
              }
              const toolCallElem = {
                functionCall,
                functionCallId: fcId,
              ...
              if (sig) {
                toolCallData.thoughtSignature = sig
              }
              e.update({
          ...
          if (actualThoughtSignature) {
            const targetMsg = callMessage.find(
              (msg) => msg.partIdx === partIdx && !msg.thoughtSignature,
            )
            if (targetMsg) {
              targetMsg.thoughtSignature = actualThoughtSignature
            } else {
              // 容错兜底：如果找不到对应的 partIdx，则赋给最后一个尚未设置 ID 的工具调用
              const lastUnsigned = [...callMessage]
                .toReversed()
                .find((msg) => !msg.thoughtSignature)
              if (lastUnsigned) {
                lastUnsigned.thoughtSignature = actualThoughtSignature
              }
            }
          }
        }
    ...
    const finalUsage = lastUsageMetadata || {
    ...
    if (callMessage && callMessage.length > 0) {
      timeMetrics.toolsCalled = callMessage
        .map((msg) => msg.functionCall?.name)
        .filter(Boolean)
    }
    ...
    if (callMessage.length > 0) {
      const currentStep = (e._recursionStep || 0) + 1
      const normalizedToolCalls = callMessage.map((msg) => {
        ...
        return {
          function: {
            arguments:
              typeof msg.functionCall.args === 'string'
      ...
      normalizedToolCalls.forEach((call) => {
        const matchingElem = callMessage.find((m) => m.id === call.id)
        if (!matchingElem || !matchingElem.startedEmitted) {
          const toolCallData = {
            action: 'started',
            id: call.id,
          ...
          if (matchingElem) {
            matchingElem.startedEmitted = true
          }
        }
      })
  ...
  async _prepareChatBody(body, context = null) {
    ...
    const adapterType =
      this.constructor.getAdapterMetadata()?.type || this.provider
    const gemini =
      extraSettings[adapterType] ||
      extraSettings['gemini'] ||
      extraSettings['geminiOauth'] ||
      extraSettings['agentPlatform'] ||
      extraSettings ||
      {}
    const internalTools =
      gemini.internalTools || extraSettings.internalTools || {}
    const { safetySettings = {} } = gemini
    const isImageModel = base.model.toLowerCase().includes('image')
    const parsedTools = []
    if (mode !== 'NONE' && !isImageModel) {
      const validTools = this._getFormattedTools(
        tools,
    ...
    const isEnabled = (val) => {
      ...
      if (
        typeof val === 'object' &&
        val !== null &&
        (val.enable === true || val.enabled === true)
      ) {
        return true
      }
      return false
