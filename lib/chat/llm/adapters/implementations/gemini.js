...
export default class GeminiAdapter extends BaseLLMAdapter {
    ...
    for await (const chunk of stream) {
      ...
      if (content?.parts) {
        for (let partIdx = 0; partIdx < content.parts.length; partIdx++) {
          ...
          if (thoughtSignature) {
            const targetMsg = callMessage.find((msg) => msg.partIdx === partIdx && !msg.id)
            if (targetMsg) {
              targetMsg.id = thoughtSignature
            } else {
              // 容错兜底：如果找不到对应的 partIdx，则赋给最后一个尚未设置 ID 的工具调用
              const lastUnsigned = [...callMessage].reverse().find((msg) => !msg.id)
              if (lastUnsigned) {
                lastUnsigned.id = thoughtSignature
              }
            }
          }
        }
  ...
  async _prepareChatBody(body) {
    ...
    const hasBuiltIn = parsedTools.some(
      (t) =>
        t.googleSearch ||
        t.googleSearchRetrieval ||
        t.codeExecution ||
        t.urlContext,
    )
