...
const ALL_SOURCES = ['web', 'channel', 'proxy', 'internal']
...
function narrowAccess(parent, child) {
  return {
    ...
    scene: {
      ...
      sources: intersect(parent.scene.sources, child.scene.sources),
...
function canonicalSource(source) {
  if (ALL_SOURCES.includes(source)) return source
...
export function normalizeToolAccessContext(context = null) {
  ...
  const triggerKind =
    event?.triggerKind ||
    wrapper.triggerKind ||
    (rawSource === 'scheduled_task' || rawSource === 'subagent'
      ? 'task'
      : rawSource === 'trigger' || rawSource === 'system'
        ? 'system'
        : 'interactive')
...
export function evaluateToolAccess(tool, context = null, operation = 'execute') {
  ...
  if (!policy.scene.sources.includes(accessContext.source)) {
...
/**
 * Agent chats have one immutable core tool policy. The Agent manager owns
 * profile/model/session operations; Channels remain delivery-only.
