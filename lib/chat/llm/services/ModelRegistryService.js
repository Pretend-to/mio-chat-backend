...
const BUILTIN_MODEL_RULES = [
  ...
  // DeepSeek 系列
  // DeepSeek Flash（原生 1M 上下文，384K 输出，支持图像多模态与思考模式）
  {
  ...
  // DeepSeek V4 Pro（原生 1M 上下文，384K 输出，不支持图像多模态，支持思考模式）
  {
  ...
  // DeepSeek 基础/其他系列 (Chat/Reasoner/V3/R1/V4)
  {
...
class ModelRegistryService {
  ...
  /**
   * 重建归一化索引：key 为去掉 provider 前缀的裸模型名
   * (如 openrouter/deepseek/deepseek-v4.1-flash -> deepseek-v4.1-flash)，
   * 让裸名配置也能命中带前缀的 LiteLLM 条目。
   * 规则：跳过裸 key (本身已可直接命中)、modelsData 已有裸名则跳过、先到先得。
   */
  _rebuildNormalizedIndex() {
    this.normalizedIndex = new Map()
    for (const [key, value] of this.modelsData.entries()) {
      if (!key.includes('/')) continue
      const normKey = this._normalizeModelName(key)
      if (!normKey || normKey === key) continue
      if (this.modelsData.has(normKey)) continue
      if (!this.normalizedIndex.has(normKey)) {
        this.normalizedIndex.set(normKey, value)
      }
    }
  }
  ...
  getModelMetadata(rawModelName) {
    ...
    let entry =
      this.modelsData.get(exactName) ||
      this.modelsData.get(normName) ||
      (this.normalizedIndex.get(normName) ?? null)
