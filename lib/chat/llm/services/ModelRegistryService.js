...
const BUILTIN_MODEL_RULES = [
  ...
  // 智谱 GLM 系列
  { pattern: /glm-[45](\.\d+)?(-turbo)?(-air)?(-flash)?$/i, maxInput: 131072, maxOutput: 4096, vision: false, fc: true, reasoning: false },
  { pattern: /glm-4v|glm-5v/i, maxInput: 8192, maxOutput: 4096, vision: true, fc: true, reasoning: false },
  // 零一万物 (Yi) 系列
  ...
  // 商汤 SenseNova 系列
  { pattern: /sensenova/i, maxInput: 131072, maxOutput: 8192, vision: true, fc: true, reasoning: false },
  // Google Gemma 开源系列
  { pattern: /gemma-[34]/i, maxInput: 131072, maxOutput: 8192, vision: false, fc: true, reasoning: false },
  // 字节豆包 Seed 系列
  { pattern: /doubao-seed/i, maxInput: 131072, maxOutput: 8192, vision: true, fc: true, reasoning: false },
  // 百川 Baichuan 系列
...
class ModelRegistryService {
  ...
  getModelMetadata(rawModelName) {
    ...
    for (const rule of BUILTIN_MODEL_RULES) {
      if (rule.pattern.test(normName) || rule.pattern.test(exactName)) {
        matchedRule = rule
        if (!matchSource) { matchSource = 'builtin-rule' }
        break
      }
    }
    if (!matchSource) { matchSource = 'fallback' }
  ...
  auditConfiguredModels(llmService) {
    ...
    const stats = { ['litellm-exact']: 0, ['litellm-prefix']: 0, ['builtin-rule']: 0, fallback: 0 }
    ...
    for (const [instanceId, instance] of Object.entries(llmService.llms)) {
      ...
      for (const model of modelIds) {
        ...
        if (source === 'fallback') {
          fallbackModels.push({ instanceId, model })
        }
        logger.info(
          '[ModelRegistry] 模型匹配 ' + source.padEnd(14) + ' | ' + instanceId.padEnd(20) + ' | ' + model +
          ' (ctx=' + meta.max_input_tokens + ', wm=' + meta.crystallize_watermark + ', vision=' + meta.supports_vision + ')'
        )
      }
    }
    ...
    logger.info(
      '[ModelRegistry] 存量模型匹配审计完成: 共 ' + total + ' 个模型 | 精确命中 ' + stats['litellm-exact'] +
      ' | 前缀命中 ' + stats['litellm-prefix'] + ' | 内置规则命中 ' + stats['builtin-rule'] +
      ' | 未知兜底 ' + (stats.fallback || 0)
    )

    if (fallbackModels.length > 0) {
      logger.warn(
        '[ModelRegistry] 以下 ' + fallbackModels.length + ' 个模型未命中注册表，使用默认 128k 上下文兜底，建议补充规则:' +
        fallbackModels.map((f) => ' ' + f.instanceId + '/' + f.model).join(',')
      )
    }
  }
