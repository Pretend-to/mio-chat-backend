  ...
  for (const msg of history) {
    if (msg.role === 'system') {
      ...
      const textContent = Array.isArray(msg.content)
        ? msg.content.filter(c => c.type === 'text').map(c => c.data?.text || '').join('')
        : String(msg.content || '')
      msgs.push({ content: textContent, role: 'user' })
    } else if (msg.role === 'assistant' || msg.role === 'other') {
      if (!Array.isArray(msg.content)) {
        msgs.push({ content: String(msg.content || ''), role: msg.role })
        continue
      }
      ...
      msg.content.forEach((elm) => {
        if (elm.type === 'reason') {
          ...
          if (!currentAssistant) {
            currentAssistant = { content: '', role: msg.role }
          }
...
function convertLLMMessagesToFrontendHistory(messages) {
  ...
  while (i < messages.length) {
    ...
    if (msg.role === 'system') {
      ...
      if (msg.content) {
        assistantMsg.content.push({ data: { text: msg.content }, type: 'text' })
      }
...
class TaskRunnerService {
  ...
  async runTask(presetName, triggerPrompt, options = {}) {
    ...
    try {
      ...
      logger.error(`[TaskRunner] 未能在数据库中检索到任务: ${taskId}`, error.message)
    }
    ...
    if (task && task.history) {
      try {
        ...
        logger.error(`[TaskRunner] 解析任务 "${taskId}" 的 history 失败:`, error.message)
        history = []
      }
    }
    ...
    // 4. 创建虚拟客户端 (用于推送和状态追踪)
    const client = new VirtualLLMClient(taskId, { contactorId, userId })
    ...
    // 7. 使用规范工厂构造标准后台任务事件
    const event = ChatEventFactory.createForTask({
      client,
      contactorId,
      ...
      settings: {
        ...
        crystallization_token_watermark: 64000,
    ...
    try {
      ...
      const runResult = await Promise.race([client.done, timeoutPromise])
      const aborted = Boolean(runResult && runResult.aborted)

      if (aborted) {
        logger.info(`[TaskRunner] 任务 "${taskId}" 收到中止信号，正在处理已生成的部分内容入库`)
      } else {
      ...
      // 提取本轮的最终/部分文本和思考内容，拼接到 event.body.messages 中，以确保重建的 History 包含最新的回复
      const textResponse = chunks
        .filter((c) => c.type === 'content')
        .map((c) => c.content)
        .join('')
      const reasoningResponse = chunks
        .filter((c) => c.type === 'reason')
        .map((c) => c.data?.text || '')
        .join('')

      if (textResponse || reasoningResponse) {
        if (!event.body.messages) {
          event.body.messages = []
        }
        const lastMsg = event.body.messages[event.body.messages.length - 1]
        // 避免重复推送同一条完整内容
        if (
          !lastMsg ||
          lastMsg.role !== 'assistant' ||
          (lastMsg.content !== textResponse && lastMsg.reasoning_content !== reasoningResponse)
        ) {
          event.body.messages.push({
            content: textResponse,
            reasoning_content: reasoningResponse || undefined,
            role: 'assistant',
          })
        }
      }
      ...
      try {
        ...
        logger.error(`[TaskRunner] 写入失败记录时出错:`, error.message)
      }
  ...
  _assembleAssistantMessage(chunks, _messageId) {
    ...
    if (!chunks || !Array.isArray(chunks) || chunks.length === 0) {
      msg.content.push({ data: { text: 'Success' }, type: 'text' })
      return msg
    }
