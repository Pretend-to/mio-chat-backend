...
function convertFrontendHistoryToLLMMessages(history) {
  ...
  for (const msg of history) {
    ...
    if (Array.isArray(msg.content)) {
      ...
      const reasonElms = msg.content.filter(c => c.type === 'reason' || c.type === 'reasoningContent')
      if (reasonElms.length > 0) {
        reasoningContent = reasonElms.map(r => r.data?.text || r.content || '').join('')
      }
...
class TaskRunnerService {
  ...
  async runTask(presetName, triggerPrompt, options = {}) {
    ...
    try {
      ...
      //    包含普通文本、推理和工具调用，防止后续运行退化为零样本学习而伪造工具结果
          ...
          if (chunk.type === 'content') {
            ...
            newAssistantMsg.content.push({
              ...
              data: {
                ...
                arguments: chunk.content.arguments,
                status: chunk.content.status || 'done',
