...
const STREAM_DIR = path.resolve(process.cwd(), 'output', 'task-streams')
...
class TaskRunnerService {
  ...
  async runTask(presetName, triggerPrompt, options = {}) {
    ...
    // 创建临时流缓存文件路径（用于重启后恢复）
    const streamFilePath = path.join(STREAM_DIR, `${taskId}-${messageId}.json`)

    let running = true
    let debounceTimer = null

    // 防抖写文件：500ms 内无新 chunk 时落盘
      ...
      debounceTimer = setTimeout(() => {
        ...
        try {
          ...
          if (cachedItem && Array.isArray(cachedItem.chunks)) {
            ...
            fs.writeFileSync(streamFilePath, JSON.stringify(cachedItem.chunks))
          }
        } catch { /* 写文件失败不影响主流程 */ }
      }, 500)
    }
    ...
    try {
      ...
      // 8. 后台轮询 streamCache，防抖持久化（每 1s 检查一次）
      const pollTimer = setInterval(() => {
        ...
        const messagesInCache = streamCache.cache.get(cacheKey)
        const cachedItem = messagesInCache?.find(m => m.messageId === messageId)
        if (cachedItem && Array.isArray(cachedItem.chunks) && cachedItem.chunks.length > 0) {
          scheduleFlush()
        }
      }, 1000)
      ...
      //    优先读 streamCache，若丢失（如进程重启）则从文件恢复
      let chunks = null
      const cacheKey2 = `${userId}:${contactorId}`
      const messagesInCache2 = streamCache.cache.get(cacheKey2)
      const cachedItem2 = messagesInCache2?.find(m => m.messageId === messageId)

      if (cachedItem2 && Array.isArray(cachedItem2.chunks) && cachedItem2.chunks.length > 0) {
        chunks = cachedItem2.chunks
      } else {
        // fallback：从文件恢复
        try {
          if (fs.existsSync(streamFilePath)) {
            const raw = fs.readFileSync(streamFilePath, 'utf-8')
            chunks = JSON.parse(raw)
            logger.info(`[TaskRunner] 从文件恢复 streamCache: ${streamFilePath} (${chunks.length} chunks)`)
          }
        } catch (err) {
          logger.warn(`[TaskRunner] 从文件恢复 streamCache 失败: ${err.message}`)
        }
      }
      ...
      // 如果 chunks 为空（包括 fallback 失败），兜底文本
      if (newAssistantMsg.content.length === 0) {
        newAssistantMsg.content.push({
          type: 'text',
          data: { text: 'Success' }
        })
      ...
      // 清理临时文件
      try { fs.unlinkSync(streamFilePath) } catch { /* ignore */ }
