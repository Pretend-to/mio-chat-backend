...
export function scanFrontendTurns(messages, keepTurns = 1) {
  ...
  // Manual compaction may explicitly retain no raw turns. In that mode the
  // whole transcript is summarized and only the durable crystal remains.
  if (keepTurns <= 0) {
  ...
  while (i >= 0) {
    ...
    // Tool-generated _postMessages are continuations of the current
    // frontend turn.  Counting them as a new turn would make crystallization
    // discard the actual current user message and its tool-call chain.
    if (msg.role === 'user' && !msg._is_recursive_context) {
      turnsFound++
...
function buildCompressMessages(
  ...
  const copiedPrefix = messagesToCompress.map((msg) => {
    ...
    if (msg.name) {
      copied.name = msg.name
    }
    if (msg.tool_call_id) {
      copied.tool_call_id = msg.tool_call_id
    }
    return copied
  })
...
export async function compress(e, llm, boundaryIndex) {
  ...
  const compressMessages = buildCompressMessages(
    compactPrompt,
    previousSummary,
    messages,
    pendingEvents,
  ...
  let summaryXml = ''
  const compressEvent = ChatEventFactory.createInternal({
    messages: compressMessages,
    model: settings.base?.model || llm.models[0]?.models?.[0],
    onContent: (content) => {
      summaryXml += content
  ...
  logger.debug(
    `[CrystallizationService] 压缩完成，新结晶长度: ${summaryXml.length} 字符`,
  )
