...
export const ChannelNormalizer = {
  ...
  normalize({ ctx, messages, settings }) {
    ...
    const isTask = Boolean(ctx.isTask || ctx.isWake)
    ...
    const actorId = String(ctx.principal?.externalUserId || ctx.from || rawUser)
    ...
    const externalConversationId =
      ...
      null
    // The UI conversation identity is the Agent, except for a SubAgent's
    // explicit child stream contact. A physical Channel is transport metadata.
    const contactorId = ctx.streamContactorId
      ? String(ctx.streamContactorId)
      : ctx.agentId
        ? String(ctx.agentId)
        : channelId
    const resolvedPrincipalId = String(
    ...
    const requestId = String(
      ctx.eventId || `${channelType}_${sessionId || Date.now()}_${Date.now()}`,
    )
    ...
    return {
      ...
      channelContext: ctx,
      channelSource: ctx.source || EVENT_SOURCE.CHANNEL,
      channelId,
