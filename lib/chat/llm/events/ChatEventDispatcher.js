...
export class ChatEventDispatcher {
  ...
  async submit(event, { start } = {}) {
    ...
    if (activeEvent) {
      ...
      if (event.source === 'web') {
        logger.info(
          `[LLMTrace] requestId=${event.requestId} stage=dispatch.adjust ` +
            `activeRequestId=${activeEvent.requestId} status=${adjustStatus}`,
        )
      }
      return {
        adjustStatus,
    ...
    if (event.source === 'web') {
      logger.info(
        `[LLMTrace] requestId=${event.requestId} stage=dispatch.start ` +
    ...
    try {
      ...
      if (event.historyOwnership === 'client') {
  ...
  _scheduleDrain(conversationKey, delayMs = 0) {
    if (!this.wakeRunner) {
      // 以前这里静默 return false：组合根忘了先 registerWakeRunner，
      // 待处理工作就无声地永远不执行。宁可吵。
      console.warn(
        '[ChatEventDispatcher] 尚未安装 wake runner，排程被丢弃，工作项将无人执行。' +
          ' 请确认 registerWakeRunner 先于 resumePending / requestDrain 调用。',
      )
      return false
    }
    if (!conversationKey || this.drainTimers.has(conversationKey)) {
      return false
    }
