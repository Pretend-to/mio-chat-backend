...
export class WebChatEvent extends ChatEvent {
  ...
  /** Close the visible answer at an adjustment checkpoint, keeping this LLM loop alive. */
  splitOutputForAdjustment(nextMessageId) {
    const nextId = String(nextMessageId || '')
    if (!nextId || nextId === this.messageId || this.completed) return false
    const previousMessageId = this.messageId
    if (this.contactorId) {
      streamCache.complete(this.principalId, this.contactorId, previousMessageId)
    }
    this._sendToSocket('complete', { segmentBoundary: true })
    this.messageId = nextId
    this._everUpdate = false
    this._sendToSocket('pending', { continuationOfRequestId: this.requestId })
    return true
  }
  ...
  async _checkAndSummarize() {
    ...
    if (
      ...
      const newTitle = await llmService.generateChatTitle(titleContext, {
        model: this.settings?.base?.model || null,
