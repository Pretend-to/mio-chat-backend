...
export class ChatEventFactory {
  ...
  static create(canonicalParams) {
    ...
    switch (source) {
      ...
      default:
        throw new TypeError(`[ChatEventFactory] 不支持的 source 类型: "${source}"`)
    }
  }
   ...
   */
  static createForWeb({ req, client }) {
   ...
   * 构造外部渠道 (微信、飞书等) 对话事件
   */
  static createForChannel({ ctx, messages, settings }) {
   ...
   * 构造后台自动化任务事件
   */
  static createForTask(taskOptions) {
   ...
   * 构造 HTTP API 代理事件
   */
  static createForProxy(proxyOptions) {
   ...
   * 构造系统内部轻量级任务事件
   */
  static createInternal(internalOptions) {
    const canonical = InternalNormalizer.normalize(internalOptions)
    return new InternalChatEvent(canonical)
  }

  static createForInternal(internalOptions) {
    return this.createInternal(internalOptions)
  }
