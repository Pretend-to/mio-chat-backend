...
class LLMChatService {
  ...
  async generateChatTitle(messages) {
    try {
      ...
      // 仅取最近 5 条消息以极大地减少 token 消耗，并截断单条过长的文本
      const cleanContext = filteredMessages.slice(-5).map(m => {
  ...
  async runTool(toolCall, user, parentEvent = null) {
    try {
      ...
      const e = {
        ...
        registerInteraction: (interactionId, callback) => parentEvent?.registerInteraction(interactionId, callback),
