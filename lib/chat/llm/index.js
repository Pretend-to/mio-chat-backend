...
class LLMChatService {
  ...
  async refreshInstanceModels(instanceId) {
    ...
    try {
      logger.info(`刷新实例 ${instanceId} 的模型列表`)
      await llm.loadModels()
      logger.info(`实例 ${instanceId} 模型列表刷新成功`)
      return { success: true }
    } catch (error) {
      logger.error(`实例 ${instanceId} 模型列表刷新失败:`, error)
  ...
  _broadcastModelsUpdate() {
    try {
      ...
      const models = this.getModelList(true) // 获取管理员视图的完整模型列表
      ...
      if (global.middleware && global.middleware.socketServer && global.middleware.socketServer.io) {
        ...
        global.middleware.socketServer.io.emit('message', JSON.stringify(updateMessage))
        logger.info('[_broadcastModelsUpdate] 已向所有客户端推送模型更新')
      }
    } catch (error) {
      logger.error('[_broadcastModelsUpdate] 推送模型更新失败:', error)
    }
  }
