...
class LLMChatService {
      ...
      if (adapterType === 'geminiOauth' && !preparedConfig.onConfigUpdate) {
        preparedConfig.onConfigUpdate = async (updates) => {
          try {
            ...
            logger.error(`[Gemini OAuth][onConfigUpdate] 持久化凭证失败: ${err.message}`)
          }
        }
      }
  ...
  handleMessage(e) {
    try {
      ...
      this._injectSkillCatalog(e)

      // Merge preset history into messages
      this._mergePresetHistory(e)
  ...
  async generateChatTitle(messages) {
    try {
      ...
      logger.error('[LLMChatService] Chat title generation failed:', err)
      return null
    }
  }
  ...
  async runTool(toolCall, user, parentEvent = null) {
    try {
      ...
      if (!app) {
        // 尝试模糊匹配：如果 AI 记错了哈希后缀，尝试通过前缀找回
        const calledName = toolCall.name
        const calledBase = calledName.split('_mid_')[0]
        
        const suggestions = []
        for (const toolName of allTools.keys()) {
          const toolBase = toolName.split('_mid_')[0]
          if (toolBase === calledBase) {
            suggestions.push(toolName)
          }
        }

        let msg = `未找到插件: ${calledName}`
        if (suggestions.length > 0) {
          msg += `。你是不是想调用: ${suggestions.join(', ')}? 请使用完整的工具名重新调用。`
        }
        
        logger.warn(`[runTool] ${msg}`)
        throw new Error(msg)
      }
      ...
      const e = {
        user,
        params,
        // 注入原始请求上下文，供工具提取环境快照（如 SystemPrompt, ContactorId 等）
        ...
        setExtraRender: (renders) => {
          ...
          const newRenders = [...processed, ...existingOuter]
          toolCall.extraRender = newRenders
          toolCall.innerRender = newRenders.filter(r => r.placement === 'inner')
          toolCall.outerRender = newRenders.filter(r => r.placement === 'outer')
          if (parentEvent) {
            parentEvent.update({
              type: 'toolCall',
              content: toolCall
        ...
        setOuterRender: (renders) => {
          ...
          const newRenders = [...existingInner, ...processed]
          toolCall.extraRender = newRenders
          toolCall.innerRender = newRenders.filter(r => r.placement === 'inner')
          toolCall.outerRender = newRenders.filter(r => r.placement === 'outer')
          if (parentEvent) {
  ...
  _injectSkillCatalog(e) {
    try {
      ...
      let systemMessage = e.body.messages.find(m => m.role === 'system')
      if (systemMessage) {
      ...
      logger.error('Error injecting skill catalog:', err)
    }
  }

  /**
   * Merge preset history into the message chain
   * @private
   */
    ...
    try {
      ...
      for (let i = 0; i < e.body.messages.length; i++) {
        if (e.body.messages[i].role === 'system') {
          ...
          // System messages should be at the very beginning
          break
        }
      }

      // Insert history after system messages
      e.body.messages.splice(lastSystemIndex + 1, 0, ...history)
    } catch (err) {
      logger.error('Error merging preset history:', err)
    }
  }
}
