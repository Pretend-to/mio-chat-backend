...
class LLMChatService {
  ...
  async generateChatTitle(messages) {
    try {
      const channelId = await SystemSettingsService.get('system_llm_channel')
      const prompt = await SystemSettingsService.get('system_llm_title_prompt') || '请根据这段对话内容，为这个会话起一个极其简短、精准的标题（不超过 6 个字）。注意：你的回答只能包含标题本身，不要包含任何标点符号、解释或引用。'

      const llm = this.llms[channelId] || Object.values(this.llms)[0]
      if (!llm) return null

      // 仅保留文本内容，排除推理链等非展示内容
      const cleanContext = messages.slice(-10).map(m => ({
        role: m.role,
        content: typeof m.content === 'string' ? m.content : JSON.stringify(m.content)
      }))

      let resultText = ''
      await llm.handleChatRequest({
        requestId: `system_title_${Date.now()}`,
        body: {
          settings: {
            base: { model: llm.models[0]?.models?.[0] || 'default', stream: false },
            toolCallSettings: { mode: 'NONE' }
          },
          messages: [{ role: 'user', content: `${prompt}\n\n对话内容：\n${JSON.stringify(cleanContext)}` }]
        },
        client: { pushEvent: () => {}, popEvent: () => {}, popConnection: () => {} },
        update: (data) => { if (data.type === 'content') resultText += data.content },
        complete: () => {},
        error: (err) => { throw err }
      })
