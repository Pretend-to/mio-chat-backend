...
// 定义一个异步函数，接受JPEG Base64编码的图像作为参数，返回PNG Base64编码的图像
const getPng64 = async (jpegBase64) => {
    try {
        // 将JPEG Base64编码的图像转换为Jimp对象
        const image = await Jimp.read(jpegBase64);
        // 将图像转换为PNG格式，并返回PNG Base64编码的图像
    ...
    app.post("/api/:model/sdapi/v1/txt2img", async (req, res) => {
        ...
        if (!wholeModel) {
            ...
            if (result.code === 0) {
                ...
                const png64 = await getPng64(result.path)
