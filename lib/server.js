    ...
    app.post("/api/:model/sdapi/v1/txt2img", async (req, res) => {
        ...
        try {
            ...
            if (!wholemodel) {
                ...
                logger.info(req, `ip地址：${req.headers["x-real-ip"]? req.headers["x-real-ip"] : req.remoteAddress}的用户开始生成图片，使用模型：${wholemodel}，prompt：\n${prompt}`);
                const result = await painter.draw(engine, prompt, config);
                const endTime = new Date().getTime();
                logger.info(req, `ip地址：${req.headers["x-real-ip"]? req.headers["x-real-ip"] : req.remoteAddress}的用户图片生成完成，耗时：${(endTime - beginTime) / 1000}s`);
                if (result.code === 0) {
