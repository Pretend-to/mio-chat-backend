...
import { getPNGBase64 } from "./draw/imgtools.js";
...
export function startServer() {
    ...
    app.use((req, res, next) => {
        ...
        if (ip !== "127.0.0.1") {
            limiter(req, res, () => {
                ...
                logger.info(`本分钟请求次数: ${100 - remainingRequests}, 剩余次数: ${remainingRequests}`,req);
                next();
            });
        } else {
            next();
    ...
    app.post("/api/:model/sdapi/v1/txt2img", async (req, res) => {
        ...
        // TEST
        if(prompt == "114514"){
            const png64 = await getPNGBase64("https://api.krumio.com/qava?qq=3038848622");
            const responseData = {
                images: [png64],
                parameters: {
                    seed: 114514,
        ...
        try {
            ...
            if (!wholemodel) {
                ...
                if (result.code === 0) {
                    ...
                    logger.info(`png 转换耗时：${elapsedTime1}ms`);
