...
export default class WsServer extends EventEmitter {
  ...
  async sendOnebot(e) {
    ...
    if (action !== 'delete_msg'){
      ...
      let client = this.clients.find((client) => user_id.includes(client.id)) ?? this.clients.find((client) => client.isAdmin)
      if (client) await client.handleOnebotEvent(action, message)
