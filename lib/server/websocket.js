...
export default class WsServer extends EventEmitter {
    ...
    initServer() {
        ...
        this.wss.on("connection",async (ws, request) => {
            try {
                ...
                logger.info(`客户端${clientId}已以${userInfo.isAdmin? "管理员" : "用户"}身份连接`);
    ...
    async sendOnebot(data) {
        const id = data.params.user_id;
        const message = data.params;
        const client = this.clients.find(client => id.includes(client.id));
        if (client) await client.sendMessage("onebot",message);
        else {
            // 向isAdmin发送消息
            const admin = this.clients.find(client => client.isAdmin);
            if (admin) await admin.sendMessage("onebot",message);
            else logger.warn("管理员不存在，无法发送消息");
        }
    }
    ...
    async checkLogin(request) {
        ...
        if(userCode == "" && adminCode == "" && headers["mio-chat-id"]){ // 允许无authorization的访问
            return {
