...
export function authSocketConnection(socket, next) {
  const { token, id, firstLogin } = socket.handshake.auth

  if (!id || id === 'undefined' || id === 'null') {
    logger.error(`Socket.io连接失败: 缺少ID (收到: ${JSON.stringify(id)})`)
  ...
  const userInfo = {
    firstLogin: firstLogin === true || firstLogin === 'true',
    id: id,
...
function getDomain(url) {
  ...
  try {
    ...
    logger.error(`解析URL失败: ${url}, 错误: ${error.message}`)
