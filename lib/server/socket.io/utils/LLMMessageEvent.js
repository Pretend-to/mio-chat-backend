...
export default class LLMMessageEvent {
  constructor(req, client) {
    ...
    this.startTime = Date.now() // 后端发起请求的绝对时间戳
  }
  ...
  async _checkAndSummarize() {
    ...
    if (currentCount === 1 || (currentCount > 1 && (currentCount - 1) % 6 === 0)) {
      ...
      logger.info(`[AutoTitle] Triggering title generation for contactor ${this.metaData.contactorId} (count: ${currentCount})`)
      
      const newTitle = await llmService.generateChatTitle(titleContext)
      
      if (newTitle) {
        logger.info(`[AutoTitle] New title generated: ${newTitle}`)
