...
import { getPluginToolNames } from '../../../chat/llm/toolPolicy.js'
import {
...
export function buildChannelHistory(chat, payload = {}, ctx = {}) {
  ...
  const mappedMessages = slicedChat.map((item, idx) => {
    ...
    if (Array.isArray(item.content) && item.content.length > 0) {
      content = coalesceCrystallizeEvents(item.content).map((elm) => {
    ...
    switch (type) {
      ...
      case 'get_tools': {
        ...
        client.send({
          ...
          data: {
            ...
            mutable: false,
            plugin: 'ai-plugin',
            tools: getPluginToolNames('ai-plugin', { channel }),
          },
          message: 'ok',
          protocol: 'channel',
          request_id,
      ...
      case 'save_tools': {
        ...
        client.send({
          ...
          data: {
            ignored: true,
            message: 'Channel 工具策略固定为完整 ai-plugin',
            success: true,
          },
          message: 'ok',
          protocol: 'channel',
