 ...
 *
 * 时间戳兜底策略：存量消息可能缺失 time/create_time_ms。此前用 Date.now() 临时兜底，
 ...
 * @param {object} ctx - { sessionId, channel }
...
export function buildChannelHistory(chat, payload = {}, ctx = {}) {
  ...
  // 锚点：首条带真实时间戳的消息；若全无时间戳则用当前时间兜底（仅单次调用内一致）
  let firstRealIdx = -1
  let firstRealTime = 0
  for (let i = 0; i < chat.length; i++) {
    const t = chat[i].time || chat[i].create_time_ms
    if (t) {
      firstRealIdx = i
      firstRealTime = t
      break
  ...
  const resolveTime = (item, chatIndex) => {
    const real = item.time || item.create_time_ms
    if (real) {return real}
    if (firstRealIdx === -1) {
