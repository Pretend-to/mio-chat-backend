...
export class LogStreamController {
  ...
  validateSearchQuery(query) {
    ...
    if (query.page && (query.page < 1 || query.page > 10_000)) {
  ...
  validateExportOptions(options) {
    ...
    if (options.maxRecords && (options.maxRecords < 1 || options.maxRecords > 50_000)) {
   ...
   */
  getActiveExports() {
    return [...this.activeExports.entries()].map(([id, task]) => ({
      id,
