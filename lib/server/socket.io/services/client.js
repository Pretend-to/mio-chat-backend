...
export default class WebUser extends EventEmitter {
  constructor(info, socket) {
    ...
    if (!(cachedClient.activeEvents instanceof Map)) {
      cachedClient.activeEvents = new Map()
    }
    ...
    this.socket.onAnyOutgoing((eventName, ...args) => {
      ...
      this._finishSocketRequestFromResponse(response)
    })
    ...
    this.socket.on('message', (message) => {
      ...
      if (
        e.protocol !== 'console' &&
        !(e.protocol === 'llm' && e.type === 'abort')
      ) {
        this.requestTimings.set(String(e.request_id), {
          protocol: e.protocol,
      ...
      if (e.protocol === 'onebot') {
        ...
        this.finishSocketRequest(e.request_id, 'completed', 'forwarded')
        ...
        if (e.type === 'abort') {
          const timing = this.requestTimings.get(String(e.request_id))
          if (timing) timing.status = 'aborted'
          const abortStartedAt = process.hrtime.bigint()
          this.abortConnection(e.request_id)
          this._logSocketRequest(
            e.protocol,
            e.type,
            e.request_id,
            Number(process.hrtime.bigint() - abortStartedAt) / 1e6,
            'completed',
          )
        } else {
          if (e.type === 'completions') {
            logger.info(
              `[LLMTrace] requestId=${e.request_id} stage=socket.received ` +
                `contactorId=${e.metaData?.contactorId || 'unknown'} ` +
                `messageId=${e.metaData?.messageId || 'unknown'}`,
            )
          }
    ...
    this.socket.on('tool:interact', (data, ack) => {
      ...
      // 异步 SubAgent 没有自己的 Web Socket，先从统一审批中心回传给原任务。
      if (
        ...
        logger.info(
          `[Socket] 已将审批反馈派发给异步 Agent 任务 ${interactionId}`,
        )
      ...
      // 2. 动态注入 YOLO 状态：当用户开启 YOLO 模式时，同步更新当前活跃事件的 settings.yolo
      if (payload?.trustedAgent || payload?.yolo) {
        if (event.settings && typeof event.settings === 'object') {
          event.settings.yolo = true
        }
        if (event.body?.settings && typeof event.body.settings === 'object') {
          event.body.settings.yolo = true
        }
        logger.info(`[Socket] 已为活跃会话 ${requestId} 动态开启 YOLO 免审模式`)
      }

      // 3. 将数据注入挂起的 Promise，恢复大模型工具执行
  ...
  abortConnection(id, broadcast = true) {
    ...
    if (event) {
      const timing = this.requestTimings.get(String(event.requestId || id))
      if (timing) timing.status = 'aborted'
      event.abort()
    }
  ...
  handleOnebotEvent(data) {
    ...
    const {params} = data
    logger.info(
      `[OneBot] 收到应用端事件 action=${type} user_id=${params?.user_id ?? '-'} group_id=${params?.group_id ?? '-'}`,
    )
    if (['send_group_msg', 'send_private_msg', 'send_msg'].includes(type)) {
      ...
      this.sendMessage('onebot', {
        ...
        type: 'message',
        // 修正：身份在 params 里，data 只是 { action, params, echo }
        user_id: params.user_id || params.group_id,
      })
    } else {
  ...
  async login() {
    ...
    // Rehydrate the application-level approval queue immediately after login,
    // even if the user has not opened the originating Agent conversation.
    approvalNotificationBroker.notifyClient(this)
  }
  ...
  finishSocketRequest(requestId, status = 'completed', responseType = 'handled') {
    const key = String(requestId)
    const timing = this.requestTimings.get(key)
    if (!timing) return

    const durationMs = Number(process.hrtime.bigint() - timing.receivedAt) / 1e6
    this._logSocketRequest(
      timing.protocol,
      timing.type,
      requestId,
  ...
  _finishSocketRequestFromResponse(response) {
    if (!response || response.request_id == null) return

    const key = String(response.request_id)
    const timing = this.requestTimings.get(key)
    if (!timing) return

    const responseType = response.message || response.type || 'response'
    if (
      timing.protocol === 'llm' &&
      !['complete', 'failed'].includes(response.message)
    ) {
      return
    }
    if (timing.type === 'enter_chat') return
    if (timing.protocol === 'onebot') return
    if (timing.protocol === 'logs' && response.type === 'EXPORT_STARTED') return
    ...
    this.finishSocketRequest(response.request_id, status, responseType)
  }

  _logSocketRequest(protocol, type, requestId, durationMs, status, responseType = '') {
    const responseLabel = responseType ? `, response=${responseType}` : ''
    logger.info(
      `[Socket.IO][${this.ip || 'unknown'}] ${protocol}/${type} ${requestId} ` +
        `${durationMs.toFixed(1)}ms (${status}${responseLabel})`,
    )
  }
