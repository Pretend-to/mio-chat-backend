...
export default class WebUser extends EventEmitter {
  ...
  sendOpenaiMessage(type, data = {}, requestId) {
    ...
    if (type === 'complete' || type === 'failed') {
      const timing = this.requestTimings.get(String(requestId))
      if (timing) {
        const durationMs = Number(process.hrtime.bigint() - timing.receivedAt) / 1e6
        const status = type === 'failed' ? 'failed' : timing.status
        logger.info(
          `[Socket.IO][${this.ip || 'unknown'}] ${timing.protocol}/${timing.type} ${requestId} ${durationMs.toFixed(1)}ms (${status})`,
        )
        this.requestTimings.delete(String(requestId))
      }
    }
  }
