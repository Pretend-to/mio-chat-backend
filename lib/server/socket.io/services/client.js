...
export default class WebUser extends EventEmitter {
  constructor(info, socket) {
    ...
    this.activeEvents = cachedClient.activeEvents // requestId -> LLMMessageEvent
  ...
  async login() {
    ...
    // 获取推荐预设，仅在前端初次加载（没有联系人）时才拉取，避免每次重连都传输庞大的预设历史
    let recommendedPresets = []
    if (this.firstLogin) {
      logger.info(`[Login] 检测到用户 ${this.id} 首次登录，正在拉取推荐预设...`)
      const presets = await (await import('../../../database/services/PresetService.js')).default.findAll()
      recommendedPresets = presets.filter(p => p.recommended).map(p => ({
        name: p.name,
        avatar: p.avatar,
        opening: p.opening,
        history: p.history,
