...
class StreamCacheService {
  ...
  _mergeChunk(item, chunk) {
    ...
    if (lastChunk && lastChunk.type === chunk.type && typeof chunk.content === 'string') {
      ...
      if (existingToolCall) {
        ...
        // A later tool-call state can contain metadata that was absent from the
        // first streamed chunk. Keep it so replayed history can reconstruct
        // Gemini's serial turns and preserve its signed function-call parts.
        if (chunk.content.step !== undefined) {existingToolCall.content.step = chunk.content.step}
        if (chunk.content.thoughtSignature) {
          existingToolCall.content.thoughtSignature = chunk.content.thoughtSignature
        }
  ...
  snapshot(userId, contactorId) {
    ...
    if (!messages || messages.length === 0) {
      // 容纳后台渠道与定时任务在 admin 下沉淀的共享流快照
      const adminKey = `admin:${contactorId}`
      messages = this.cache.get(adminKey)
    }
  ...
  delete(userId, contactorId) {
    const keys = [`${userId}:${contactorId}`, `admin:${contactorId}`]
