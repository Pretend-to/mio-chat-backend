  ...
  initClient(client, offlineCallback) {
    client.on('llm_message', (req) => {
      ...
      if (req.type === 'enter_chat') {
        ...
        const cachedList = streamCache.snapshot(client.id, contactorId)
        if (cachedList && Array.isArray(cachedList)) {
          ...
          cachedList.forEach(cached => {
            ...
            if (cached.metaData?.isTask && cached.metaData?.executionId) {
              TaskExecutionService.markSynced(cached.metaData.executionId).catch(error => {
                logger.error(`[Sync] 标记执行记录 #${cached.metaData.executionId} 为已同步失败:`, error.message)
              })
            }
        ...
        this._syncUnsyncedTaskExecutions(client, contactorId).catch(error => {
