...
class SessionPool {
  ...
  addSession(client) {
    ...
    if (!cachedClient) {
      cachedClient = {
        activeEvents: client.activeEvents || new Map(),
      ...
      if (!(cachedClient.activeEvents instanceof Map)) {
        cachedClient.activeEvents =
          client.activeEvents instanceof Map ? client.activeEvents : new Map()
      }
    }
    ...
    client.initCacheMesageMethod((message) => {
      ...
      if (message.protocol === 'llm') {
        ...
        // Complete / failed 是状态控制帧，不含内容，必须穿透水位线拦截直接转发
        const isControlFrame = message.message === 'complete' || message.message === 'failed'

        // 检查这个 chunk 是否已经在刚刚同步的快照包里了（控制帧豁免）
        if (!isControlFrame && contactorId && streamCache.isAlreadySynced(client.id, contactorId)) {
