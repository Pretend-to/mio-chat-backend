...
export class LogStreamService extends EventEmitter {
  constructor(options = {}) {
    ...
    // 日志缓冲区 - 集成现有的 LogBuffer，直接使用全局 logger 的缓冲区避免丢失启动日志
    this.logBuffer = (logger && typeof logger.getLogBuffer === 'function')
      ? logger.getLogBuffer()
      : new LogBuffer({ maxSize: this.config.bufferSize })
  ...
  init() {
    ...
    logger.debug('LogStreamService 已初始化')
  }
  ...
  handleNewLog(logEntry) {
    try {
      // 添加到缓冲区 (如果使用的是 logger 的缓冲区，则 logger 在 emit 前已添加过，避免重复添加)
      if (!logger || typeof logger.getLogBuffer !== 'function' || this.logBuffer !== logger.getLogBuffer()) {
  ...
  shouldSendLog(logEntry, subscription) {
    ...
    // 检查日志级别
    const levels = { DEBUG: 4, ERROR: 0, INFO: 3, MARK: 2, WARN: 1 }
  ...
  searchLogs(clientId, query) {
    try {
      ...
      if (query.sortBy === 'timestamp') {
        ...
        const levels = { DEBUG: 4, ERROR: 0, INFO: 3, MARK: 2, WARN: 1 }
        logs.sort((a, b) => {
          const aLevel = levels[a.level] || 3
          const bLevel = levels[b.level] || 3
          return query.sortOrder === 'desc' ? bLevel - aLevel : aLevel - bLevel
        })
