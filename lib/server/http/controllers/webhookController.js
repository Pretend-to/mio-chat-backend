...
async function handleWebhook(payload) {
  ...
  // 双重保护：推送分支必须与当前本地工作区分支一致，否则跳过执行
  const currentLocalBranch = await getCurrentLocalBranch()
  if (!currentLocalBranch || branch !== currentLocalBranch) {
    logger.info(
      `⏭️  [Webhook] 推送分支 "${branch}" 与当前本地 Git 分支 "${currentLocalBranch}" 不一致，跳过执行`,
    )
    return {
      currentBranch: currentLocalBranch,
      targetBranch: branch,
      message: 'Skipped, push branch does not match current local branch',
      skipped: true,
    }
  }
  logger.info(`✅ 推送分支与当前分支一致 (${currentLocalBranch})，继续处理`)
...
export async function postWebhook(req, res) {
  ...
  // 1. 核心保护：仅当「本次推送的目标分支 === 当前本地工作区分支」时才响应 Webhook。
  //    既避免推送其他分支（如 codex/*）时覆盖运行中的代码，又让当前分支自己的推送能正常触发部署。
  const currentBranch = await getCurrentLocalBranch()
  const targetBranch = (payload.ref || '').replace('refs/heads/', '')
  if (!currentBranch || !targetBranch || targetBranch !== currentBranch) {
    logger.info(
      `⏭️ [Webhook] 推送分支 "${targetBranch}" 与当前本地工作区分支 "${currentBranch}" 不一致，忽略此 Webhook 请求`,
    )
    res.status(200).json({
      currentBranch,
      targetBranch,
      message: 'Ignored: push branch does not match current local branch',
      skipped: true,
    })
    return
  }
  logger.info(`✅ [Webhook] 推送分支与当前本地分支一致 (${currentBranch})，继续处理`)
