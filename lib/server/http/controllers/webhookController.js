...
/**
 * 检测当前进程的运行环境，返回最佳重启策略
 * 支持三种模式：PM2 / Docker / 裸 Node
 */
function detectProcessManager() {
  // 1. 检测 PM2 环境
  if (process.env.PM2_USAGE || process.env.pm_id !== undefined) {
    // 从 PM2 环境变量获取进程名，如果读不到则查 pm2 list
    const pmName = process.env.PM2_PROCESS_NAME || process.name
    return { type: 'pm2', name: pmName || 'Mio-Chat' }
  }

  // 2. 检测 Docker 容器环境
  try {
    if (fs.existsSync('/.dockerenv')) {
      // 尝试从容器 hostname 获取容器名
      const hostname = fs.readFileSync('/etc/hostname', 'utf-8').trim()
      return { type: 'docker', name: hostname || 'mio-chat-backend' }
    }
    // 另一种检测方式：检查 cgroup
...
function buildRestartCommand(pmInfo) {
  switch (pmInfo.type) {
    case 'pm2': {
      // 先尝试用进程名，若失败则用 pm2.json 中的配置名
      const names = [pmInfo.name, 'Mio-Chat', 'mio-chat-backend']
      return `sleep 3 && for name in ${names.join(' ')}; do pm2 restart "$name" 2>/dev/null && break; done`
    }
    case 'docker': {
      const containerName = process.env.DOCKER_CONTAINER_NAME || pmInfo.name
      return `sleep 3 && docker restart ${containerName}`
    }
    case 'node':
    default: {
      // 裸 Node：获取当前进程启动命令，kill 后原地拉起
      const script = process.env.NODE_SCRIPT || 'app.js'
      return `sleep 3 && kill -15 ${process.pid} ; nohup node ${script} > /dev/null 2>&1 &`
    }
  }
}

const _pmInfo = detectProcessManager()
logger.info(`[Deploy] 进程环境检测: ${_pmInfo.type} → ${_pmInfo.name}`)
