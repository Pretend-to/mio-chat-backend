...
const CRITICAL_PATTERNS = [
  'app.js', 'lib/', 'utils/', 'config/', 'prisma/', 'channels/', 'channel/',
...
function analyzeChanges(changedFiles) {
  ...
  return { hasCritical, needsRestart: hasCritical && changedFiles.length > 0 && !onlyPlugins, onlyPlugins, onlyStatic }
}
...
async function handleWebhook(payload) {
  ...
  if (!allowedBranches.includes(branch)) {
    ...
    return { message: 'Skipped, branch not monitored', skipped: true }
  }
  ...
  try {
    ...
    if (needsInstall) {
      ...
      let installResult
      try {
        installResult = await execPromise('pnpm install --frozen-lockfile --reporter=append-only', { cwd: REPO_DIR, timeout: 60_000 })
      } catch (error) {
        log(`⚠️ lockfile 过期或安装失败，尝试普通安装... 错误: ${error.message}`)
        try {
          installResult = await execPromise('pnpm install --reporter=append-only', { cwd: REPO_DIR, timeout: 60_000 })
        } catch (error) {
    ...
    if (needsRestart) {
      ...
      const actionLabel = { docker: 'Docker', node: '裸 Node', pm2: 'PM2' }[_pmInfo.type] || 'PM2'
