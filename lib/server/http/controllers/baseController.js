...
import {
  connectOnebotAndWait,
  getOnebotState,
} from '../../../chat/onebot/connectOnebot.js'
...
export async function getBaseInfo(req, res) {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      llm_providers: providers_with_defaults, // 包含 default_model 和 description 的 providers
...
async function ensureOnebotPluginOptions() {
  try {
    ...
    const needsMigration =
      ...
      (typeof currentConfig.plugins.options === 'object' &&
        Object.keys(currentConfig.plugins.options).length === 0)
...
export async function setShare(req, res) {
  ...
  const shareDate = await shareService.createShare(contactor, origin)

  res.json(makeStandardResponse(shareDate))
}

// 获取 OneBot 运行与连接状态
export async function getOnebotStatus(req, res) {
  try {
    res.json(makeStandardResponse(getOnebotState()))
  } catch (error) {
    logger.error('获取 OneBot 状态失败:', error)
    res.status(500).json({
      code: 1,
      data: null,
      message: '获取 OneBot 状态失败: ' + error.message,
    })
  }
}

/**
 * 手动触发一次 OneBot 连接尝试（按当前配置重建连接）。
 * 连接是异步的，这里最多等 3 秒抓结果，超时返回「连接中」，前端继续轮询。
 */
export async function reconnectOnebot(req, res) {
  try {
    logger.info('POST /api/onebot/reconnect 手动触发 OneBot 连接')
    const { attempt, state } = await connectOnebotAndWait({ source: 'manual' })
    res.json(makeStandardResponse({ ...state, attempt }))
  } catch (error) {
    logger.error('OneBot 重新连接失败:', error)
    res.status(500).json({
      code: 1,
      data: null,
      message: 'OneBot 重新连接失败: ' + error.message,
    })
  }
}
