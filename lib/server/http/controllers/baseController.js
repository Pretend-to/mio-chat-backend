...
export async function getOnebotStatus(req, res) {
  try {
    const isEnabled = config.onebot?.enable ?? false
    let isConnected = false
    const bot_qq = config.onebot?.bot_qq || ''
    const admin_qq = config.onebot?.admin_qq || ''
    const reverse_ws_url = config.onebot?.reverse_ws_url || ''

    if (isEnabled && global.middleware?.onebot) {
      isConnected = global.middleware.onebot.isAvaliable()
    }

    const response = makeStandardResponse({
      enable: isEnabled,
