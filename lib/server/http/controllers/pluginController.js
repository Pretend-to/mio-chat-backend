...
function managementToolContext() {
  return {
...
function catalogTool(tool) {
  return tool.toCatalogEntry?.(managementToolContext()) || null
}
    ...
    const pluginList = (await Promise.all(
      plugins.map(async (plugin) => {
        ...
        for (const tools of toolsMap.values()) {
          toolCount += [...tools].map(catalogTool).filter(Boolean).length
        }
...
export async function togglePlugin(req, res) {
  try {
    ...
    // `enabled` has one source of truth: PluginConfig.enabled. It must never
    // be mixed into the plugin's configData object.
    const updated = await PluginConfigService.update(
      pluginName,
      currentConfig,
      nextEnabled,
    )
    if (!updated) {
      await PluginConfigService.create(pluginName, currentConfig, nextEnabled)
    }
...
export async function getPluginTools(req, res) {
  try {
    ...
    for (const [group, toolsIterator] of toolsMap.entries()) {
      const toolsArray = [...toolsIterator].map(catalogTool).filter(Boolean)
      if (toolsArray.length === 0) continue
      tools.push({
