  ...
  try {
    ...
    const pluginList = await Promise.all(plugins.map(async plugin => {
      ...
      try {
        ...
        hasConfig = !!dbConfig && !!dbConfig.configData && Object.keys(dbConfig.configData).length > 0
      } catch (error) {
...
export async function updatePluginConfig(req, res) {
  try {
    ...
    // 更新数据库中的插件配置
    const updatedConfig = await PluginConfigService.update(pluginName, newConfig)
...
export async function getPluginConfig(req, res) {
  try {
    ...
    try {
      ...
      logger.warn(`[${pluginName}] 从数据库获取配置失败，回退到插件默认配置:`, dbError.message)
      config = await plugin.getConfig()
    }
