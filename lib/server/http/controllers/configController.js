...
import {
  ...
  getModelMetadataMap as getModelMetadataMapService,
  refreshModels as refreshModelsService,
  resetConfig as resetConfigService,
  testLLMConnection as testLLMConnectionService,
  fetchLLMModels as fetchLLMModelsService,
  testLLMModels as testLLMModelsService,
  testStorageConfig as testStorageConfigService,
...
/**
 * 获取模型元数据映射（Registry 规格决策结果，供前端展示 ctx/watermark/vision/matchSource）
 */
export async function getModelMetadata(req, res) {
  try {
    const metadataMap = getModelMetadataMapService()
    res.json(makeStandardResponse(metadataMap))
  } catch (error) {
    logger.error('获取模型元数据失败:', error)
    res.status(500).json({
      code: 1,
      data: null,
      message: '获取模型元数据失败: ' + error.message,
    })
  }
}
...
export async function updateLLMInstance(req, res) {
  try {
    const { adapterType, instanceId } = req.params
    const instanceConfig = req.body

    const result = await updateLLMInstanceService(
      adapterType,
      instanceId,
      instanceConfig,
    )
...
export async function deleteLLMInstance(req, res) {
  try {
    ...
    const result = await deleteLLMInstanceService(adapterType, instanceId)
 ...
 * 测试存储配置可用性
 */
export async function testStorageConfigEndpoint(req, res) {
  try {
    const testConfig = req.body
    const result = await testStorageConfigService(testConfig)

    res.json(makeStandardResponse(result))
  } catch (error) {
    logger.error('测试存储配置失败:', error)
    res.status(400).json({
      code: 1,
      data: null,
      message: error.message,
...
export async function refreshModels(req, res) {
  try {
    ...
    const result = await refreshModelsService(adapterType, instanceId)
...
export async function testLLMConnection(req, res) {
  try {
    ...
    const result = await testLLMConnectionService(adapterType, testConfig)
    res.json(makeStandardResponse(result))
  } catch (error) {
    logger.error('[testLLMConnection] 测试连通性失败:', error)

    if (
      error.message.includes('无效') ||
      error.message.includes('未配置') ||
      error.message.includes('需要') ||
      error.message.includes('未指定')
    ) {
      return res.status(400).json({
        code: 1,
        data: null,
 ...
 */
export async function fetchLLMModels(req, res) {
  try {
    const { adapterType } = req.params
    const testConfig = req.body

    const result = await fetchLLMModelsService(adapterType, testConfig)
    res.json(makeStandardResponse(result))
  } catch (error) {
    logger.error('[fetchLLMModels] 获取模型失败:', error)

    if (error.message.includes('无效') || error.message.includes('未配置') || error.message.includes('需要')) {
      return res.status(400).json({
        code: 1,
        data: null,
        message: error.message,
