...
export function uploadImage(req, res) {
  try {
    ...
    const md5Hash = crypto.createHash('md5').update(imageBuffer).digest('hex').substring(0, 6) // 截取 MD5 的前 6 位
    ...
    if (fs.existsSync(uploadPath)) {
      logger.info(`POST /api/upload/image: 文件已存在, URL: /api/uploaded/image/${imageName}`)
      return res.json(
        makeStandardResponse({
          url: `/api/uploaded/image/${imageName}`,
        })
      )
    }
    fs.writeFileSync(uploadPath, imageBuffer)
    logger.info(`POST /api/upload/image: 图片上传成功. URL: /api/uploaded/image/${imageName}`)
    res.json(
      makeStandardResponse({
        url: `/api/uploaded/image/${imageName}`,
      })
