...
let aliasMap = { ...BUILTIN_ADAPTER_ALIASES }
const initPromise = (async () => {
  try {
    const list = await getAdapterMetadataList()
    const map = { ...BUILTIN_ADAPTER_ALIASES }
...
export async function getModelAvatar(req, res) {
  ...
  if (!target) {
    ...
    target = (query && aliasMap[query.toLowerCase()]) || query
  }

  // 如果不是直接的模型匹配，则通过别名转换
  if (!isDirectModelMatch) {
    target = (target && aliasMap[target.toLowerCase()]) || target
  }
  ...
  // 1. 优先从内存缓存中直接返回 SVG，彻底消除 301 重定向与公网往返延迟
  if (svgCache.has(fileName)) {
    res.setHeader('Content-Type', 'image/svg+xml')
    res.setHeader('Cache-Control', 'public, max-age=86400, stale-while-revalidate=604800')
    return res.send(svgCache.get(fileName))
  }

  const url = `https://registry.npmmirror.com/@lobehub/icons-static-svg/latest/files/icons/${fileName}`
  logger.debug(`GET /p/mava?model=${model}&provider=${provider}&adapter=${adapter} -> fetch ${url}`)
  ...
  try {
    ...
    logger.debug(`Fetch icon from npmmirror failed: ${e.message}, fallback to redirect`)
  }
...
export async function testImageGeneration(req, res) {
  try {
    const { prompt = 'A cute cat sitting on a desk', adapterId, size = '1024x1024', image, images, strength } = req.body
    const results = await imageService.generate({ prompt, size, image, images, strength }, adapterId)
