...
export function uploadChunk(req, res) {
  try {
    ...
    logger.debug(`POST /api/upload/chunk, 文件名: ${req.file.originalname}, md5: ${md5}, chunkIndex: ${chunkIndex}, totalChunks: ${totalChunks}`)
...
export async function finalizeUpload(req, res) {
  try {
    ...
    logger.info(`POST /api/upload/finalize, 文件名: ${filename}, md5: ${md5}, totalChunks: ${totalChunks}`)
    ...
    const existingFilePath = path.join('output', 'uploaded', 'file', safeFilename)
    
    if (fs.existsSync(existingFilePath)) {
      logger.warn(`POST /api/upload/finalize: MD5为${md5}的文件已存在. 使用现有文件.`)
      return res.json(
        makeStandardResponse({
          url: `/api/uploaded/file/${safeFilename}`,
    ...
    if (chunks.length < parseInt(totalChunks) && fs.existsSync(tempDir)) {
      ...
      if (tempFiles.length > 0) {
        ...
        // 重新排序
        chunks = chunks.sort((a, b) => parseInt(a.split('-')[1]) - parseInt(b.split('-')[1]))
      }
    }
    ...
    if (finalMd5 !== md5) {
      logger.error(`POST /api/upload/finalize: MD5验证失败. 预期: ${md5}, 实际: ${finalMd5}`)
    ...
    // 返回最终文件信息
    logger.info(`POST /api/upload/finalize: 文件上传成功. 文件名: ${finalFilename}, MD5: ${finalMd5}`)
    res.json(
      makeStandardResponse({
...
export function serveGeneratedFile(req, res) {
  ...
  logger.info(`GET /api/generated/${type}/${name} 从 ${filePath}`)
    
  if (!fs.existsSync(filePath)) {
    logger.warn(`GET /api/generated/${type}/${name}: 文件未找到`)
...
export function serveUploadedFile(req, res) {
  ...
  logger.info(`GET /api/uploaded/${type}/${name} 从 ${filePath}`)
