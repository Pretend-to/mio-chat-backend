...
export async function listChannels(req, res) {
  try {
    ...
    for (const c of channels) {
      try {
        const memory = new MemoryStore({ agentId: c.agentId || 'wechat-master', baseDir: d().runtime.memoryBase })
        ...
        if (sid) {
          ...
          if (session?.chat?.length > 0) {
            ...
            c.lastActive = last.time || last.create_time_ms || session.created_at || c.lastActive
          }
        }
      } catch {}
    }
...
export async function getChannel(req, res) {
  try {
    ...
    const memory = runningEntry?.memory || new MemoryStore({ agentId: ch.agentId || 'wechat-master', baseDir: d().runtime.memoryBase })
...
export async function updateChannel(req, res) {
  try {
    ...
    if (tools !== undefined || toolCallMode !== undefined) {
      const memory = runningEntry?.memory || new MemoryStore({ agentId: c.agentId || 'wechat-master', baseDir: d().runtime.memoryBase })
