...
// OpenAI 聚合代理路由
  ...
  // 统一的请求体/文件上传错误处理（multer/busboy multipart 解析失败等）。
  // 必须注册在所有路由之后：4 参数中间件仅在 next(err) 被调用时触发。
  app.use((err, req, res, next) => {
    if (!err) {return next()}
    const isBodyParseError =
      err.name === 'MulterError' ||
      err.name === 'LimitFileSizeError' ||
      /multipart|form|boundary|body/i.test(err.message || '')
    if (isBodyParseError) {
      logger.warn(`[Upload] 请求体解析失败: ${err.message}`)
      return res.status(400).json({
        code: 1,
        data: null,
        message: `上传请求不完整: ${err.message}`,
      })
    }
    next(err)
  })
