...
export async function getPresetsList(options = {}) {
  ...
  try {
    ...
    const result = await PresetService.findWithPagination(
      page,
      nums,
      queryOptions,
    )
    ...
    stats.forEach((stat) => {
      categoryCount[stat.category] =
        (categoryCount[stat.category] || 0) + stat.count
...
export function validatePreset(preset) {
    ...
    for (let i = 0; i < preset.history.length; i++) {
      ...
      if (!msg.role || !msg.content) {
        ...
        if (!validRoles.includes(msg.role)) {
          errors.push(
            `第 ${i + 1} 条消息的 role 无效，必须是 system、user 或 assistant 之一`,
          )
        }
  ...
  if (preset.tools !== undefined) {
    ...
    } else if (preset.tools.length > 100) {
      errors.push('工具数量不能超过 100 个')
    } else {
