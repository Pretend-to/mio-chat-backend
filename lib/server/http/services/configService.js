  ...
  if (configData.onebot) {
    ...
    if (configData.onebot.enable && configData.onebot.reverse_ws_url) {
      ...
      if (!url.startsWith('ws://') && !url.startsWith('wss://')) {
        errors.push('onebot.reverse_ws_url 必须是有效的 WebSocket URL（以 ws:// 或 wss:// 开头）')
      }
    }
    ...
    if (configData.onebot.admin_qq && !/^\d+$/.test(configData.onebot.admin_qq)) {
...
export async function validateConfigSection(section, data) {
  ...
  switch (section) {
    ...
    case 'onebot':
      if (typeof data.enable !== 'undefined' && typeof data.enable !== 'boolean') {
      ...
      if (data.reverse_ws_url && !data.reverse_ws_url.startsWith('ws://') && !data.reverse_ws_url.startsWith('wss://')) {
        errors.push('reverse_ws_url 必须是有效的 WebSocket URL（以 ws:// 或 wss:// 开头）')
      }
        ...
        for (const [adapterType, instances] of Object.entries(config.llm_adapters)) {
          if (Array.isArray(instances)) {
            instances.forEach((inst, index) => {
              ...
              const metadata = global.middleware.llm.instanceMetadata[instanceId]
              if (metadata) {
...
export async function updateConfig(updates) {
  ...
  try {
    ...
    if (updates.onebot) {  // 使用原始 updates 而不是 filteredUpdates
      const currentOnebotSetting = await SystemSettingsService.get('onebot')
      currentOnebotConfig = (currentOnebotSetting && currentOnebotSetting.value) || {}
...
export async function updateConfigSection(section, sectionData) {
  try {
    ...
    if (section === 'server') {
      ...
      const currentOnebotSetting = await SystemSettingsService.get('onebot')
      const currentOnebotConfig = (currentOnebotSetting && currentOnebotSetting.value) || {}
