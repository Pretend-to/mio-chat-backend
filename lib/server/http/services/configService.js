...
export async function getFullConfig() {
  try {
    ...
    const systemLlmCompactPrompt = await SystemSettingsService.get('system_llm_compact_prompt')
    if (systemLlmChannel || systemLlmTitlePrompt || systemLlmCompactPrompt) {
      config.system = {
        system_llm_channel: systemLlmChannel?.value || '',
        system_llm_title_prompt: systemLlmTitlePrompt?.value || '',
        system_llm_compact_prompt: systemLlmCompactPrompt?.value || '',
      }
    }
...
export async function getConfigSection(section) {
  try {
    ...
    return await sanitizeSectionConfig(section, fullConfig[section])
  } catch (error) {
...
export async function updateConfigSection(section, sectionData, options = {}) {
  try {
    ...
    if (section === 'server') {
      ...
      if (filteredData.system_llm_compact_prompt !== undefined) {
        settingsToUpdate.push({
          key: 'system_llm_compact_prompt',
          value: filteredData.system_llm_compact_prompt,
          category: 'system',
          description: '对话压缩（上下文总结）提示词',
        })
      }
    } else {
      // 其他配置节点直接存储
