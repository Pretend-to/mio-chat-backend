...
export async function getFullConfig() {
  try {
        ...
        for (const [adapterType, instances] of Object.entries(
          ...
          if (Array.isArray(instances)) {
            instances.forEach((inst, index) => {
              // 计算 displayName / instanceId
              let enabledCounter = 0
              for (let i = 0; i <= index; i++) {
                if (instances[i]?.enable) {enabledCounter++}
              }
              const instanceId = inst.name || `${adapterType}-${enabledCounter}`
              const metadata =
                global.middleware.llm.instanceMetadata[instanceId]
              if (metadata) {
    ...
    const systemLlmModel = await SystemSettingsService.get('system_llm_model')
    const systemLlmTitlePrompt = await SystemSettingsService.get('system_llm_title_prompt')
    if (systemLlmChannel || systemLlmModel || systemLlmTitlePrompt) {
      config.system = {
        system_llm_channel: systemLlmChannel?.value || '',
        system_llm_model: systemLlmModel?.value || '',
        system_llm_title_prompt: systemLlmTitlePrompt?.value || '',
      }
    }
...
export async function updateConfigSection(section, sectionData, options = {}) {
  try {
    ...
    if (section === 'server') {
      ...
      if (filteredData.system_llm_model !== undefined) {
        settingsToUpdate.push({
          category: 'system',
          description: '系统 LLM 任务执行模型',
          key: 'system_llm_model',
          value: filteredData.system_llm_model,
        })
      }
      if (filteredData.system_llm_title_prompt !== undefined) {
        settingsToUpdate.push({
          category: 'system',
          description: '对话标题自动生成提示词',
          key: 'system_llm_title_prompt',
          value: filteredData.system_llm_title_prompt,
        })
      }
    } else {
      // 其他配置节点直接存储
...
export async function updateLLMInstance(adapterType, index, instanceConfig) {
  ...
  logger.info(
    `更新 ${adapterType} 适配器实例 #${instanceIndex} (${oldInstanceId})`,
  )
  ...
  const updatedInstance =
    updatedConfig.llm_adapters[adapterType]?.[instanceIndex]
...
export async function deleteLLMInstance(adapterType, index) {
  ...
  logger.info(
    `准备删除 ${adapterType} 适配器实例 #${instanceIndex} (${instanceId}: ${displayName})`,
  )
...
export async function refreshModels(adapterType, index) {
  ...
  if (adapterType && index !== undefined) {
    ...
    const instanceConfig =
      currentConfig.llm_adapters?.[adapterType]?.[instanceIndex]
...
export async function testLLMModels(adapterType, testConfig) {
  ...
  try {
    ...
    // 检查模型加载状态是否出错，防止静默失败
    const instanceMetadata = global.middleware.llm.instanceMetadata[tempInstanceId]
    if (instanceMetadata && instanceMetadata.status === 'error') {
      throw new Error(instanceMetadata.errorMsg || '加载模型列表失败')
    }
