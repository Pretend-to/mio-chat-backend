...
export async function prepareInstanceConfig(adapterType, instanceConfig, fullConfig) {
  ...
  if (adapterType === 'vertex') {
    ...
    if (instanceConfig.service_account_json && instanceConfig.service_account_json.trim()) {
      ...
      try {
        const fileContent = fs.readFileSync(instanceConfig.auth_file_path, 'utf8')
        preparedConfig.authConfig = JSON.parse(fileContent)
      } catch (error) {
...
export async function getFullConfig() {
  try {
    ...
    // 移除敏感信息
    const safeConfig = sanitizeConfig(systemSettings)
...
export async function getConfigSection(section) {
  try {
    ...
    const sectionConfig = await SystemSettingsService.getByCategory(section)
  ...
  try {
    ...
    for (const [key, value] of Object.entries(updates)) {
      settingsToUpdate.push({
        key,
        value,
        category: 'general', // 默认分类，可以根据 key 判断
        description: `配置项: ${key}`
      })
    }
...
export async function updateConfigSection(section, sectionData) {
  try {
    ...
    // 更新数据库中的配置节点
    await SystemSettingsService.set(section, sectionData, section, `${section} 配置节点`)
    logger.info(`配置节点 ${section} 已更新到数据库`)
