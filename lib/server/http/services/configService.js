...
export async function getFullConfig() {
  try {
    ...
    const serverMaxRate = await SystemSettingsService.get('server_max_rate_pre_min')
    ...
    if (serverMaxRate && serverMaxRate.value) {
      config.server.max_rate_pre_min = serverMaxRate.value
    }
  ...
  try {
    ...
    for (const [key, value] of Object.entries(filteredUpdates)) {
      if (key === 'debug') {
        ...
        if (value.max_rate_pre_min !== undefined) {
          settingsToUpdate.push({
            key: 'server_max_rate_pre_min',
            value: value.max_rate_pre_min,
            category: 'server',
...
export async function updateConfigSection(section, sectionData) {
  try {
    ...
    if (section === 'server') {
      ...
      if (filteredData.max_rate_pre_min !== undefined) {
        settingsToUpdate.push({
          key: 'server_max_rate_pre_min',
          value: filteredData.max_rate_pre_min,
          category: 'server',
          description: '每分钟最大请求数'
        })
      }
