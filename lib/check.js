...
import { connectOnebot } from './chat/onebot/connectOnebot.js'
...
export async function statusCheck() {
  ...
  logger.debug('✓ 安全配置检查通过（管理员访问码已设置）')

  const enabledProtocols = []

  // 检查是否配置了 OneBot 协议（连接逻辑统一在 connectOnebot，运行期可重建）
  const onebotConfig = config.getOnebotConfig()
  if (onebotConfig.enable) {
    enabledProtocols.push('OneBot')

    logger.debug('正在检查 OneBot 协议配置...')
    const attempt = await connectOnebot({ source: 'startup' })
    if (!attempt.ok) {
      logger.warn(
        `OneBot 协议未启动：${attempt.reason}（可在 Web 管理界面修正后重新连接）`,
      )
    }
  }
  ...
  if (avaliableList.length > 0) {
    ...
    logger.debug('初始化 LLM 模块以支持动态配置...')
    await global.middleware.loadLLMAdapters()
  }
