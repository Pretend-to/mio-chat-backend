...
export class SubAgentRunService {
  ...
  async getDependencyResults(runId, { agentId = null } = {}) {
    const prisma = await this._database()
    const run = await prisma.subAgentRun.findFirst({
      select: {
        agentId: true,
    ...
    return run.dependencies
      .map(({ dependsOn }) => {
        if (
          dependsOn.agentId !== run.agentId ||
          dependsOn.groupId !== run.groupId
        ) {
          throw new DomainError(
            'invalid_subagent_dependency',
            'SubAgent dependencies must belong to the same Agent and RunGroup',
            409,
          )
        }
        if (dependsOn.status !== RUN_STATUS.RESULT_READY) {
          throw new DomainError(
            'subagent_dependency_not_ready',
            'SubAgent dependencies must be ready before execution',
            409,
            { dependencyRunId: dependsOn.id, status: dependsOn.status },
          )
        }
        return dependsOn
      })
      .toSorted((left, right) => left.jobKey.localeCompare(right.jobKey))
  }
  ...
  async transitionRun(runId, nextStatus, patch = {}) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const terminal =
        ...
        nextStatus === RUN_STATUS.INTERRUPTED
  ...
  async recoverStaleRuns({ reason = 'process_restarted' } = {}) {
    const prisma = await this._database()
    return await prisma.$transaction(async (tx) => {
      const staleRuns = await tx.subAgentRun.findMany({
        where: {
          status: { in: [RUN_STATUS.RUNNING, RUN_STATUS.WAITING_TOOL] },
        },
      })
      ...
      for (const run of staleRuns) {
        const errorJson = json({
          ...
          message: 'Process restarted while run was in flight',
          reason,
        })
        const updated = await tx.subAgentRun.update({
          data: {
            ...
            errorJson: run.errorJson || errorJson,
            finishedAt: now,
            leaseExpiresAt: null,
            leaseOwner: null,
        ...
        affectedGroupIds.add(run.groupId)
      }

      for (const groupId of affectedGroupIds) {
        await this._recalculateGroup(tx, groupId)
      }
      return recovered
    })
  }
    ...
    const threshold =
      group.completionPolicy === 'any'
        ...
        : group.completionPolicy === 'quorum'
          ...
          : runs.length
    // `continue` cancels the in-flight Run first and only then creates its
    // replacement. In that window the Run looks finished while the job is not,
    // so CANCELLED/FAILED here would report the group as terminal moments
    // before it is redispatched — and the completion notification would send
    // the parent looking for a replacement worker that already exists.
    const superseded = runs.some(isSupersededByContinuation)
    if (ready >= threshold) status = GROUP_STATUS.COMPLETED
    else if (superseded) status = GROUP_STATUS.WAITING_CHILDREN
