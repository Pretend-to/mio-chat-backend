...
import { normalizeToolPolicyName } from '../chat/llm/toolPolicy.js'
import {
  assertRunTransition,
  GROUP_STATUS,
  RUN_STATUS,
...
export class SubAgentRunService {
  ...
  _assertToolSubset(requested, allowed) {
    const allowedByName = new Map(
      uniqueStrings(allowed).map((item) => [
        normalizeToolPolicyName(item),
        item,
      ]),
    )
    const requestedNames = uniqueStrings(requested)
    const forbidden = requestedNames.filter(
      (tool) => !allowedByName.has(normalizeToolPolicyName(tool)),
    )
    if (forbidden.length) {
      throw new DomainError(
        'subagent_tool_forbidden',
        `SubAgent requested tools outside the parent capability snapshot: ${forbidden.join(', ')}`,
        403,
        { forbidden },
      )
    }
    return requestedNames.map((tool) =>
      allowedByName.get(normalizeToolPolicyName(tool)),
    )
  }
  ...
  async createGroup({
    ...
    const normalizedJobs = jobs.map((job, index) => {
      const requestedTools = Array.isArray(job.tools)
        ? job.tools
        : defaultToolNames
      const tools = this._assertToolSubset(requestedTools, allowedToolNames)
      return {
        ...job,
        key: keys[index],
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const group = await tx.subAgentRunGroup.create({
        data: {
          ...
          deliveryBindingId: deliveryBindingId ? String(deliveryBindingId) : null,
          id: newId('run_group'),
          idempotencyKey,
          originTaskId,
          originTriggerId,
  ...
  async continueRun(runId, { instruction = '' } = {}) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (run.status !== RUN_STATUS.RESULT_READY) {
        throw new DomainError(
          'subagent_result_not_ready',
          'Run must have a completed result before it can continue',
          409,
        )
      }
