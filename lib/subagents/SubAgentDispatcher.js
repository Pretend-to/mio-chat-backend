...
export class SubAgentDispatcher {
  ...
  async abortRun(runId, reason = '') {
    ...
    const current = await this.runService.getRun(run.id)
    if (
      !TERMINAL_RUN_STATUSES.has(current.status) &&
      current.status !== RUN_STATUS.CANCELLED
    ) {
      await this.runService.transitionRun(current.id, RUN_STATUS.CANCELLED, {
        cancelReason: reason || 'stopped',
  ...
  async _wakeParent(group) {
    ...
    // `completionPolicy: any|quorum` lets a group reach a terminal status while
    // children are still running. Report them so the parent can tell "the work
    // I dispatched is finished" apart from "the group already has a worker for
    // that resource, do not dispatch another one".
    const activeRuns = group.runs.filter((run) =>
      ACTIVE_RUN_STATUSES.has(run.status),
    )
    const activeSummary = activeRuns.length
      ? `${activeRuns.length}（${activeRuns
          .map((run) => `${run.id}=${run.status}`)
          .join(', ')}）`
      : '0'
    const wakeMessage = [
      'system：SubAgent RunGroup 已进入终态，请主 Agent 主动读取结果。',
      `groupId: ${group.id}`,
      `status: ${group.status}`,
      `activeRuns: ${activeSummary}`,
      `runIds: ${runIds.join(', ')}`,
      activeRuns.length
        ? 'activeRuns 非 0 表示分组只是满足了完成策略，仍有子任务在跑：不要为这些资源重复派发 SubAgent。'
        : '',
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      try {
        ...
        const receipt = await dispatcher.submitWake({
          ...
          originRef: `subagent_group:${group.id}:revision:${group.revision}`,
          idempotencyKey: `subagent:${group.id}:revision:${group.revision}`,
  ...
  async _executeRun(run) {
    ...
    try {
      const dependencyResults =
        run.dependencies?.length && this.runService.getDependencyResults
          ? await this.runService.getDependencyResults(current.id, {
              agentId: current.agentId,
            })
          : []
      const result = await this.executor.execute(current, dependencyResults)
