...
export class SubAgentDispatcher {
  ...
  async abortRun(runId, reason = '') {
    ...
    if (this.activeRuns.has(run.id)) await this.executor.abort(run)
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      try {
        await this.sessionTurnService.runTurn({
          ...
          isWake: true,
          // The deterministic message id makes retries one logical wake for
          // persistence layers that support message idempotency.
          messageId: `msg_subagent_wake_${group.id}_${group.revision}`,
