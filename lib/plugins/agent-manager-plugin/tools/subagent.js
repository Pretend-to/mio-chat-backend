...
} from '../../../subagents/ResourcePolicy.js'
...
function deliveryBindingId(event) {
  return (
    event.bindingId ||
...
function cleanRun(run) {
  return {
    ...
    role: run.role || run.session?.subagentRole || run.session?.title || run.jobKey,
...
export default class SubAgentTool extends MioFunction {
  constructor({ runtimeFactory = getSubAgentRuntime } = {}) {
    super({
      ...
      description: [
        ...
        '多个 jobs 默认并行；用 dependsOn 指定串行 DAG。每个 job 的 tools 必须是当前对话有效工具的子集，并在创建 Run 时冻结。请只分配完成该子任务真正需要的工具。',
        '未提供 tools 时完整继承当前主 Agent 的有效工具与权限快照，包括终端和文件工具；显式提供 tools 时则使用该快照的子集。高风险操作仍沿用现有安全检查与审批策略。',
        '任务完成后会唤醒主 Agent，但只发送 groupId/runId 和读取指令，不注入结果正文。主 Agent 应使用 status/read_result tool call 主动读取结果；只有确实需要调整结果或执行下一步时，才调用 continue，它会复用原 SubAgent Session 的完整上下文。',
      ].join('\n'),
      name: 'subagent',
      parameters: {
        properties: {
  ...
  async execute(event) {
    ...
    const parentDeliveryBindingId = deliveryBindingId(event)
    ...
    if (params.action === 'capabilities') {
      ...
      const assignableTools = getAssignableBackgroundToolNames(
        parentTools,
        event,
      )
      return {
        assignableTools,
        defaultTools,
        explicitOnlyTools: assignableTools.filter(
          (tool) => !defaultTools.includes(tool),
        ),
        message:
          'SubAgent 默认完整继承当前父上下文的工具权限快照；jobs[].tools 可显式收窄为其子集。',
        success: true,
      }
    }
    ...
    if (params.action === 'spawn' || params.action === 'spawn_batch') {
      ...
      const assignableToolNames = getAssignableBackgroundToolNames(
        parentTools,
        event,
      )
      const jobs =
      ...
      const group = await runtime.runService.createGroup({
        agentId,
        allowedToolNames: assignableToolNames,
        completionPolicy: params.completionPolicy || 'all',
        deliveryBindingId: parentDeliveryBindingId,
      ...
      return {
        ...
        runs: group.runs.map((run) => ({
          ...
          jobKey: run.jobKey,
          objective: run.objective,
          role: run.role || run.session?.subagentRole || params.role || run.jobKey,
          sessionId: run.sessionId,
          status: run.status,
          tools: run.toolNamesJson ? JSON.parse(run.toolNamesJson) : [],
        })),
