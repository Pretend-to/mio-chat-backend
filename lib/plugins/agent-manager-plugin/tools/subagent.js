...
export default class SubAgentTool extends MioFunction {
  constructor({
    ...
    super({
      ...
      description: [
        ...
        '多个 jobs 默认并行；用 dependsOn 指定 DAG。依赖会控制下游启动时机，并将上游摘要和有长度限制的结果正文传入下游上下文；正文过长时可按 runId 调用 read_result。',
        'SubAgent 无条件完整继承当前主 Agent 的有效工具与权限快照，包括终端和文件工具。高风险操作仍沿用现有安全检查与审批策略。',
        '任务完成后会唤醒主 Agent，但只发送 groupId/runId 和读取指令，不注入结果正文。主 Agent 应使用 subagent action=status/read_result tool call 主动读取结果；若需要对已完成、已停止或正在运行的 SubAgent 进行调整或执行下一步，可调用 continue 并传入新指令（若 SubAgent 正在运行会自动打断并无缝转向新指示），复用原 SubAgent Session 的完整上下文。',
        'read_result 的 resultText/summary 只是最终答复，子 Agent 的过程叙述单独在 result.trace；不要从 resultText 里找结论，也不要为此重新派发 SubAgent。',
        'continue 打断正在运行的 Run 属于中间状态，不会产生完成通知：同一 RunGroup 会立刻用同一 Session 继续，不要因为旧 Run 被取消就重新派发。只有 status 到达终态（completed/cancelled/failed）的通知才代表该分组结束。',
        '可用 delete 删除当前 Session 的直接 SubAgent 子 Session。删除会不可恢复地级联清除该子树的消息、Run 和结果；只能删除已停止的 Run，存在 Task/Trigger 依赖时会拒绝。',
      ].join('\n'),
      name: 'subagent',
      parameters: {
        properties: {
          ...
          sessionId: {
            description:
              'delete 时要删除的当前 Session 的直接 SubAgent 子 Session ID',
            type: 'string',
          },
        },
        required: ['action'],
  ...
  async execute(event) {
    ...
    if (params.action === 'delete') {
      ...
      if (!sessionId) {
        return { error: 'delete requires sessionId', success: false }
      }
      const sessions = await this.sessionService.listSessions(agentId, {
      ...
      if (
        ...
        return {
          error: '只能删除当前 Session 的直接 SubAgent 子 Session',
          success: false,
        }
      }
      const result = await this.sessionService.deleteSession(agentId, sessionId)
