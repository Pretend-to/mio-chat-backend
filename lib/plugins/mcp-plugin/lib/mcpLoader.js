import { Client } from '@modelcontextprotocol/sdk/client/index.js'
import { StdioClientTransport } from '@modelcontextprotocol/sdk/client/stdio.js'
import { StreamableHTTPClientTransport } from '@modelcontextprotocol/sdk/client/streamableHttp.js'
import process from 'process'
import { execSync } from 'child_process'
import { MioFunction } from '../../../function.js'

export async function loadMcpPlugins(mcpConfig) {
  // 验证配置文件格式
  let veryfiedConfig
  try {
    veryfiedConfig = veryfyMcpConfig(mcpConfig)
  } catch (error) {
    logger.error(`mcp 配置文件验证失败: ${error.message}`)
    return new Map() // 返回空的 Map
  }

  const groupedTools = new Map()
  const entries = Object.entries(veryfiedConfig)

  // 并行加载所有 MCP 服务，用 allSettled 确保单个失败不影响其他
  const results = await Promise.allSettled(
    entries.map(([name, param]) => loadMcpService(name, param))
  )

  for (let i = 0; i < entries.length; i++) {
    const [name] = entries[i]
    const result = results[i]
    if (result.status === 'fulfilled' && result.value.length > 0) {
      groupedTools.set(name, result.value)
    }
  }

  let toolsCount = 0
  for (const tools of groupedTools.values()) {
    toolsCount += tools.length
  }
  logger.debug(`Mcp 插件加载完成, 共加载了${toolsCount}个工具`)
  return groupedTools
}

export function veryfyMcpConfig(config) {
  // 检查有没有 mcpServers 字段
  if (!config.mcpServers) {
    throw new Error('Mcp 配置文件缺少 mcpServers 字段')
  }

  // 为空的话直接返回空配置（这是正常情况，不应该报错）
  if (Object.keys(config.mcpServers).length === 0) {
    logger.info('MCP 配置为空，没有配置任何服务器')
    return {}
  }

  // 首先检查本地运行环境有没有基础运行命令
  const basicCommands = [
    {
      command: 'node',
      isAvaliable: false, // 不能用 node 那很逆天了
      testCommand: 'node --version',
    },
    {
      command: 'uv',
      isAvaliable: false,
      testCommand: 'uv --version',
    },
    {
      command: 'npx',
      isAvaliable: false,
      testCommand: 'npm --version',
    },
    {
      command: 'pip',
      isAvaliable: false,
      testCommand: 'pip --version',
    },
    {
      command: 'docker',
      isAvaliable: false,
      testCommand: 'docker --version',
    },
    {
      command: 'python',
      isAvaliable: false,
      testCommand: 'python --version',
    },
  ]

  logger.debug('开始检查本地运行环境是否有基础运行命令')

  for (const command of basicCommands) {
    // 检查命令是否存在
    const { command: cmd, testCommand } = command
    try {
      execSync(testCommand, { stdio: 'ignore' }) // 静默执行探测
      command.isAvaliable = true
    } catch {
      // 降级为 debug，因为并不是所有环境都必须有 python/pip
      logger.debug(`[${cmd}] 命令不可用，环境探测跳过`)
    }
  }

  const avaliableList = basicCommands.filter((cmd) => 
    cmd.isAvaliable
  )

  logger.debug(
    `本地运行环境有基础运行命令: ${ 
      avaliableList
        .map((cmd) => {
          return cmd.command
        })
        .join(', ')}`,
  )

  const result = {}
  const serverList = config.mcpServers || {}
  // 检查每个 mcpServer 的字段是否存在和类型是否正确
  for (const server of Object.keys(serverList)) {
    const serverConfig = serverList[server]
    if (serverConfig && (serverConfig.disabled === true || serverConfig.disabled === 'true')) {
      logger.info(`Mcp 插件 ${server} 已被设置为禁用 (disabled)，跳过加载`)
      continue
    }
    // 检查 server.command 是否可用
    const { command, url } = serverConfig
    if (url) {
      result[server] = serverConfig
    } else {
      const matchCommand = basicCommands.find((cmd) => 
        command.includes(cmd.command) && cmd.isAvaliable
      )
      if (!matchCommand) {
        logger.error(`Mcp 插件 ${server} 的命令 ${command} 不可用, 跳过加载`)
        continue
      }
      result[server] = serverConfig
    }
  }

  return result
}

async function loadMcpService(name, param) {
  const tools = []
  const client = new Client(
    {
      name,
      version: '1.0.0',
    },
    {
      capabilities: {
        prompts: {},
        resources: {},
        tools: {},
      },
    },
  )
  let transport

  // 如果有 url 就用 Streamable HTTP 连接，否则用 stdio 连接
  if (param.url) {
    transport = new StreamableHTTPClientTransport(new URL(param.url))
  } else {
    transport = new StdioClientTransport({
      ...param,
      env: getMergedEnv(param.env),
    })
  }

  try {
    logger.debug(`开始加载 Mcp 插件 ${name}`)
    await client.connect(transport)
    const mcpTools = await client.listTools()
    for (const tool of mcpTools.tools) {
      const toolAccess = param.toolAccess?.[tool.name] || param.access || null
      const mcpTool = new mcpFunction(tool, client, toolAccess)
      tools.push(mcpTool)
    }
    logger.mark(
      `Mcp 插件 ${name} 加载成功, 共加载了${mcpTools.tools.length}个工具`,
    )
  } catch (error) {
    logger.error(`Mcp 插件 ${name} 加载失败: ${error.message}`)
    if (error.code === -32_000 && param.command === 'npx') {
      // ENOENT 的 errno 值 (不同系统可能不同)
      logger.warn(
        '这大概率是一个已知问题，您可以通过手动安装 mcp 中的插件来解决。',
      )
      logger.warn(`原命令为：${  param.command  } ${  param.args.join(' ')}`)
    }
  }
  return tools
}

class mcpFunction extends MioFunction {
  constructor(tool, client, access = null) {
    super({
      access,
      description: tool.description,
      name: tool.name,
      parameters: getValidParameters(tool.inputSchema),
    })
    this.client = client
    this.func = this._execute
  }
  async _execute(e) {
    const { params } = e
    const pureName = this.name.split('_mid_')[0]
    return await this.client.callTool({
      arguments: params,
      name: pureName,
    })
  }
}
function getValidParameters(parameters) {
  // 检查 parameters 是否为对象
  if (typeof parameters !== 'object' || parameters === null) {
    throw new Error('parameters 必须是一个对象')
  }

  const validKeys = new Set(['type', 'properties', 'required'])
  const validPropertiesKeys = [
    'type',
    'description',
    'enum',
    'items',
    'properties',
    'oneOf',
  ]

  // 检查每个参数的键是否有效
  for (const key of Object.keys(parameters)) {
    if (!validKeys.has(key)) {
      // 删一下
      delete parameters[key]
    }
    if (key === 'properties') {
      for (const propertie of Object.keys(parameters[key])) {
        for (const subKey of Object.keys(parameters[key][propertie])) {
          if (!validPropertiesKeys.includes(subKey)) {
            // 删一下
            delete parameters[key][propertie][subKey]
            // Logger.warn(`删除不兼容的key：${subKey}`)
          }
        }
      }
    }
  }
  return parameters
}

function getMergedEnv(env = {}) {
  // 合并环境变量
  return { ...process.env, ...env }
}
