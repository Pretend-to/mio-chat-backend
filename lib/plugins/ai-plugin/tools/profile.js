    ...
    super({
      ...
      parameters: {
        properties: {
          ...
          title: {
            description: '（群聊场景可用）设置你在群内的专属头衔或角色名（例如：系统架构师、前端开发专家、安全合规顾问）',
            type: 'string',
          },
          intro: {
            description: '（群聊场景可用）设置你在群内的专长与职责介绍（例如：负责系统架构设计、技术选型与评审，协助团队排查疑难缺陷）',
            type: 'string',
          },
          duty: {
  ...
  async execute(e) {
    ...
    try {
      ...
      if (action === 'update' || action === 'append') {
        ...
        if (!textToSet && !titleToSet && !introToSet) {
          return { error: `执行 ${action} 操作时必须至少提供 prompt、title 或 intro 之一`, success: false }
        }
