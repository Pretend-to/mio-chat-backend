...
export default class ProfileTool extends MioFunction {
  ...
  getDescription(context = null) {
    ...
    const isGroup = evt?.conversationKind === 'group'
  ...
  async execute(e) {
    ...
    const contactorId = e.contactorId
    const memberId = e.member?.id || null
    const memberName = e.member?.name || null
      ...
      if (action === 'update' || action === 'append') {
        ...
        if (memoryStore && typeof memoryStore.writeSoul === 'function' && newPrompt) {
          try {
            ...
            logger.error('[ProfileTool] 写入灵魂设定失败:', err)
          }
        }
