...
export default class vision extends MioFunction {
  ...
  async analyze(e) {
    ...
    try {
      ...
      // Priority: current model > config-specified provider/model.
      // If AI didn't explicitly pass provider/model params and current model
      // Supports vision, let the current model handle it directly.
      const currentModel = e.body?.settings?.base?.model
      if (!paramProvider && !paramModel && this._isVisionModel(currentModel)) {
        logger.info(
  ...
  _isVisionModel(modelName) {
    ...
    // 优先使用模型规格注册中心精确判断 (支持国内新模型与第三方中转)
    try {
      return modelRegistryService.supportsVision(modelName)
    } catch {
      // 极端降级：保留关键词启发式兜底
      const name = String(modelName).toLowerCase()
      return ['gpt-4', 'gpt-5', 'gemini', 'claude-3', 'claude-4', 'doubao', 'mimo']
