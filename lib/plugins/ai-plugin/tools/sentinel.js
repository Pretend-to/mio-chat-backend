...
export default class SentinelTool extends MioFunction {
  constructor({
    ...
    super({
      access: {
        // Sentinel belongs to a persisted server Agent. The transport that
        // delivered the current turn (Web, Channel, cron or wake-up) is not
        // an authorization boundary; Agent ownership is.
  ...
  async execute(e) {
    ...
    const agentId = e.agentId || e.channel?.memory?.agentId || e.user?.agentId
    const currentSessionId = e.sessionId || e.body?.sessionId || e.body?.sid
    const sourceChannelId = e.channelId || null
    if (!agentId || !currentSessionId)
      throw new Error('当前上下文缺少 agentId 或 sessionId')
    const scope = { agentId }
    ...
    switch (action) {
      case 'create': {
        ...
        const resolvedDelivery = await this.deliveryService.resolveDeliveryBinding({
          agentId,
          deliveryChannelId: params.deliveryChannelId,
          deliveryMode: params.deliveryMode || DELIVERY_MODES.DEFAULT,
          event: e,
        })
      ...
      case 'update': {
        ...
        const existing = await getOwnedTrigger(params.id)
        ...
        if (
          Object.hasOwn(params, 'deliveryMode') ||
          Object.hasOwn(params, 'deliveryChannelId')
        ) {
          const effectiveMode =
            params.deliveryMode ||
          ...
          const resolvedDelivery = await this.deliveryService.resolveDeliveryBinding({
            agentId,
            deliveryBindingId: existing.deliveryBindingId,
            deliveryChannelId: params.deliveryChannelId,
            deliveryMode: effectiveMode,
            event: e,
          })
          patch.deliveryMode = effectiveMode
          patch.deliveryBindingId = resolvedDelivery.binding?.id || null
        }
        ...
        const restart =
          Boolean(params.restart) ||
          ['mode', 'params', 'scriptPath', 'sessionId'].some((field) =>
