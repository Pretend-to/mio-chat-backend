...
export default class ManageScheduledTasks extends MioFunction {
      ...
      parameters: {
        ...
        properties: {
          ...
          cron: {
            type: 'string',
            description:
              '执行频率或时间。支持：Cron (0 9 * * *)、相对时间 (如 +1h45m, +10m, +1.5h, 必须以 + 开头，多层可叠加) 或 "once" (立即执行)',
          },
          taskPrompt: {
            type: 'string',
          ...
          inheritPersona: {
            type: 'boolean',
            description: '是否继承当前对话的人格（System Prompt）。默认为 false。若设为 true 则任务执行时将携带当前人格设定并继承所有工具。',
          },
          allowedTools: {
            type: 'array',
            items: { type: 'string' },
            description: '任务执行时允许使用的工具列表（Skill 名称）。仅在 inheritPersona 为 false 时生效。',
          },
          shWhitelist: {
            type: 'string',
            description: 'Shell 命令白名单（允许在后台定时任务中自动运行的命令规则）。多个命令可用逗号或换行分隔。非白名单命令在后台直接拒绝。',
          },
        },
        required: ['action'],
      },
  ...
  async execute(e) {
    const { action, taskId, taskName, cron, taskPrompt, shWhitelist } = e.params
    ...
    switch (action) {
      ...
      case 'update': {
        ...
        if (e.body) {
          ...
          if (inheritPersona) {
            // 1. 继承人格：直接从预设历史中获取（因为这里是纯净的人格定义，还没被注入 skills）
