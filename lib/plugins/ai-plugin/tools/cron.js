    ...
    super({
      ...
      parameters: {
        properties: {
          ...
          shWhitelist: {
            description:
              '后台 shell 命令白名单。多个命令用逗号分隔，如 "curl,cat,node"。\n⚠️ 重要：任务若需执行 shell 命令（bash），必须在此白名单中列出命令名（不含路径和参数）。未在白名单中的命令会在后台被安全拦截并拒绝执行。\n注意：仅当 inheritPersona 为 true 且当前对话拥有 bash 工具时，或在 allowedTools 中显式添加了终端工具时，此白名单才有效。',
            type: 'string',
          },
          systemPrompt: {
            description:
  ...
  async execute(e) {
    ...
    switch (action) {
      ...
      case 'disable': {
        ...
        return { message: `任务 "${taskId}" 已禁用`, success: true }
      }
      ...
      case 'enable': {
        ...
        return { message: `任务 "${taskId}" 已重新启用`, success: true }
      }
