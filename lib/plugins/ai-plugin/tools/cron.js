...
export default class ManageScheduledTasks extends MioFunction {
  ...
  async execute(e) {
    ...
    switch (action) {
      case 'list': {
        const agentId = e.agentId || e.channel?.memory?.agentId || e.memory?.agentId
      ...
      case 'update': {
        ...
        const deliveryBindingId = e.bindingId || e.channelBindingId || e.body?.bindingId || null
        if (!agentId || !sessionId) throw new Error('当前上下文缺少 agentId 或 sessionId，不能创建任务')
