...
export default class draw extends MioFunction {
    ...
    super({
      ...
      parameters: {
        properties: {
          ...
          images: {
            description: 'Optional list of reference images for adapters/models that support multi-image editing. Do not repeat the value supplied in image.',
            items: { type: 'string' },
            type: 'array',
          },
          size: {
          ...
          adapterId: {
            default: '',
            description: 'Optional: Use a specific enabled image adapter INSTANCE NAME (fill in the 「」name shown in the channel list below, e.g. "Google-Gemini"). Do NOT pass the adapter type like "sd-webui" / "openai-image". Leave empty to use the system default.',
            type: 'string',
          },
          negativePrompt: {
            default: '',
          ...
          strength: {
            default: 0.7,
            description: 'Optional denoising strength for adapters that expose diffusion controls (currently SD WebUI and Tukuai), from 0.0 to 1.0.',
            type: 'number',
          }
        },
        required: ['prompt'],
      ...
      for (const [key, adapter] of imageService.instances.entries()) {
        ...
        if (!seen.has(name)) {
          ...
          const styleNote = isTagStyle
            ...
            : '自然语言详细描述或词组'
          instances.push(`- 「${name}」(类型: ${type})${isDefault ? ' [当前默认]' : ''}${imageInputNote}：adapterId 填「${name}」；提示词格式建议为【${styleNote}】`)
        }
      }
    }
  ...
  async generateImage(e) {
    const { prompt, image, images, size = '1024x1024', adapterId, negativePrompt, strength } = e.params || {}
