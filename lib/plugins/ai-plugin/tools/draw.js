 ...
 */
function getDrawSkeletonHtml(size = '1024x1024') {
  let containerStyle = 'width: 100%; max-width: 360px; height: 320px;'
  const s = String(size || '').toLowerCase()
  if (s === 'portrait' || s === '9:16') {
    containerStyle = 'width: 100%; max-width: 290px; height: 440px;'
  } else if (s === 'landscape' || s === '16:9') {
    containerStyle = 'width: 100%; max-width: 520px; height: 280px;'
    ...
    if (w && h) {
      if (h > w * 1.2) {
        ...
        containerStyle = 'width: 100%; max-width: 360px; height: 320px;'
      }
    }
  }
...
<div class="ldr-box" style="
  ...
  box-shadow: 0 4px 20px rgba(244, 114, 182, 0.12), 0 1px 3px rgba(0,0,0,0.04);
...
export default class draw extends MioFunction {
  constructor() {
    super({
      description:
        'Generate or edit images based on text prompts or reference images. Supports synchronous streaming skeleton placeholders and instant rendering.',
      name: 'draw',
      parameters: {
        properties: {
          prompt: {
            description:
              'Text prompt describing the image you want to generate or instructions for editing.',
            type: 'string',
          },
          image: {
            description:
              'Optional reference image (URL, Base64 Data URI, or local file path) for image-to-image (img2img) editing.',
            type: 'string',
          },
          images: {
            description:
              'Optional list of reference images for adapters/models that support multi-image editing. Do not repeat the value supplied in image.',
            items: { type: 'string' },
            type: 'array',
          },
          size: {
            default: '1024x1024',
            description:
              'Image size/aspect ratio ("square", "portrait", "landscape", or "1024x1024").',
            type: 'string',
          },
          adapterId: {
            default: '',
            description:
              'Optional: Use a specific enabled image adapter INSTANCE NAME (fill in the 「」name shown in the channel list below, e.g. "Google-Gemini"). Do NOT pass the adapter type like "sd-webui" / "openai-image". Leave empty to use the system default.',
            type: 'string',
          },
          negativePrompt: {
            default: '',
            description:
              'Optional negative prompt describing elements to avoid in the generated image.',
            type: 'string',
          },
          strength: {
            default: 0.7,
            description:
              'Optional denoising strength for adapters that expose diffusion controls (currently SD WebUI and Tukuai), from 0.0 to 1.0.',
            type: 'number',
          },
        },
        required: ['prompt'],
  ...
  getDescription() {
    ...
    if (imageService?.instances) {
      for (const [key, adapter] of imageService.instances.entries()) {
        ...
        if (!seen.has(name)) {
          seen.add(name)
          const isDefault =
            name === imageService.defaultInstanceId || adapter.isDefault
          const type =
            adapter.constructor?.getAdapterMetadata?.()?.type ||
            adapter.name ||
            ''
          const imageInputNote = adapter.supportsImageInput?.()
            ? ' [支持参考图]'
            : ' [仅文生图]'
          ...
          const styleNote = isTagStyle
            ...
            : '自然语言详细描述或词组'
          instances.push(
            `- 「${name}」(类型: ${type})${isDefault ? ' [当前默认]' : ''}${imageInputNote}：adapterId 填「${name}」；提示词格式建议为【${styleNote}】`,
          )
        }
      }
    }
    ...
    const defaultAdapter = imageService?.defaultInstanceId
      ...
      : imageService?.instances?.size > 0
        ? Array.from(imageService.instances.values())[0]
        : null

    const defaultType =
      defaultAdapter?.constructor?.getAdapterMetadata?.()?.type ||
      defaultAdapter?.name ||
      ''
    const isDefaultTagStyle =
      defaultType === 'tukuai-image' || defaultType === 'sd-webui'
    ...
    const adapterListText =
      instances.length > 0
        ? `\n\n系统当前已加载并启用的生图通道（adapterId 请填「」内的实例名，不要填适配器类型名）：\n${instances.join('\n')}`
        : '\n\n当前使用环境变量配置的默认生图通道。'
  ...
  async generateImage(e) {
    ...
    logger.info(
      `[Draw Tool] 接收到生图调用: prompt="${prompt?.slice(0, 100)}...", size="${size}", adapter="${adapterId || '默认'}"${image ? ', 包含参考图' : ''}`,
    )

    // 阶段一：第 0.1 秒立即向前端推送 AnyUI 同款流体与毛玻璃微标骨架屏（沉浸式无壳渲染）
    this.setOuterRender(e, [
      {
        type: 'html',
        immersive: true,
        title: '画面绘制中...',
        html: getDrawSkeletonHtml(size),
      },
    ])
    ...
    try {
      ...
      const results = await imageService.generate(
        { prompt, image, images, size, negativePrompt, strength },
        adapterId || null,
      )
      ...
      logger.info(
        `[Draw Tool] 生图成功! 耗时 ${duration}s, 生成图片数=${validImages.length}, 首图URL=${validImages[0]?.url}`,
      )

      // 阶段三：生图完成，第二次调用 setOuterRender 将骨架屏原地替换为真实高清大图（沉浸式直接出图）
      ...
      return {
        success: true,
        message:
          '生图成功，图片已由前端在消息气泡下方直接完成高清渲染。请勿在回复正文中重复输出 Markdown 图片语法（如 ![]()），直接针对生成的画面进行生动解说、点评或推进对话即可。',
      ...
      const duration = ((Date.now() - startTime) / 1000).toFixed(2)
      logger.error(
        `[Draw Tool] 生图执行异常 (耗时 ${duration}s):`,
        error.message,
      )
      this.setOuterRender(e, [
        {
