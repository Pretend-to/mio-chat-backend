    ...
    if (action === 'toggle') {
      ...
      if (tools && Array.isArray(tools)) {
        for (const t of tools) {
          const base = t.split('_mid_')[0]
          if (allToolsMap.has(t) || allToolsMap.has(base)) {
      ...
      if (memoryStore && typeof memoryStore.setAgentMeta === 'function') {
        try {
          ...
          logger.info(`[ToolsManager] 已成功将更新后的工具列表持久化到渠道记忆存储 (共 ${newToolsList.length} 个工具)`)
        } catch (err) {
      ...
      const noticeText = `🛠️ [工具状态已更新]\n${enabled ? '✅ 已启用' : '🚫 已禁用'}: ${Array.from(targetTools).join(', ')}`
