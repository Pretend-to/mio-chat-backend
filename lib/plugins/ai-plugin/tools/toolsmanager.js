...
export default class ToolsManager extends MioFunction {
    ...
    try {
      ...
      for (const plugin of plugins) {
        ...
        for (const [groupName, toolsArray] of toolsMap.entries()) {
          const visibleTools = toolsArray.filter(t => !t.channelOnly) // channelOnly 工具对 web 端屏蔽
          if (visibleTools.length === 0) {continue}

          const displayedTools = visibleTools.slice(0, 10).map(t => t.name.split('_mid_')[0])
          let toolListStr = displayedTools.join(', ')
          if (visibleTools.length > 10) {
            toolListStr += `, ... (and ${visibleTools.length - 10} more, call action='list' to view all)`
          }
  ...
  async execute(e) {
    ...
    const plugins = global.middleware?.plugins || []
    // 渠道来源判断（与 llm/index.js isChannelEvent 保持一致）
    const isChannelSource = Boolean(
      e?.channel ||
      e?.body?.channel ||
      e?.user?.channel ||
      e?.user?.channelType ||
      e?.user?.role === 'channel_master' ||
      (typeof e?.requestId === 'string' && (e.requestId.startsWith('wechat_') || e.requestId.startsWith('channel_')))
    )
    ...
    for (const plugin of plugins) {
      ...
      for (const [groupName, toolsArray] of toolsMap.entries()) {
        ...
        for (const tool of toolsArray) {
          if (tool.channelOnly && !isChannelSource) continue // channelOnly 仅渠道端可见
