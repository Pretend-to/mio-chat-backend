...
export default class Memory extends MioFunction {
  constructor() {
    super({
      description:
        '管理记忆与事实（支持完整的 CRUD 操作）。支持 local（当前会话短期结晶）与 global（全局跨会话长期事实）两个作用域。对于全局通用事实、用户固定偏好或环境规范，请使用 scope: "global"；对于当前任务进展与短期计划，请使用 scope: "local"。',
      name: 'memory',
      parameters: {
        properties: {
          scope: {
            default: 'local',
            description: '记忆作用域：local (当前联系人会话结晶), global (全局跨会话长期事实库，所有 Agent 共享)',
            enum: ['local', 'global'],
            type: 'string',
          },
          action: {
          ...
          zone: {
            ...
            description: '目标记忆分区（仅在 scope 为 local 结晶时有效），默认为 long_term_profile (用户画像)',
