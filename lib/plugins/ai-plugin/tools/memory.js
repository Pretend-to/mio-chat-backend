...
export default class Memory extends MioFunction {
  ...
  async recordMemory(e) {
    ...
    // Keep local CRUD in a request-only preview until crystallization commits it.
    // Mutating previous_summary here invalidates the next turn's prompt prefix.
    const previousSummary = settings.pending_memory_preview || settings.previous_summary || ''
    // 解析当前 XML 记忆结晶
    ...
    // 请求态预览只用于同一轮内连续 CRUD；正式结晶只允许由压缩流程提交。
    const newSummary = buildXmlFromZones(zones)
    if (body.settings) {
      body.settings.pending_memory_preview = newSummary
    }

    const memoryStore = e.channel?.memory || e.memory
    const sessionId = e.sessionId || e.body?.sessionId || e.channel?.activeJobs?.keys()?.next()?.value
    if (memoryStore && typeof memoryStore.appendPendingMemory === 'function' && sessionId) {
    ...
    } else if (memoryStore && typeof memoryStore.setCrystal === 'function' && sessionId) {
      // 仅兼容没有 pending buffer 的旧 MemoryStore。
      await memoryStore.setCrystal(sessionId, newSummary)
      if (body.settings) body.settings.previous_summary = newSummary
    }
