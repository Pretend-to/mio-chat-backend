    ...
    super({
      ...
      parameters: {
        properties: {
          ...
          schema: {
            description:
              '当 action="call" 时传递给目标工具的参数对象。【注意】：必须严格符合先前通过 action="query" 查询到的参数 Schema 规范。',
            type: 'object',
          },
          tool_name: {
            description:
        ...
        if (typeof plugin.getTools === 'function') {
          ...
          for (const [groupName, toolsArray] of toolsMap) {
            const displayedTools = toolsArray
              .map((tool) => tool.toMetaSummary?.(context) || null)
              .filter(
                (summary) =>
                  summary &&
                  summary.name !== 'meta_tool' &&
                  summary.name !== 'meta_call',
              )
