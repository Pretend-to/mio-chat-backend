...
import { setBrowserProvider, clearBrowserProvider } from '../../chat/search/browserBridge.js'
...
export default class WebPlugin extends Plugin {
  ...
  async initialize() {
    ...
    // 把浏览器能力注入搜索层（搜索的浏览器兜底层需要借这里的共享浏览器实例）
    // 使用注册式注入而非直接 import，避免核心 lib 反向依赖插件
    setBrowserProvider(options => this.getBrowser(options))
    logger.info('[WebPlugin] Browser provider registered to SearchService (browser fallback layer enabled)')
  }
