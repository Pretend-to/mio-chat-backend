...
export default class WebPlugin extends Plugin {
  ...
  async detectActiveSearchEngine() {
    const engines = [
      { id: 'duckduckgo', name: 'DuckDuckGo (HTML)', priority: 1, url: 'https://html.duckduckgo.com' },
      { id: 'bing', name: 'Bing', priority: 2, url: 'https://www.bing.com' },
      { id: 'cn_bing', name: 'CN Bing', priority: 2, url: 'https://cn.bing.com' },
      { id: 'baidu', name: 'Baidu', priority: 3, url: 'https://www.baidu.com' },
      { id: 'google', name: 'Google', priority: 4, url: 'https://www.google.com' }
    ]
    ...
    const checks = await Promise.all(
      engines.map(async (eng) => {
        try {
          ...
          return { ...eng, ok: false, error: error.message }
        }
      })
    )
