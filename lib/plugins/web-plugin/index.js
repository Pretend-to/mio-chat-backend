...
export default class WebPlugin extends Plugin {
  ...
  async initialize() {
    ...
    if (preferredBrowser) {
      ...
      const foundPath = findBrowserPathByType(preferredBrowser)
      if (foundPath) {
  ...
  async autoSniffAndPreWarm() {
    // Priority order: obscura -> chrome -> edge -> chromium
    const obscuraPath = this.findObscuraPath()
    if (obscuraPath) {
      this.obscuraPath = obscuraPath
      this.activeBrowser = 'obscura'
      logger.info(`[WebPlugin] Priority loading: Obscura detected at ${obscuraPath}`)
      await this.startObscuraService()
      return
    }

    const chromePath = findBrowserPathByType('chrome')
    if (chromePath) {
      this.activeBrowser = 'chrome'
      logger.info(`[WebPlugin] Priority loading: Google Chrome detected at ${chromePath}`)
      await this.preWarmBrowser('chrome', chromePath)
      return
    }

    const edgePath = findBrowserPathByType('edge')
    if (edgePath) {
      this.activeBrowser = 'edge'
      logger.info(`[WebPlugin] Priority loading: Microsoft Edge detected at ${edgePath}`)
      await this.preWarmBrowser('edge', edgePath)
      return
    }

    const chromiumPath = findBrowserPathByType('chromium')
    if (chromiumPath) {
      this.activeBrowser = 'chromium'
      logger.info(`[WebPlugin] Priority loading: Chromium detected at ${chromiumPath}`)
      await this.preWarmBrowser('chromium', chromiumPath)
      return
    }

    // None detected: download Obscura in background
    logger.warn(`[WebPlugin] No browsers (Obscura, Chrome, Edge, Chromium) detected. Installing Obscura in background...`)
    this.activeBrowser = 'obscura'
