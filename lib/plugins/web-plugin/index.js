  ...
  _scheduleIdleClose() {
    ...
    this._browserIdleTimer = setTimeout(async () => {
      if (this.chromeBrowser) {
        ...
        try {
          // 绕过 Proxy 拦截，直接关闭底层 browser
          const raw = this.chromeBrowser
          // 试试 process() —— Puppeteer 内部 API 暴露真实 Browser 对象
          const realBrowser = raw.process ? raw : raw
          if (typeof realBrowser.close === 'function') {
