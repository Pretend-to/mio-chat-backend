...
import { formatAssetUrl, resolveOrigin } from '../../../../utils/origin.js'
...
export default class publish extends MioFunction {
  ...
  async pubWebpage(e) {
    const baseUrl = resolveOrigin(e.user?.origin)
    const uid = e.user?.id || 'anonymous'
      ...
      if (stats.isDirectory()) {
        ...
        for (const file of files) {
          ...
          if (
            ...
            (!entryUrl && relativePath.endsWith('.html'))
          ) {
            entryUrl = formatAssetUrl(baseUrl, result.url)
          }
        }

        if (!entryUrl) {
          throw new Error('No HTML entry file found in the directory.')
        }
        return {
          extraRender: [
            {
        ...
        const result = await this._uploadFile(data, deployPath)
        const url = formatAssetUrl(baseUrl, result.url)
        return {
          extraRender: [
            {
              type: 'link',
