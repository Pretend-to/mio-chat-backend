...
export default class vision extends MioFunction {
  ...
  async _analyzeAuto(imageData, prompt) {
    ...
    // We construct a robust mock event to satisfy BaseLLMAdapter requirements
    return new Promise((resolve) => {
      ...
      const mockEvent = {
        ...
        // BaseLLMAdapter expects these methods
        pending: () => logger.debug(`[Vision-Bridge] Request ${requestId} is pending...`),
        update: (chunk) => {
        ...
        onAbort: () => {}, // Mock abort handler
        // Mock client to satisfy base.js
        client: {
          pushEvent: () => {},
          popEvent: () => {},
          pushConnection: () => {},
      ...
      targetInstance.handleChatRequest(mockEvent)
    })
  }
