...
export default class searchInternet extends MioFunction {
  ...
  async checkConnectivity(url) {
    try {
      ...
      const timeout = setTimeout(() => controller.abort(), 1000) // 1 秒超时
  ...
  async scrapeData(e) {
    ...
    try {
      // 优先检测 www.bing.com
      let isWWWBingReachable = await this.checkConnectivity(
        'https://www.bing.com',
      )
      ...
      if (isWWWBingReachable) {
        ...
        let isCNBingReachable = await this.checkConnectivity(
          'https://cn.bing.com',
        )
        ...
        if (isCNBingReachable) {
          ...
          logger.debug(
            'Both www.bing.com and cn.bing.com are unreachable, defaulting to www.bing.com',
          )
      ...
      const parse = async (url) => {
        ...
        try {
          ...
          logger.debug(
            `Parsed page ${url} in ${pageEndTime - pageStartTime} ms`,
          )
      ...
      await Promise.all(
        Array(Math.min(results.length, this.maxConcurrency))
          .fill(null)
          .map(() => processNext()),
      )
      ...
      logger.debug(
        `Finished scrapeData function. Total time taken: ${endTime - startTime} ms`,
      )
