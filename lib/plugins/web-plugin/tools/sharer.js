...
export default class FileSharer extends MioFunction {
  constructor() {
    super({
      name: 'fileSharer',
      description:
        '读取服务器本地文件，上传到存储系统（本地/S3），生成公网可访问的链接。支持各类文件（图片、文档、视频、压缩包等）。注意：若需要分享多个文件或整个目录，请先调用 terminal-plugin 的 executeCommand 使用 zip 命令将其打包成单个文件后再进行分享，严禁为一个一个文件生成多个链接。',
      parameters: {
        type: 'object',
        properties: {
          filePath: {
