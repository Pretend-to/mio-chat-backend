export default class APIParser {
  ...
  get url() {
    ...
    const finalQuery = JSON.parse(
      JSON.stringify(this.query).replace(/\{url\}/g, this.targetUrl),
  ...
  async parse() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          `API Error: ${res.status} - ${res.statusText} for URL: ${apiUrl}`,
        )
      }
      ...
      if (success && content) {
        ...
        logger.warn(
          `API returned no content or indicated failure for URL: ${apiUrl}`,
        )
        return {
          success: false,
          source: 'api',
      ...
      logger.info(
        `API parsing of ${apiUrl} completed in ${duration.toFixed(2)}s`,
      )
    }
  }
}
