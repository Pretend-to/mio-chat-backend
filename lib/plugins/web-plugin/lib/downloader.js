...
export async function testNetwork() {
  ...
  try {
    ...
    logger.debug('[WebPlugin] Google connection test failed or timed out:', error.message)
  }
  ...
  try {
    ...
    logger.debug('[WebPlugin] npmmirror connection test failed or timed out:', error.message)
  }
...
export async function installObscuraInBackground(pluginInstance) {
  ...
  runObscuraInstallation(netStatus, pluginInstance).catch(error => {
    logger.error('[WebPlugin] Background Obscura installation failed:', error.message)
...
async function runObscuraInstallation(netStatus, pluginInstance) {
  ...
  try {
    ...
    lastError = error
    logger.warn(`[WebPlugin] Official download failed: ${error.message}. Retrying via ghproxy mirror...`)
  }
  ...
  if (!success) {
    try {
      ...
      lastError = error
      logger.error(`[WebPlugin] Mirror download failed: ${error.message}`)
    }
  }
...
export async function installChromiumInBackground(_pluginInstance) {
  ...
  runChromiumInstallation(netStatus).catch(error => {
    logger.error('[WebPlugin] Background Chromium installation failed:', error.message)
  })
}
...
async function runChromiumInstallation(netStatus) {
  ...
  if (netStatus.googleConnected) {
    try {
      ...
      logger.warn(`[WebPlugin] Failed to resolve build ID from Google, using stable fallback: ${error.message}`)
