...
export default class ManageLLMAdapter extends MioFunction {
  ...
  async execute(e) {
    const { action, adapterType, index, instanceId, id, instanceConfig } = e.params
    const targetIdentifier = instanceId || id || (index !== undefined ? String(index) : '')

    let approvalPrompt = ''
    const approvalMeta = { action, adapterType, index, instanceId: targetIdentifier, instanceConfig }
    ...
    if (action === 'add') {
      ...
      if (!targetIdentifier && index === undefined) {throw new Error('instanceId or index is required for update action')}
      if (!instanceConfig) {throw new Error('instanceConfig is required for update action')}
      const configStr = JSON.stringify(instanceConfig, null, 2)
      approvalPrompt = `是否授权 LLM 更新 ${adapterType} 适配器实例 #${targetIdentifier} 的配置？\n\`\`\`json\n${configStr}\n\`\`\``
    } else if (action === 'delete') {
      if (!targetIdentifier && index === undefined) {throw new Error('instanceId or index is required for delete action')}
      approvalPrompt = `是否授权 LLM 删除 ${adapterType} 适配器实例 #${targetIdentifier}？`
    } else {
    ...
    if (!approval.approved) {
      ...
      return { error: `[执行终止] 用户拒绝授权此 LLM 适配器配置操作。${reasonMsg}`, success: false }
    }

    try {
      let resolvedId = instanceId || id
      if (!resolvedId && index !== undefined) {
        const { getFullConfig } = await import('../../../server/http/services/configService.js')
        const cfg = await getFullConfig()
        resolvedId = cfg.llm_adapters?.[adapterType]?.[index]?.id || String(index)
      }
      ...
      if (action === 'add') {
        ...
        result = await updateLLMInstance(adapterType, resolvedId, instanceConfig)
      } else if (action === 'delete') {
        result = await deleteLLMInstance(adapterType, resolvedId)
      }
      return { result: this._maskResultApiKeys(result), success: true }
    } catch (error) {
