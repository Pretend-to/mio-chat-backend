  ...
  async execute(e) {
    ...
    try {
      ...
      if (action === 'list_plugins') {
        ...
        const combined = [...new Set([...plugins.map((p) => p.name), ...activePlugins])]
      ...
      if (action === 'get_config') {
        ...
        if (!result) {
          ...
          if (plugin) {
            ...
            return { config: defaultConfig, pluginName, source: 'default', success: true }
          }
          return { error: `Plugin "${pluginName}" not found in database or active memory.`, success: false }
        }
        return { config: result.configData, pluginName, source: 'database', success: true }
      }
      ...
      if (action === 'update_config') {
        ...
        if (!approval.approved) {
          ...
          return { error: `[执行终止] 用户拒绝授权更新插件配置。${reasonMsg}`, success: false }
        }
        ...
        return { message: `Configuration for plugin "${pluginName}" updated and hot-reloaded.`, result, success: true }
      }
