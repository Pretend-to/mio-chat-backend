...
export default class sh extends MioFunction {
  constructor() {
    super({
      name: 'sh',
      description: `Execute a terminal command (shell). NEVER restart the MioChat process (e.g., pnpm dev, pm2 restart) as it kills the current execution context. Destructive actions (rm -rf) or system changes REQUIRE user approval. System: ${os.platform()} (${os.arch()}).`,
      parameters: {
        type: 'object',
        properties: {
          command: {
          ...
          background: {
            type: 'boolean',
            description: 'If true, the command will run in the background and return a processId (e.g., proc_...) immediately. Use "ps" or "kill" with this ID to manage the process.',
            default: false
          },
          waitMs: {
            type: 'number',
  ...
  async execute(e) {
    ...
    if (background) {
      return {
        processId,
        status: 'running',
        message: 'Command started in background. Use ps to poll for results.'
      }
    } else {
        ...
        const timeout = setTimeout(() => {
          if (procInfo.status === 'running') {
            resolve({
              processId,
              status: 'running',
              message: `Command is still running after ${waitMs}ms. It will continue in background. Use ps with processId "${processId}" to poll for completion.`,
