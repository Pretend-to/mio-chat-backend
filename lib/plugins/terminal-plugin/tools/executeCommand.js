...
export default class executeCommand extends MioFunction {
  constructor() {
    super({
      name: 'executeCommand',
      description: `Execute a terminal command. NEVER restart the MioChat process (e.g., pnpm dev, pm2 restart) as it kills the current execution context. Destructive actions (rm -rf) or system changes REQUIRE user approval. System: ${os.platform()} (${os.arch()}).`,
      parameters: {
        type: 'object',
        properties: {
          command: {
