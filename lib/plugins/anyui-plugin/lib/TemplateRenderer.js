...
export function renderJsx(templateCode, variables = {}) {
  ...
  try {
    ...
    throw new Error(`JSX/JS 模板执行失败: ${err.message}`, { cause: err })
  }
...
export function validateTemplate(template) {
  ...
  if (
    ...
    try {
      ...
      if (/^\s*(async\s+)?(function|\([^)]*\)\s*=>|[a-zA-Z_$][\w$]*\s*=>)/.test(fnBody)) {
        void new Function('html', 'vhtml', 'escapeHtml', `return (${fnBody});`)
      } else {
        void new Function('props', 'html', 'vhtml', 'escapeHtml', fnBody)
      }
      return true
    } catch (err) {
      throw new Error(`JSX 模板语法错误: ${err.message}`, { cause: err })
    }
  }
