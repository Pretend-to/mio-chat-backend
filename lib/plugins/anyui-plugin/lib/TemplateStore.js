...
export class TemplateStore {
  ...
  static get(userId, name) {
    ...
    if (fs.existsSync(userMetaPath)) {
      try {
        ...
        meta.isGlobal = false
        meta.variableDocs = meta.variableDocs || {}
        return meta
      } catch {
        // 出错继续尝试全局
      }
  ...
  static save(userId, template, isGlobal = false) {
    ...
    const meta = {
      ...
      variableDocs: (template.variableDocs && typeof template.variableDocs === 'object' && !Array.isArray(template.variableDocs))
        ? template.variableDocs
        : {},
