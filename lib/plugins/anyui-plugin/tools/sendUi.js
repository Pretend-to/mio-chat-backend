      ...
      parameters: {
        ...
        properties: {
          template: {
            type: 'string',
            description:
              '模板库中的模板名（推荐，如全局模板 "gal_dialogue_card"）',
          },
          html: {
            type: 'string',
            description: '内联 HTML 正文（与 template 二选一）',
          },
          variables: {
            type: 'string',
            description:
              '变量值 JSON 对象字符串，如 {"chapterTitle":"第一章","caption":"角色名","text1":"对白..."}',
          },
          imageUrl: {
            type: 'string',
            description:
              '可选：静态图片地址（URL/Base64/本地路径）。若提供且无 prompt，直接显示该图；若同时提供 prompt 则作为参考图',
          },
          imgurl: { type: 'string', description: '可选：imageUrl 的别名' },
          prompt: {
            type: 'string',
            description:
              '可选：生图提示词。直接触发内置异步绘图并自动绑定至卡片',
          },
          ...
          imageSize: {
            ...
            default: '1024x1024',
            description:
              '生图尺寸/比例，如 "1024x1024", "square", "portrait", "landscape"',
          },
          adapterId: {
            type: 'string',
            description: '可选：指定生图适配器名称，默认走系统当前默认生图通道',
          },
        },
        required: [],
      },
      ...
      for (const t of globals) {
        ...
        const varKeys =
          Object.keys(properties).length > 0
            ...
            : t.variables || []
        const varParts = varKeys
          .map((v) => {
            const prop = properties[v] || {}
            const doc = docs[v] || {}
            const label =
              prop.description ||
              (typeof doc === 'string' ? doc : doc.description) ||
              ''
          ...
          .join('、')
        templateLines.push(
          `- 「${t.name}」: ${t.description || ''}${varParts ? ` ｜ 参数 Schema（*必填）：${varParts}` : ''}`,
        )
      }
    } catch {
      /* 模板库暂不可读时仅降级为基础说明 */
  ...
  async execute(e) {
    ...
    if (params.variables) {
      try {
        ...
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed))
          ...
          return {
            success: false,
            error: 'variables 必须是 JSON 对象，如 {"chapterTitle":"序章"}',
          }
      } catch {
    ...
    if (rawPrompt && typeof rawPrompt === 'string' && rawPrompt.trim()) {
      try {
        ...
        console.warn(
          '[send_ui] Auto async image creation warning:',
          imgErr.message,
        )
        vars.imageUrl = rawImageUrl || ''
        vars.imgurl = rawImageUrl || ''
    ...
    if (params.template) {
      ...
      if (!stored)
        return {
          success: false,
          error: `模板「${params.template}」不存在，请先用 define_ui_template 定义，或改用 html 内联发送`,
        }
      ...
      return {
        success: false,
        error: '必须提供 template（模板名）或 html（内联正文）',
      }
    }
