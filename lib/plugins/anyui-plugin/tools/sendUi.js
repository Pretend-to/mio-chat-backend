...
export default class SendUi extends MioFunction {
  ...
  getDescription() {
    ...
    // 动态汇总全局模板：名称 + 一句话描述 + 逐字段中文含义（数据源为模板库 variableDocs，与库内内容实时同步）
    const templateLines = []
    try {
      const globals = TemplateStore.list('global')
      for (const t of globals) {
        const docs = t.variableDocs || {}
        const varParts = (t.variables || []).map((v) => {
          const d = docs[v]
          const label = typeof d === 'string' ? d : (d && d.description) || ''
          return label ? `${v}（${label}）` : v
        }).join('、')
        templateLines.push(`- 「${t.name}」: ${t.description || ''}${varParts ? ` ｜ 字段：${varParts}` : ''}`)
      }
    } catch { /* 模板库暂不可读时仅降级为基础说明 */ }

    const templateSection = templateLines.length > 0
      ? `可用全局模板（字段已附中文含义；也可用 manage_ui_templates list / get 查看 variableDocs 详情）：\n${templateLines.join('\n')}`
      : ''
