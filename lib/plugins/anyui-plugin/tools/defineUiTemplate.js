...
export default class DefineUiTemplate extends MioFunction {
  constructor() {
    super({
      ...
      description:
        ...
        '可通过 variableDocs 为每个字段补充说明（type/required/description），保存后 list/get 都能看到字段含义。',
      parameters: {
        type: 'object',
        properties: {
          ...
          variableDocs: { type: 'string', description: '变量说明，JSON 对象字符串，键为变量名，值为 {type, required, description}。如 {"hp":{"type":"number","required":true,"description":"生命值"}}（可选）' }
        },
        required: ['name', 'html']
      }
    })
  ...
  async execute(e) {
    ...
    const { name, html, description, variables, variableDocs: variableDocsParam } = e.params
    ...
    // 解析可选的 variableDocs（字段说明，list/get 可见）
    let variableDocs = null
    if (variableDocsParam) {
      try {
        const parsed = JSON.parse(variableDocsParam)
    ...
    const meta = TemplateStore.save(userId, { name, html, description, variables: vars, variableDocs })
    return {
      success: true,
