...
export default class TerminalPtyPlugin extends Plugin {  // HOT_RELOAD_TEST: timestamp ${Date.now()}
  ...
  getInitialConfig() {
    return {
      defaultCols: 120,
      defaultRows: 40,
      // Keep PTY canonical input well below MAX_CANON. Larger commands are
      // written to a temporary script and sourced through a short dispatch
      // line, avoiding silent tty truncation (especially with UTF-8 text).
