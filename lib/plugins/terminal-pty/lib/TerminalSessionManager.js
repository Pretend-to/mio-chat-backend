...
export function filterExecOutput(raw, command, marker = '') {
  ...
  const result = lines
    ...
    .filter((line) => !marker || !line.includes(marker))
  ...
  async execCommand(sessionId, command, timeout = 30_000) {
    ...
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        ...
        resolve({
          exitCode: null,
          stdout: filterExecOutput(output, command),
          timedOut: true,
        })
      }, timeout)
      ...
      const onData = (data) => {
        ...
        if (
          ...
          resolve({
            exitCode: 0,
            stdout: filterExecOutput(output, command, marker),
            timedOut: false,
          })
        }
      }
