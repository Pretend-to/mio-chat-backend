...
export default class TerminalSessionManager extends EventEmitter {
    ...
    return new Promise((resolve) => {
      ...
      const cleanup = () => {
        if (!resolved) {
          resolved = true
          clearTimeout(timer)
          try { session.ptyProcess.removeListener('data', onData) } catch (_) {}
        }
      }
  ...
  async _loadPty() {
    try {
      ...
      throw new Error(`[PTY Error] 系统未检测到真实的交互式终端底层驱动包 'node-pty'。若要启用高级 TTY/PTY 会话支持，请在 backend 目录运行 "npm install node-pty" 安装依赖。`)
    }
  }
  ...
  async _loadXtermHeadless() {
    try {
      ...
      throw new Error(`[PTY Error] 系统缺少 PTY ANSI 流式渲染包 '@xterm/headless'。若要启用高级交互式终端，请在 backend 目录运行 "npm install @xterm/headless" 安装依赖。`)
    }
  }
