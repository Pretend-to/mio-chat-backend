...
function cleanTerminalOutput(raw) {
  ...
  // Step 3: Handle CR (\r) and BS (\b) per logical segment
  return normalized
    .split('\n')
    .map((chunk) => {
      // If line contains standalone \r (e.g. carriage returns from progress bars / prompt updates)
      const parts = chunk.split('\r')
      // Process each segment with backspace
      ...
      return resolvedParts.length > 0 ? resolvedParts[resolvedParts.length - 1] : ''
    })
...
export default class TerminalSessionManager extends EventEmitter {
  ...
  readScreen(sessionId, opts = {}) {
    ...
    // Clean text directly from ANSI ring buffer to ensure multiline carriage return updates are correctly resolved
  ...
  async waitForIdle(sessionId, timeoutMs = 30_000) {
    const session = this.sessions.get(sessionId)
    if (!session || session.status !== 'running') {
      return { completed: false, timedOut: false, status: session?.status || 'not_found' }
    }
    const marker = `${SH_DONE_MARKER}_idle_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    const isWin = process.platform === 'win32'
    const enter = isWin ? '\r' : '\n'
    session.ptyProcess.write(
      isWin ? `Write-Output ""; Write-Output ${marker}${enter}` : `echo; echo ${marker}${enter}`
    )
    return new Promise((resolve) => {
      let settled = false
      let timer = null
