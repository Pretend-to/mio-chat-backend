...
export default class bash extends MioFunction {
  constructor() {
    super({
      adminOnly: true,
      description: 'Execute a shell command in a persistent PTY session. IMPORTANT: the working directory (cwd) is BOUND to the session when it is first created and stays fixed for its whole lifetime — reuse sessionId to keep running commands in the same workspace; changing workspace requires ending the session (or letting it expire) and creating a new one with a new cwd. Use sessionId to run commands in parallel or reuse a session. Returns command output, exit status, and session status.',
      name: 'bash',
      parameters: {
        properties: {
