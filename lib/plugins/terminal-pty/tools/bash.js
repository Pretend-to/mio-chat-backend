...
export default class bash extends MioFunction {
  constructor() {
    super({
      ...
      description: 'Execute a shell command in a PTY session, synchronized by default.\n【前台同步 (async: false，默认)】\n- 适用：短小快速的命令（git status、cat、ls、单条构建/脚本）。\n- 行为：同步执行并直接返回 stdout 与 exitCode；等待上限 waitMs（默认 5s、最大 20s）。\n- 超时用三态 status 区分，不要只看有没有 exitCode：\n  · executing：静默期内仍持续有新输出，命令确实还在跑 → 继续 wait(sessionId) 或 read_screen\n  · waiting_for_input：无输出且 PTY 停在续行提示符（> / quote> / heredoc> …）——命令没被 shell 完整收下，它不会自己结束 → 用 bash_input 发 \\u0003 取消后重试\n  · timed_out：无输出且看不出在等输入（静默长命令等），保守上报 → 可 wait / read_screen 继续观察\n  超时回执另带 timedOut: true、idleMs（静默时长）、tailText（屏幕末行），便于自行判定。\n【后台异步 (async: true)】\n- 适用：长时间任务（编译、下载、运行服务、sleep、耗时测试）。\n- 行为：立即返回 sessionId（独立子进程运行，不占用交互会话），可并行启动多个；之后用 wait(sessionId) 实时等待完成、read_screen 查看进度/输出。\n- 注意：命令里接管道（如 `cmd 2>&1 | tail -80`）会把输出缓冲到结束才吐字，执行期间 read_screen 会一直为空；要看实时进度就别接管道。\n【工作区】cwd 在会话创建时绑定为持久工作区，复用 sessionId 期间保持不变；更换工作区需结束旧会话并新建。\n【并行】不同 sessionId 的会话可并行运行。',
      name: 'bash',
      parameters: {
        properties: {
  ...
  async execute(e) {
    ...
    try {
      ...
      if (result.timedOut) {
        // 超时不等于「还在跑」：PTY 可能只是停在续行提示符上等输入（P4-1）。
        // 状态由 TerminalSessionManager.classifyForegroundState 判定。
        const state = result.state || 'executing'
        const response = {
          idleMs: result.idleMs,
          sessionId: session.id,
          status: state,
          stdout: result.stdout,
          tailText: result.tailText,
          timedOut: true,
          waitMs: timeout,
        }
        if (state === 'waiting_for_input') {
          response.hint =
            'PTY 末行是续行提示符，通常是命令没被 shell 完整收下（未闭合引号 / for…done / heredoc 等），它不会自己结束。' +
            '先核对 stdout/tailText 确认这不是命令自己输出的 `>`；确认卡住就发 \\u0003 取消后重试，或换一个会话重新执行。'
          response.message =
            `Command looks stuck waiting for input: after ${timeout}ms the PTY is parked on a continuation prompt` +
            `${result.tailText ? ` (${result.tailText})` : ''} with no output for ${result.idleMs}ms. ` +
            `The session ${session.id} is still available.`
        } else if (state === 'executing') {
          response.message =
            `Command is still running after ${timeout}ms; the PTY session remains available.`
        } else {
          response.message =
            `Command produced no output for ${result.idleMs}ms (waitMs=${timeout}ms); ` +
            `it may still be running, or it may be waiting for input. The PTY session remains available.`
        }
        return response
      }
