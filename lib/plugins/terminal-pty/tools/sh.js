...
Commands run in a real pseudo-terminal, so programs that require isatty()=true work correctly.`,
  ...
  async execute(e) {
    ...
    const approval = await this.requestUserApproval(e, `安全提示：系统检测到高危 Pseudo-Terminal Shell 指令:\n\`${command}\`\n是否授权运行此命令？`)
    if (!approval.approved) {
      const reasonMsg = approval.reason ? ` 理由: ${approval.reason}` : ''
      return `【执行终止】用户拒绝了该 Shell 命令的执行授权。${reasonMsg}`
    }
