...
• Current Workspace: ${process.cwd()}`,
      ...
      parameters: {
        properties: {
          background: { description: 'If true, creates a persistent PTY interactive session. Default: false.', type: 'boolean' },
          ...
          waitMs: { description: 'Max time (ms) to wait before converting to background session. Default 5000, max 20000.', type: 'number' },
        },
        required: ['command'],
        type: 'object',
      },
  ...
  async execute(e) {
    ...
    if (background) {
      ...
      plugin._cmdMap.set(cmdId, { marker, sessionId })
    ...
    try {
      ...
      plugin.processBus.emit('process:done', sessionId, {
        elapsed: 0, exitCode: result.exitCode, status: 'finished',
      })
      ...
      sessions.close(sessionId)
      plugin.processBus.emit('process:done', sessionId, {
        status: 'error', exitCode: null, error: error.message,
      })
      return { processId: sessionId, status: 'error', error: error.message }
    }
  }
}
