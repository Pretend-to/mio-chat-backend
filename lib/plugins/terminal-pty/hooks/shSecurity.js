...
import { getSessionYolo } from '../../../chat/sessionExecutionState.js'
/**
 * Shell 安全拦截 Hook
 *
...
export default class ShSecurityHook extends BaseHook {
  ...
  async execute(ctx) {
    ...
    // 1. yolo 全局/会话免审。会话级开关由 /yolo on|off 设置，必须从
    // 当前聊天 sessionId 读取，不能误用 PTY sessionId。
    const sessionId = event.sessionId || null
    let sessionYolo = false
    try {
      sessionYolo = event.channel?.isSessionYoloEnabled
        ? await event.channel.isSessionYoloEnabled(sessionId)
        : await getSessionYolo(event.memory, sessionId)
    } catch {
      // 存储异常时不能把失败当作 YOLO，继续走正常审批策略。
      sessionYolo = false
    }
    const isYolo = event.settings?.yolo === true || sessionYolo
    if (isYolo) {
      return true
    }
    ...
    try {
      policy = await shellPolicyService.evaluate(command, actualCwd, {
        allowPersistedAllow: !isInteractiveInput,
        allowSafeReadonly,
      })
    } catch (error) {
      logger.warn(
    ...
    // 定时/触发任务仍然无人值守并失败关闭。SubAgent 虽是异步任务，
    // 但可以挂起并把审批请求委托给父 Agent 窗口。
    const isUnattendedTask =
      event.triggerKind === 'task' && !event.subagentRunId
    if (isUnattendedTask) {
