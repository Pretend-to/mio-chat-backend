...
export default class ShSecurityHook extends BaseHook {
  ...
  async execute(ctx) {
    ...
    try {
      ...
      logger.warn('[sh-security-guard] ShellPolicyService 判定失败，按阻断策略处理:', error.message);
      policy = { reason: 'policy-error', verdict: 'block' };
    }
    const isHighRisk = policy.reason === 'force-approval-command' || policy.reason === 'deny-rule-hit';
    ...
    if (approval.rememberType && ['prefix1', 'prefix2'].includes(approval.rememberType)) {
      try {
        const prefixes = shellPolicyService.getCommandPrefixes(command)
        const rememberMatch = approval.rememberType === 'prefix2' ? prefixes.prefix2 : prefixes.prefix1
        ...
        logger.warn('[sh-security-guard] 记住放行规则失败（强制人工确认）:', error.message);
      }
    } else if (approval.rememberType) {
      logger.warn(`[sh-security-guard] 忽略未知白名单粒度: ${approval.rememberType}`)
    }
    return true;
  }
  ...
  _isRememberable(policy, command) {
    ...
    if (policy.reason === 'unsafe-shell-syntax' || policy.reason === 'policy-error') return false
