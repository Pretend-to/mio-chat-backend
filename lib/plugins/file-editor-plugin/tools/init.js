      ...
      parameters: {
        properties: {
          ...
          nodes: {
            ...
            items: {
              properties: {
                content: { description: 'Initial content for files. Ignored for directories.', type: 'string' },
                id: { description: 'Unique identifier for this node.', type: 'string' },
                name: { description: 'Name of the file or directory.', type: 'string' },
                parentId: { description: 'ID of the parent directory. Leave empty for root nodes.', type: 'string' },
                type: { description: 'Type of the node.', enum: ['file', 'dir'], type: 'string' }
              },
    ...
    try {
      ...
      for (const node of nodes) {
        ...
        try {
          ...
          results.errors.push(`Failed to create ${node.name}: ${error.message}`)
        }
      }
