...
export function lineSimilarity(a, b) {
  ...
  return (2 * intersection) / (aStr.length + bStr.length)
}
...
function _trimmedMatch(lines, targetLines, content) {
  ...
  for (let i = 0; i <= trimmedLines.length - trimmedTarget.length; i++) {
    ...
    const pos = _findPosition(content, matchedText)
    if (pos >= 0) {
      return { index: pos, matchedText, method: 'trimmed' }
    }
  }
  return null
}
...
function _wsNormalizedMatch(lines, targetLines, content) {
  ...
  for (let i = 0; i <= normLines.length - normTarget.length; i++) {
    ...
    const pos = _findPosition(content, matchedText)
    if (pos >= 0) {
      return { index: pos, matchedText, method: 'ws-normalized' }
    }
  }
  return null
}
...
function _fuzzyMatch(lines, targetLines, content) {
  const EMPTY = { closest: { bestScore: -1, bestStart: -1, worstFileLine: '', worstLineIdx: -1, worstTargetLine: '' }, match: null }
  ...
  for (let i = 0; i <= lines.length - targetLines.length; i++) {
    ...
    if (avgSim > bestScore) {
      ...
      isAmbiguous = false
    } else if (avgSim > 0 && Math.abs(avgSim - bestScore) < 1e-6) {
      isAmbiguous = true
    }
  }

  const closest = { bestScore, bestStart, worstFileLine, worstLineIdx, worstTargetLine }

  if (bestScore >= 0.6 && bestStart >= 0 && !isAmbiguous) {
