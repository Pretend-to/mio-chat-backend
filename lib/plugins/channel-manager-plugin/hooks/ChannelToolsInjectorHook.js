...
export default class ChannelToolsInjectorHook extends BaseHook {
  ...
  async [HOOK_POINTS.LLM_BEFORE_RECURSION](ctx) {
    ...
    if (currentTools.length === 0) {
      ...
      const defaultTools = getRegisteredSystemToolNames()
      if (Array.isArray(savedTools)) {
        const { migrated, tools: completedTools } = completeToolHashes(savedTools, defaultTools)
        if (migrated) {
          savedTools = completedTools
          if (memoryStore && typeof memoryStore.setAgentMeta === 'function') {
            memoryStore.setAgentMeta('tools', completedTools).catch(() => {})
          }
        }
      }
      ...
      const finalTools = (Array.isArray(savedTools) && savedTools.length > 0)
        ? savedTools
        : (defaultTools.length > 0 ? defaultTools : [])
