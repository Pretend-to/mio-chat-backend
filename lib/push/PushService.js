...
class PushService {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!savedKeys || !savedKeys.value?.publicKey || !savedKeys.value?.privateKey) {
        ...
        await systemSettingsService.set('vapid_keys', generated, {
          category: 'push',
          description: 'Web Push VAPID KeyPair',
        })
  ...
  async addSubscription(sub, meta = {}) {
    ...
    await systemSettingsService.set('push_subscriptions', list, {
      category: 'push',
      description: 'Web Push Subscriptions List',
    })
  ...
  async removeSubscription(endpoint) {
    ...
    if (filtered.length !== list.length) {
      await systemSettingsService.set('push_subscriptions', filtered, {
        category: 'push',
        description: 'Web Push Subscriptions List',
      })
      ...
      list.map(async (sub) => {
        try {
          ...
          if (err.statusCode === 404 || err.statusCode === 410) {
            ...
            logger.warn(`[PushService] 发送推送至 ${sub.endpoint} 失败:`, err.message)
          }
        }
      })
    )
