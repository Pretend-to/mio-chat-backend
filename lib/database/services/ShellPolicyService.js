...
export class ShellPolicyService {
  ...
  /**
   * 首次启动时播种高危 deny 规则（仅当表为空）。清单与前端旧 isHighRiskCommand 保持一致。
   */
  async ensureDefaults() {
    await this.initialize()
    if (!this.prisma.shellAutoApproveRule) return
  ...
  async add(input) {
    ...
    if (!deny) {
      // 防呆：高危命令不允许进入放行名单（与前端旧行为一致：高危只能人工审批）
      if (this.isHighRiskCommand(match)) {
  ...
  async evaluate(command, actualCwd = null) {
    ...
    // 1. deny 优先：高危黑名单命中 → block（不看 cwd，全局生效）
    ...
    // 2. allow：命中命令 + 目录匹配（规则无 cwd=全局；有 cwd=按前缀包含）
    for (const rule of rules) {
      if (rule.deny) continue
  ...
  _toPlain(rule) {
    return { cwd: rule.cwd, deny: rule.deny, id: rule.id, match: rule.match, matchType: rule.matchType }
  }
}
