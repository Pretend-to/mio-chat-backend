...
export class ShellPolicyService {
  ...
  _matchCommand(cmd, rule, analysis = analyzeShellCommand(cmd)) {
    ...
    // Persisted rules may narrow the approved inspection closure, but cannot
    // turn an unknown executable or an unsafe parameter shape into a silent
    // approval. Complex commands are never persisted as an allow path.
    if (!match || !canonicalCommand || !analysis.safe || analysis.compound)
      return false
