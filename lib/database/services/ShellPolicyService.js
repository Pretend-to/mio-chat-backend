...
export class ShellPolicyService {
  ...
  async list() {
    ...
    const rules = await this.prisma.shellAutoApproveRule.findMany({
      orderBy: [{ deny: 'desc' }, { id: 'desc' }],
    })
    return rules.map((r) => ({
      cwd: r.cwd,
      deny: r.deny,
  ...
  async add(input) {
    ...
    if (!deny) {
      ...
      if (this.isForceApprovalCommand(match)) {
        throw new Error(
          `命令 "${match}" 属绝对危险命令，禁止记住或自动放行，必须每次人工确认`,
        )
      }
      if (this.isHighRiskCommand(match) && !input.allowHighRisk) {
        throw new Error(
          `命令 "${match}" 属高危命令，不允许加入自动放行名单，只能人工审批`,
        )
      }
    }
  ...
  async remove(id) {
    await this.initialize()
    const existing = await this.prisma.shellAutoApproveRule.findUnique({
      where: { id: Number(id) },
    })
    if (!existing) throw new Error(`规则 #${id} 不存在`)
    await this.prisma.shellAutoApproveRule.delete({
   ...
   */
  async evaluate(command, actualCwd = null, options = {}) {
    if (!command || !command.trim())
      return { verdict: 'unknown', reason: 'empty-command' }
    const cmd = command.trim()

    // Analyse first: terminal controls must never reach the lexical fallback
    // matcher, and a parser error is fail-closed.
    ...
    if (!analysis.syntaxSafe) {
      return { verdict: 'block', reason: 'unsafe-shell-syntax', ...metadata }
    }

    if (this.isForceApprovalCommand(cmd)) {
      return { verdict: 'block', reason: 'force-approval-command', ...metadata }
    }

    let rules
    try {
      await this.initialize()
      rules = await this.prisma.shellAutoApproveRule.findMany({
        where: { enabled: true },
      })
    } catch {
    ...
    for (const rule of rules) {
      ...
      if (options.allowPersistedAllow === false) continue
      ...
      return {
        verdict: 'allow',
        rule: this._toPlain(rule),
        reason: 'allow-rule-hit',
        ...metadata,
      }
    }
    ...
    if (this.isHighRiskCommand(cmd)) {
      return {
        verdict: 'block',
        rule: this._legacyHighRiskRule(cmd),
        reason: 'deny-rule-hit',
        ...metadata,
      }
    }
    ...
    if (options.allowSafeReadonly !== false && analysis.safe) {
  ...
  _matchCommand(cmd, rule, analysis = analyzeShellCommand(cmd)) {
    ...
    // A persisted allow records a user's explicit authorization. Unlike the
    // built-in fast path it may cover an unknown executable, but it is still
    // limited to one statically plain command: no external executable path,
    // expansion, assignment, redirect, or compound structure.
    if (
      !match ||
      !canonicalCommand ||
      !analysis.syntaxSafe ||
      ...
      analysis.commands[0].path
    )
      return false
    if (rule.matchType === 'prefix') {
      return (
        canonicalCommand === match || canonicalCommand.startsWith(`${match} `)
      )
    }
  ...
  _cwdInScope(actualCwd, scopeCwd) {
    const isWindowsPath = (value) =>
      /^[A-Za-z]:[\\/]/.test(String(value)) || String(value).includes('\\')
    const normalize = (value) => {
      const raw = String(value || '')
      const api = isWindowsPath(raw) ? path.win32 : path.posix
      return api.normalize(api.resolve(raw)).replace(/[\\/]+$/, '')
    }
    let a = normalize(actualCwd)
    let s = normalize(scopeCwd)
    if (isWindowsPath(a) || isWindowsPath(s)) {
      a = a.toLowerCase()
      s = s.toLowerCase()
    }
  ...
  _legacyHighRiskRule(command) {
    // Pre-existing non-empty databases may not receive the seeded deny row.
    // This represents the same tier-2 fallback as those rows: still high
    // risk, but eligible for an explicit, cwd-scoped remembered approval.
    return {
      cwd: null,
      deny: true,
      id: null,
      match: this._getExecutableName(command),
      matchType: 'prefix',
      rememberable: true,
    }
  }
