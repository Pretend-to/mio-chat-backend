...
export class ShellPolicyService {
  ...
  async evaluate(command, actualCwd = null) {
    ...
    // 白名单只允许单一、无展开的命令。命令替换、重定向、进程替换等语法
    // 可能在用户看到的前缀之外执行额外动作，必须 fail-closed。
    if (this.isForceApprovalCommand(cmd)) {
      return { verdict: 'block', reason: 'force-approval-command' }
    }
    if (this._hasUnsafeShellSyntax(cmd)) {
  ...
  _matchCommand(cmd, rule) {
    ...
    if (!match || !canonicalCommand || this._hasUnsafeShellSyntax(cmd) || !this._hasSingleShellCommand(cmd)) return false
  ...
  isForceApprovalCommand(command) {
    ...
    const forceSet = new Set([...FORCE_APPROVAL_EXECS, ...FORCE_APPROVAL_WRAPPERS])
    for (const segment of this._splitShellSegments(command)) {
      ...
      const exe = this._getExecutableName(segment)
      if (/^\.{1,2}\//.test(rawExecutable)) return true
