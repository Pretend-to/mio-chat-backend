...
class PluginConfigService {
  ...
  async setEnabled(pluginName, enabled) {
    try {
      ...
      logger.info(
        `设置插件状态成功: ${pluginName} -> ${enabled ? '启用' : '禁用'}`,
      )
      return this._parseConfig(config)
    } catch (error) {
      if (error.code === 'P2025') {
  ...
  /**
   * 清理孤儿插件配置（数据库中存在但未在活跃插件列表中的配置）
   * @param {string[]} activePluginNames 活跃/存在的插件名称列表
   * @returns {Promise<{ count: number, deleted: string[] }>}
   */
  async cleanOrphans(activePluginNames = []) {
    try {
      const activeSet = new Set(activePluginNames)
      const allConfigs = await this.prisma.pluginConfig.findMany({
        select: { pluginName: true },
      })
      const orphans = allConfigs.filter((c) => !activeSet.has(c.pluginName))
      const deleted = []

      for (const orphan of orphans) {
        const success = await this.delete(orphan.pluginName)
        if (success) {
          deleted.push(orphan.pluginName)
          logger.info(
            `[PluginConfigService] 已清理孤儿插件配置: ${orphan.pluginName}`,
          )
        }
      }
  ...
  async updateManyEnabled(pluginNames, enabled) {
    try {
      ...
      logger.info(
        `批量更新插件状态成功: ${result.count} 个插件 -> ${enabled ? '启用' : '禁用'}`,
      )
      return result.count
    } catch (error) {
  ...
  async importConfigs(configs, overwrite = false) {
    try {
      ...
      logger.info(
        `导入插件配置完成: 创建 ${results.created}, 更新 ${results.updated}, 跳过 ${results.skipped}, 错误 ${results.errors.length}`,
      )
      return results
    } catch (error) {
