...
/**
 * 将旧预设中的 Skill 工具名归一化为 ai-plugin 当前使用的小写名称。
 * 保留实例哈希，并去除归一化后产生的重复项。
 */
export function normalizePresetTools(tools = []) {
  if (!Array.isArray(tools)) return []

  const normalized = []
  for (const tool of tools) {
    const value = typeof tool === 'string'
      ? tool.replace(/^skill(?=_mid_|$)/i, 'skill')
      : tool
...
class PresetService {
    ...
    try {
      const preset = await this.prisma.preset.create({
        data: {
          ...
          tools: JSON.stringify(normalizePresetTools(presetData.tools)),
  ...
  async update(name, presetData) {
    try {
      const preset = await this.prisma.preset.update({
        data: {
          avatar: presetData.avatar !== undefined ? presetData.avatar : undefined,
          category: presetData.category || 'common',
          hidden: presetData.hidden || false,
          history: JSON.stringify(presetData.history || []),
          model: presetData.model !== undefined ? presetData.model : '',
          opening: presetData.opening || '',
          provider: presetData.provider !== undefined ? presetData.provider : '',
          recommended: presetData.recommended || false,
          tools: JSON.stringify(normalizePresetTools(presetData.tools))
        },
        where: { name }
  ...
  async createMany(presetsData) {
    try {
      ...
      for (const preset of presetsData) {
        await this.prisma.preset.upsert({
          create: {
            ...
            tools: JSON.stringify(normalizePresetTools(preset.tools)),
      ...
      logger.info(`批量初始化预设完成: ${count} 个`)
      return { count }
    } catch (error) {
      logger.error('批量初始化预设失败:', error)
      throw error
    }
  }
  ...
  _parsePreset(preset) {
    ...
    try {
      return {
        ...
        tools: normalizePresetTools(JSON.parse(preset.tools))
      }
    } catch (error) {
