...
export default class taskScheduler {
    ...
    async  initSupportChecks() {
        ...
        if (zipAvailable && rmAvailable) {
            ...
            logger.wrong("zip或rm命令不可用，请安装zip和rm命令");
            return false;
        }
    }
    ...
    bakPics = async () => {
        ...
        try {
            ...
            const { stdout: sizeStdout, stderr: sizeStderr } = await this.execCommand("du  -sh ./backup/ai-paint-$(date +%Y%m%d).zip");
            const size = sizeStdout.split(/\s+/)[0];

            logger.info(`成功压缩至 ./backup/ai-paint-$(date +%Y%m%d).zip, 原输出目录大小: ${du}, 备份文件大小: ${size}`);
