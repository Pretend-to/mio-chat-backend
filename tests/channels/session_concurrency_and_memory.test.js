...
async function createPrismaFixture() {
  const databasePath = path.join(
    os.tmpdir(),
    `mio-concurrency-${process.pid}-${crypto.randomUUID()}.db`,
  )
  execFileSync(
...
test('DatabaseMemoryStore pending memories: 缓冲记录与归档清空', async () => {
  const { databasePath, prisma } = await createPrismaFixture()
  const memory = new DatabaseMemoryStore({ agentId: 'test-agent', prisma })
  await memory.ensure()
...
test('BaseChannel Session FIFO Queue: 确保同一 Session 主会话严格串行执行', async () => {
  const { databasePath, prisma } = await createPrismaFixture()
  const memory = new DatabaseMemoryStore({
  ...
  try {
    ...
    // 验证最大并发数严格为 1：Task 1 执行期间 Task 2/3 排队，随后触发批量合并处理
    assert.equal(maxConcurrentExecutions, 1, '同一 session 主任务必须严格串行')
    assert.equal(
      executionOrder.length,
      2,
      'Task 1 单飞后，排队的 Task 2 与 3 应自动合并为一个批次',
    )
    assert.equal(executionOrder[0], 'Task 1')
    assert.ok(
      executionOrder[1].includes('Task 2') &&
        executionOrder[1].includes('Task 3'),
    )
  } finally {
    await prisma.$disconnect()
    fs.rmSync(databasePath, { force: true })
...
test('streamCache fallback: 管理员连接未命中独立 client.id 时能够回退获取 admin 渠道镜像', async () => {
  ...
  assert.equal(
    cachedList.length,
    1,
    '新客户端直接通过 fallback 命中 admin 渠道镜像',
  )
