...
async function createPrismaFixture(agentId) {
  const databasePath = path.join(
    os.tmpdir(),
    `mio-ka-${process.pid}-${crypto.randomUUID()}.db`,
  )
  execFileSync(
...
function makeHarness(memory, keepAlive = {}) {
  const mockClient = {
    botId: 'bot', sendLog: [],
