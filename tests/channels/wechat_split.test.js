...
test('微信分条集成：流式交错多阶段输出（文字 -> 工具执行 -> 最终文字）分条且顺序正确', async () => {
  ...
  channel.llm = {
    process: async (ctx) => {
      ...
      await ctx.onEmitTextBlock('<msg>收到，正在为你检索中～</msg><msg>已连接服务节点</msg>')
      // 模拟工具耗时
      await new Promise(r => setTimeout(r, 20))
      // 阶段 2：工具执行后最终文字
      await ctx.onEmitTextBlock('<msg>查询完毕！</msg>这是详细结果<break/>请确认～')
  ...
  assert.ok(times.every((t, i) => i === 0 || t > times[i - 1]), '时序必须单调递增')
...
test('微信分条集成：多图连发与图文防抖聚合测试', async () => {
  ...
  channel.bufferToImageUrl = async (buf) => `/f/up/image/mock_${Date.now()}_${Math.random().toString(36).slice(2, 6)}.png`
