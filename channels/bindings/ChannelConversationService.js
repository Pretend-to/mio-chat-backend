...
export class ChannelConversationService {
  ...
  async resolve({ agentId, channelId, envelope }, retryCount = 0) {
    ...
    try {
      ...
      let conversation = await tx.channelConversation.findUnique({ where: key })
      if (!conversation) {
        const session = await tx.session.create({
          data: {
            agentId: String(agentId),
            id: newId('session'),
            title: envelope.conversation.type === 'group'
      ...
      if (!conversation.activeSessionId) {
        const session = await tx.session.create({
          data: { agentId: String(agentId), id: newId('session'), title: '渠道会话' },
        })
      ...
      if (error?.code === 'P2002' && retryCount < 2) {
        return await this.resolve({ agentId, channelId, envelope }, retryCount + 1)
      }
      throw error
    }
  }
  ...
  scope(conversationId, agentId) {
    return {
      activate: sessionId => this.activate(conversationId, agentId, sessionId),
      create: title => this.createAndActivate(conversationId, agentId, title),
      delete: sessionId => this.deleteSession(conversationId, agentId, sessionId),
  ...
  async getActive(conversationId, agentId) {
    const prisma = await this._database()
    return (await prisma.channelConversation.findFirst({
  ...
  async listSessions(conversationId, agentId) {
    ...
    const links = await prisma.channelSessionLink.findMany({
      include: { session: { include: { _count: { select: { messages: true } } } } },
  ...
  async createAndActivate(conversationId, agentId, title = '新会话') {
    ...
    return await prisma.$transaction(async tx => {
      ...
      if (!conversation) throw new DomainError('channel_conversation_not_found', 'Channel conversation not found', 404)
      const session = await tx.session.create({
        data: { agentId: String(agentId), id: newId('session'), title: String(title || '新会话') },
      })
      await tx.channelSessionLink.create({
        data: { channelConversationId: conversation.id, sessionId: session.id },
      })
  ...
  async activate(conversationId, agentId, sessionId) {
    ...
    if (!link || link.session.agentId !== String(agentId)) {
      throw new DomainError('session_forbidden', 'Session is not linked to this Channel conversation', 403)
    }
  ...
  async deleteSession(conversationId, agentId, sessionId) {
    ...
    return await prisma.$transaction(async tx => {
      ...
      if (!linked) throw new DomainError('session_forbidden', 'Session is not linked to this Channel conversation', 403)
      ...
      if (activeSessionId === String(sessionId)) {
        activeSessionId = (await tx.channelSessionLink.findFirst({
