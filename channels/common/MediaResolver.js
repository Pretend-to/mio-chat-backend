 ...
 */
export async function resolveBuffer({ buffer, url, localPath }) {
  ...
  if (url) {
    ...
    if (url.startsWith('/f/gen/') || url.startsWith('/f/up/')) {
      const subPath = url.startsWith('/f/gen/')
        ? url.slice('/f/gen/'.length)
        : url.slice('/f/up/'.length)
...
export const MediaResolver = { resolveBuffer }
